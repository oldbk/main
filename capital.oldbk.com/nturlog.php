	...
	$tr = mysql_fetch_array(mysql_query("SELECT * FROM `ntur_logs` WHERE `id` = '".$trrid."'"));
	?>
<HTML>
	<HEAD>
...
<?
		if ($tr['id'] > 0)
		{
		$trd = mysql_fetch_array(mysql_query("SELECT * FROM `ntur_users` WHERE `id` = '".$tr['id'] ."'"));
		$trus = mysql_fetch_array(mysql_query("SELECT * FROM `users` WHERE `id` = '".$trd['o1'] ."'"));
		
		  $nazv[304]='(новичков)';
		  $nazv[308]='(общий)';
		$type=$nazv[$tr[type]];
		$winer=$tr[winer];
		echo "<H3>Отчет о турнире: «Одиночные сражения ".$type.".»</H3>";
		...
		if ($trd['nazva']!='<auto>OLDBK')
			{		
			echo "Создал: <b>".$trus['login']."</b> <a target=_blank href=inf.php?".$trus['id']."><img border=0 src=http://i.oldbk.com/i/inf.gif></a><br><br>";
			}
