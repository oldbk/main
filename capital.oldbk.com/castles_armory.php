...
$all_kom=mysql_query("select * from oldbk.castles_profile where owner='{$user[id]}' and prof>0 ORDER BY def DESC ") or die(); 
...
//////////////////////////////////////////////////////////////////////////////////////
		 ...
		 //
		 $d=(int)($_GET[setdef]);
		  mysql_query("UPDATE oldbk.`castles_profile` set def=0 WHERE `owner`='{$user[id]}' AND `def`=1;") or die("11");
  		  mysql_query("UPDATE oldbk.`castles_profile` set def=1 WHERE `owner`='{$user[id]}' AND `prof`='{$d}' ;") or die("12");
		 ...
		 $dell=(int)($_GET[delcomplect]);
		  ...
		  if ($dell>0)
		  	{
			 mysql_query("DELETE FROM oldbk.`castles_profile` WHERE `owner`='{$user[id]}' AND `prof`='{$dell}';") or die("13");
			unset($arr_kom[$dell]);
...
/////////////////////////////////////////////////////////////////////////////////////////////									
		...
		$asql="INSERT INTO oldbk.`castles_profile` SET `owner`='{$user[id]}',`pname`='{$_POST[savecomplect]}' ,`prof`='{$next_id}', ".$sk_row." ON DUPLICATE KEY UPDATE  ".$sk_row."  ; ";
		//echo $asql;
...
//21-22-23-24-25-26-27-28-210-211
		...
		$us=mysql_query("SELECT * FROM `users` WHERE `id` = '{$_SESSION['uid']}' LIMIT 1;") or die("9");
		  ...
		  if (dressitem_prim ($prim,$ot)==true)
		   ...
		   if ($goto>0)
		     ...
		     $user_real=mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`castles_profile` WHERE  prof=0 and  `owner` = '{$user[id]}' LIMIT 1;"));
		...
		`mlight`='{$user_real[mlight]}',`mgray`='{$user_real[mgray]}',`mdark`='{$user_real[mdark]}', `bpbonushp`='{$user_real[bpbonushp]}'  ";
		      ...
		      //mysql_query('COMMIT') or die();
		      header('Location: castles_tur.php');
		      die();
		     }
		     else
		     {
	 ...
	 if ($nadeto>0)
		...
		//echo "$nadeto";		
		 if ($nadeto!=$heved[0])
			...
			header("Location: castles_armory.php");
			die("");
		 	}
		 	else
		 	{
...
<SCRIPT LANGUAGE="JavaScript">
		...
		function returned2(s){
			location.href='castles_armory.php?'+s+'tmp='+Math.random();
		}
...
<A HREF="castles_armory.php?otdel=11&sid=&0.337606814894404">Оружие: топоры</A><BR>
<A HREF="castles_armory.php?otdel=12&sid=&0.286790872806733">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;дубины,булавы</A><BR>
<A HREF="castles_armory.php?otdel=13&sid=&0.0943516060419363">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;мечи</A><BR>
<A HREF="castles_armory.php?otdel=2&sid=&0.76205958316951">Одежда: сапоги</A><BR>
<A HREF="castles_armory.php?otdel=21&sid=&0.648260824682342">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;перчатки</A><BR>
<A HREF="castles_armory.php?otdel=22&sid=&0.520447517792988">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;легкая броня</A><BR>
<A HREF="castles_armory.php?otdel=23&sid=&0.99133839275569">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;тяжелая броня</A><BR>
<A HREF="castles_armory.php?otdel=24&sid=&0.567932791291376">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;шлемы</A><BR>
<A HREF="castles_armory.php?otdel=3&sid=&0.725667864710179">Щиты</A><BR>
<A HREF="castles_armory.php?otdel=4&sid=&0.321709306035984">Ювелирные товары: серьги</A><BR>
<A HREF="castles_armory.php?otdel=41&sid=&0.902093651333512">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ожерелья</A><BR>
<A HREF="castles_armory.php?otdel=42&sid=&0.510210803380268">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;кольца</A><BR>
</div>
    </td>
  </tr>
</table>  
