...
if ($user['zayavka']>0) { header("Location: zayavka.php?level=grclass&zerror=1&tmp=".mt_rand(111111,999999) ); die(); }
		
$all_kom=mysql_query("select * from oldbk.class_profile where owner='{$user[id]}' and prof>0 ORDER BY def DESC ") or die(); 
...
if (isset($_POST['classfall'])) {
	$_SESSION['classfall']=(int)$_POST['classfall'];
} else {
	$_POST['classfall']=(int)$_SESSION['classfall'];	
}

//print_r($_POST);

if (isset($_POST['newuclass'])) 
	{
		$new_class=(int)($_POST['newuclass']);
		if (($new_class>=1) AND ($new_class<=3))
			{
			mysql_query("UPDATE `oldbk`.`users` SET `uclass`='{$new_class}' WHERE `id`='{$user['id']}' limit 1;");
			}
	}
...
//echo "RUN TEST";
		...
		nlight > {$user[mlight]} OR ngray > {$user[mgray]} OR ndark > {$user[mdark]}  OR (nclass!='{$user['uclass']}' and nclass>0) ); ";
...
mysql_query("UPDATE `users` SET 
				...
				`users`.`mwater`=0,`users`.`mair`=0,`users`.`mearth`=0,`users`.`mlight`=0,`users`.`mgray`=0 ,`users`.`mdark`=0 ,`users`.`uclass`=0
...
function dressitem_prim ($id,$ot) 
{
	...
	// и может быть одета
	$it=mysql_query("SELECT * FROM oldbk.shop WHERE  `new_item` > 0 AND `razdel` = '{$ot}' AND type!=12 AND
	...
	//echo "Одеваем....";
					...
					( `bs_owner` , `present`, `dressed` , `prototype`,`owner`,`name`,`type`,`massa`,`cost`,`img`,`maxdur`,`isrep`,
						`gsila`,`glovk`,`ginta`,`gintel`,`ghp`,`gnoj`,`gtopor`,`gdubina`,`gmech`,`gfire`,`gwater`,`gair`,`gearth`,`glight`,`ggray`,`gdark`,`needident`,`nsila`,`nlovk`,`ninta`,`nintel`,`nmudra`,`nvinos`,`nnoj`,`ntopor`,`ndubina`,`nmech`,`nfire`,`nwater`,`nair`,`nearth`,`nlight`,`ngray`,`ndark`,`nclass`,
		...
		if ($good==1)
		{
			...
			if (!($item['type']==12 && $user['level'] < 4))
				...
				$sql="UPDATE `users` as u SET u.{$slot1} = {$id},
					...
					u.maxhp = u.maxhp + '{$item['ghp']}', u.hp = u.maxhp + '{$item['ghp']}',
...
//////////////////////////////////////////////////////////////////////////////////////
		 ...
		 //
		 $d=(int)($_GET[setdef]);
		  mysql_query("UPDATE oldbk.`class_profile` set def=0 WHERE `owner`='{$user[id]}' AND `def`=1;") or die("11");
  		  mysql_query("UPDATE oldbk.`class_profile` set def=1 WHERE `owner`='{$user[id]}' AND `prof`='{$d}' ;") or die("12");
		 ...
		 $dell=(int)($_GET[delcomplect]);
		  ...
		  if ($dell>0)
		  	{
			 mysql_query("DELETE FROM oldbk.`class_profile` WHERE `owner`='{$user[id]}' AND `prof`='{$dell}';") or die("13");
			unset($arr_kom[$dell]);
		...
		{
		$gsql="select * from oldbk.shop where id={$kv} and  `new_item` > 0 AND type!=12 ".$seven_level." ; ";
...
/////////////////////////////////////////////////////////////////////////////////////////////									
		...
		$asql="UPDATE `users` SET 
				`users`.`sila`={$arrneed[sila]},`users`.`lovk`={$arrneed[lovk]},`users`.`inta`={$arrneed[inta]},`users`.`vinos`={$arrneed[vinos]},
				...
				`users`.`mair`={$arrneed[mair]},`users`.`mearth`={$arrneed[mearth]},`users`.`mlight`={$arrneed[mlight]},`users`.`mgray`={$arrneed[mgray]} ,`users`.`mdark`={$arrneed[mdark]} ,`users`.`uclass`={$arrneed[uclass]}
		...
		die("3131");
			...
			if (ref_drop_prem()==true) //проверяем на удержание предметы
				...
				$user = mysql_fetch_array($us);
                  ...
                  if ($kom_kol>=30)
                    {
                    echo "<font color=red>У Вас уже есть 30 профилей...</font>";
                    }
                    else
                    {
                    //можем сохранить?
                     if (chck_items_prem_classes()==true)
                       {
		...
		`mlight`='{$user[mlight]}',`mgray`='{$user[mgray]}',`mdark`='{$user[mdark]}', `bpbonushp`='0' , `uclass`='{$user['uclass']}'   ";
		$asql="INSERT INTO oldbk.`class_profile` SET `owner`='{$user[id]}',`pname`='{$_POST[savecomplect]}' ,`prof`='{$next_id}', ".$sk_row." ON DUPLICATE KEY UPDATE  ".$sk_row."  ; ";
		//echo $asql;
...
//21-22-23-24-25-26-27-28-210-211
		...
		$us=mysql_query("SELECT * FROM `users` WHERE `id` = '{$_SESSION['uid']}' LIMIT 1;") or die("9");
		  ...
		  if (dressitem_prim ($prim,$ot)==true)
		   ...
		   if (($_GET['exit']) and  ($user[battle]==0) and  ($user[zayavka]==0)  )
		   {
		   $goto=20;
		   if ($goto>0)
		     {
		     ///загружаем параметры prof=0 для выхода
		     $user_real=mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`class_profile` WHERE  prof=0 and  `owner` = '{$user[id]}' LIMIT 1;"));
		...
		`mlight`='{$user_real[mlight]}',`mgray`='{$user_real[mgray]}',`mdark`='{$user_real[mdark]}', `bpbonushp`='{$user_real[bpbonushp]}' , `uclass`='{$user_real['uclass']}'   ";
		      mysql_query("UPDATE `users` SET ".$sk_row." , `users`.`room` = '{$goto}'  WHERE `users`.`id`  = '{$user[id]}' ;");
	 ...
	 if ($nadeto>0)
		...
		//echo "$nadeto";		
		 if ($nadeto!=$heved[0])
		 	...
		 	dropall_prem();
			//mysql_query('COMMIT') or die();		 	
		 	addchp('<font color=red>Внимание castle armory fix for:'.$user[login].' </font>'.$nsql,'{[]}Bred{[]}');
			header("Location: class_armory.php");
			die("");
		 	}
		 	else
		 	{
...
<SCRIPT LANGUAGE="JavaScript">
		...
		function returned2(s){
			location.href='class_armory.php?'+s+'tmp='+Math.random();
		}
...
<TD valign=top >
		...
		if($user['klan']) {
			...
			?>
			<form method="POST" action="?otdel=<?=intval($_GET['otdel']);?>" id="chclass" style="margin:0px;padding:0px;display:block;">
                                <div class="head-title" style="top:1px;">Класс персонажа:
                                    <select name="newuclass" style="width:100px;margin:0px;top:0px;" OnChange="document.getElementById('chclass').submit();">
					<option value = "0" <? if ($user['uclass']==0) { echo ' selected ' ; } ?>>не установлен</option>
					<option value = "1" <? if ($user['uclass']==1) { echo ' selected ' ; } ?>>Уворотчик</option>
					<option value = "2" <? if ($user['uclass']==2) { echo ' selected ' ;  } ?>>Критовик</option>
					<option value = "3" <? if ($user['uclass']==3) { echo ' selected ' ;  } ?>>Танк</option>					
                                    </select>
                                </div>
			</form>
			<?
...
?>
		...
		Броня<br/>
		 ...
		 <?
		 echo " &nbsp; эффективность:&nbsp;".round(get_rabota_boni_lvls($user)*100,2)."-".round((get_rabota_boni_lvls($user)+0.2)*100,2)."%<br>";						
		 ?>
		<!-- </> -->
		<HR>
		Мастерство владения:<BR>
		 &nbsp; ножами и кастетами: <?=$user['noj']?><?=($user['master']&&($user['noj']<5) )?"<a href='?up=21&s4i=".($user['sid'])."&edit=1'><img src=i/up.gif></a>":""?><BR>
		 &nbsp; мечами: <?=$user['mec']?><?=($user['master']&&($user['mec']<5) )?"<a href='?up=22&s4i=".($user['sid'])."&edit=1'><img src=i/up.gif></a>":""?><BR>
		 &nbsp; дубинами, булавами: <?=$user['dubina']?><?=($user['master']  &&($user['dubina']<5))?"<a href='?up=23&s4i=".($user['sid'])."&edit=1'><img src=i/up.gif></a>":""?><BR>
		 &nbsp; топорами и секирами: <?=$user['topor'] ?><?=($user['master'] &&($user['topor']<5))?"<a href='?up=24&s4i=".($user['sid'])."&edit=1'><img src=i/up.gif></a>":""?><BR>
		<?if ($user['level'] > 3) {?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
    	<tr>
	    <td align=left> </td>
    	    <td align=left>    <h3>Оружейная Комната</h3>
    	  <?/* <font color=red><b>Внимание!</b> В комнате тестирования боёв классов нельзя пользоваться инвентарём, все надетые на вашего персонажа предметы и руны будут сняты автоматически.<br>*/?>
	    <font color=red><b>Внимание!</b> Рассчитывайте профиль так, чтобы удержать Ваши свитки!<br>
	    <?
		//if ($defaltis==0) { echo "<font color=red><b>Внимание!</b> У Вас не установлен профиль по умолчанию - Вы не сможете войти в бои классов!<br>";}
		chck_items_prem_classes ();
	echo "</td><td align=right>";
    		echo "<form method=GET > <input type=button value='Обновить' onClick=\"returned2('refresh=3.14&');\">
		<INPUT TYPE=button value=\"Выйти на Центральную площадь\" onClick=\"returned2('exit=true&');\"><br>
		<INPUT TYPE=button value=\"Бои классов\" onClick=\"location.href='http://capitalcity.oldbk.com/zayavka.php?level=grclass&'+'tmp='+Math.random();\"><br>
		</form>";
	echo "</td></tr></table>";
    ?>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?
    	$filt_sql='';
    			if ((int)($_POST['fall'])==1) 
    			{
		    	$filt_sql=" and nlevel='{$user['level']}' ";
    			}
    			elseif ((int)($_POST['fall'])>1) 
    			{
		    	$filt_sql=" and nlevel<='{$user['level']}' ";
    			}
    			
    			if ((int)($_POST['classfall'])!=0) 
    			{
		    	$filt_sql.=" and (nclass='{$user['uclass']}' OR nclass=0) ";
    			}    			
    		
	$data = mysql_query("SELECT * FROM oldbk.`shop` WHERE `new_item` > 0 AND `razdel` = '{$_GET['otdel']}' ".$filt_sql." ORDER by `cost` ASC");
...
<div style="background-color:#d2d0d0;padding:1"><center><font color="#oooo"><B>Ассортимент</B></center></div>
			...
			<form method="POST" action="?otdel=<?=intval($_GET['otdel']);?>" id="fall" style="margin:0px;padding:0px;display:block;">
                                <div class="head-title" style="top:1px;">
                                    <select name="fall" style="width:250px;margin:0px;top:0px;" OnChange="document.getElementById('fall').submit();">
					<option value = "0" <? if ((int)($_POST['fall'])==0) { echo ' selected ' ; } ?>>Показывать все вещи</option>
					<option value = "1" <? if ($_POST['fall']==1) { echo ' selected ' ; $viewlevel=true; } ?>>Показывать вещи только моего уровня</option>
					<option value = "2" <? if ($_POST['fall']>1) { echo ' selected ' ; $viewlevel=true; } ?>>Показывать вещи моего и ниже уровня</option>					
                                    </select>
                                    <select name="classfall" style="width:250px;margin:0px;top:0px;" OnChange="document.getElementById('fall').submit();">
					<option value = "0" <? if ((int)($_POST['classfall'])==0) { echo ' selected ' ; } ?>>Показывать всех классов</option>
					<option value = "1" <? if ($_POST['classfall']>0) { echo ' selected ' ; $viewlevel=true; } ?>>Показывать вещи только моего класса</option>
                                    </select>
                                </div>
				</form>
<A HREF="class_armory.php?otdel=11&tmp=<?=mt_rand(1111,9999);?>">Оружие: топоры</A><BR>
<A HREF="class_armory.php?otdel=12&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;дубины,булавы</A><BR>
<A HREF="class_armory.php?otdel=13&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;мечи</A><BR>
<A HREF="class_armory.php?otdel=2&tmp=<?=mt_rand(1111,9999);?>">Одежда: сапоги</A><BR>
<A HREF="class_armory.php?otdel=21&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;перчатки</A><BR>
<A HREF="class_armory.php?otdel=22&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;легкая броня</A><BR>
<A HREF="class_armory.php?otdel=23&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;тяжелая броня</A><BR>
<A HREF="class_armory.php?otdel=6&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;плащи</A><BR>
<A HREF="class_armory.php?otdel=24&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;шлемы</A><BR>
<A HREF="class_armory.php?otdel=3&tmp=<?=mt_rand(1111,9999);?>">Щиты</A><BR>
<A HREF="class_armory.php?otdel=4&tmp=<?=mt_rand(1111,9999);?>">Ювелирные товары: серьги</A><BR>
<A HREF="class_armory.php?otdel=41&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ожерелья</A><BR>
<A HREF="class_armory.php?otdel=42&tmp=<?=mt_rand(1111,9999);?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;кольца</A><BR>
</div>
    </td>
  </tr>
</table>  
