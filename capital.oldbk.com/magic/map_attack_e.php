				...
				if (($user[hidden] >0) and ($user[hiddenlog]==''))
				 {
					...
					if ($rowm['img']=='') { $rowm['img']='attac_blood_berezka.gif'; $mag_gif='<img src=http://i.oldbk.com/i/sh/'.$rowm['img'].'>';						 }
...
//			addlog($id,'Часы показывали <span class=date>'.date("Y.m.d H.i").'</span>, когда '.$p2.' бросили вызов друг другу. <BR>');
			...
			if ($user['hidden'] > 0) {
				addch('<img src=i/magic/attac_blood_berezka.gif> <b><i>Невидимка</i></b>, применив магию кровавого нападения, внезапно напал на <b>'.$jert['login'].'</b>.',$jert['room']);
			} else {
				addch('<img src=i/magic/attac_blood_berezka.gif> <b>'.$user['login'].'</b>, применив магию кровавого нападения, внезапно напал на <b>'.$jert['login'].'</b>.',$jert['room']);
			}
