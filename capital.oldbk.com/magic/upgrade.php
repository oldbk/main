...
if(($_SERVER['PHP_SELF'] != '/_repair.php') && ($_SERVER['PHP_SELF'] != '/repair.php') && ($_SERVER['PHP_SELF'] !='/lab2.php') && ($_SERVER['PHP_SELF'] !='/new_rings.php') && ($_SERVER['PHP_SELF'] !='/_new_repair.php'))
{
	if ($user['battle'] > 0) {
		echo "Не в бою...";
	}
	...
	{
	$anlim_items=array(6,8,10,12,13,14,15,23,25,33,34,38,39,40,41,42,43,44,45,52,57,58,60,61,68,69,76,80,87,90,94,95,110,130,144,145,147,257);
	
		$dress = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `owner` = '{$user['id']}' and (sowner=0 or sowner = '{$user['id']}')  AND `id` = '{$_POST['target']}' AND (`type` < 12 or `type` = 28) AND `dressed`=0 AND bs_owner = '".$user[in_tower]."' AND `setsale`=0 AND `prokat_idp`=0 AND (arsenal_klan = '' OR arsenal_owner=1 )
		...
		{
			echo "<font color=red><b>Нельзя улучшить вещь до меньшего уровня ".$upgrade['level']."!<b></font>";
		}
		elseif($dress['up_level'] > 0 AND ($dress['up_level'] == $upgrade['level']))
		{
			echo "<font color=red><b>Данная вещь уже имеет ".$upgrade['level']." уровень улучшения!<b></font>";
		}
		elseif($dress['up_level'] > 0 AND ($dress['up_level'] < $upgrade['level']) AND !($dress['up_level'] == $upgrade['level'] - 1))
		{
			echo "<font color=red><b>Нельзя улучшить вещь до уровня ".$upgrade['level'].", минуя предыдущий!<b></font>";
		}
		elseif ( ($upgrade['level'] >=8) AND (in_array($dress['prototype'],$anlim_items) ) )
		{
		
			echo "<font color=red><b>В связи со скорым вводом новых вещей, невозможно поднять уровень новичковой вещи выше 7го уровня.<b></font>";
		}
		else
		{
		$stop=false;
		...
		// Внимание - дальше в строчке баг - нет поля -cost!!!- Мы знаем - и не трогаем потому что пиздец :) с) Fred
			...
			$nlevel = $dress['nlevel'];
			if ($upgrade['level'] > $dress['nlevel']) $nlevel = $upgrade['level'];
		...
		WHERE id = '{$dress['id']}' AND `owner` = '{$user['id']}';";
			...
			if(mysql_query($query))
			{
				$bet=1;
				$sbet = 1;
				echo "<font color=red><b>Вещь \"{$dress['name']}\" удачно улучшена до уровня {$upgrade['level']}<b></font>";
