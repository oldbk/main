...
echo "В турнирных боях нельзя призвать монстров!"; 
	...
	if (($get_test_baff[id] > 0) )
	{
	...
	else
	 {
		...
		$get_count_baff=mysql_query("select  * from effects where battle='{$user[battle]}' and type=828  and owner in (select id from users where battle='{$user[battle]}' and battle_t='{$user[battle_t]}' and hp >0) LIMIT 2;");
		$kow=0;
		...
		while ($baff_owners = mysql_fetch_array($get_count_baff))
			...
			{
			//никого я начинаю
				mysql_query("INSERT INTO `effects` SET `type`='828',`name`='Начало Светлый Обряд', `time`='".(time()+60)."', `owner`='{$user[id]}', `add_info`='{$user[level]}' , `battle`='{$user[battle]}';");//1 минута
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>. <i>(Начал цепь...)</i> <BR>');
			...
			{
			//есть 1 я продолжаю
				mysql_query("INSERT INTO `effects` SET `type`='828',`name`='Продолжение Светлый Обряд',`time`='".$remem_time[1]."',`owner`='{$user[id]}',  `add_info`='{$user[level]}' ,  `battle`='{$user[battle]}';");//время дублируем с эфекта начала
				if (mysql_affected_rows()>0)
				{
					if ($user[hidden]>0 and $user[hiddenlog]=='') 	{ $user[sex]=1;	}
					elseif ($user[hidden]>0 and $user[hiddenlog]!='') {  $fuser=load_perevopl($user); $user[sex]=$fuser[sex]; }
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>. <i>(Продолжил цепь...)</i> <BR>');
			...
			foreach($remem_own as $ic=>$owner)
				...
				///Делаем бота
				$clon_count=mysql_fetch_array(mysql_query("select * from battle_data WHERE battle='".$user['battle']."' ; "));

				$bot_data=mysql_fetch_array(mysql_query("select * from users where id='{$bots_ar[$bot_lvl]}' ;"));
				$bot_data[login]=$bot_data[login]." (".($clon_count[baff_863]+1).")";
				...
				mysql_query("INSERT `battle_data` (`battle`,`baff_863`) values('".$user['battle']."', '1' ) ON DUPLICATE KEY UPDATE `baff_863` =`baff_863`+1;");
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>, и призвал'.$action.' '.$all_bots_namea.'  <i>(Закрыл цепь...)</i> <BR>');									
				$all_bots_namea=str_replace(':','^',$all_bots_namea);
