...
$baff_name='Защита Мусорщика';
$baff_type=1730;//812-задержка
...
{	
	...
	$get_imun_baff= mysql_fetch_array(mysql_query("select * from effects where owner='{$user[id]}'  and type='812' ; "));
		...
		else
		{
			...
			mysql_query("INSERT INTO `effects` SET `type`='812',`name`='Задержка на использование Защиты Мусорщика',`time`='".(time()+6*60*60)."',`owner`='{$user[id]}' ;");//6часов задержка
		
			 $mag_gif='<img src=i/magic/3n2.jpg>';
