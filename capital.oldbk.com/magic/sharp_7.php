...
if ($user['battle'] > 0) {
	echo "Не в бою...";
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE  magic=190 AND `setsale`=0 AND `bs_owner`='{$user[in_tower]}' AND (arsenal_klan = '' OR arsenal_owner=1 )  AND `owner` = '{$user['id']}' and id='{$rowm['id']}' LIMIT 1;"));
	//проверка на уже заточенную
	$predmet=explode("+",$dress[name]);
	$start=(int)($predmet[1]);
	if ($start > 7)
	{
	echo "Уже заточено на +7 или выше ...";
	}
	elseif( (($svitok['prototype']==190199) and ($svitok['sowner']>0) ) and ($dress['arsenal_klan'] != '') )
	{
	...
	$new_name=$predmet[0]."+7";
	
	
	$add_sql_update='';
	
	if (($svitok['prototype']==190199) and ($svitok['sowner']>0) )
			{
		...
		if ($dress && $svitok) {
			...
			if ($asql!='')
				...
				if (mysql_query("UPDATE oldbk.`inventory` SET  {$add_sql_update}  `sharped` = 1 {$present}  , `name` = '{$new_name}', `minu` = `minu`+{$sharp}, `maxu`=`maxu`+{$sharp}, `cost` = `cost`+30 , ".$asql."   WHERE `id` = {$dress['id']} LIMIT 1;")) 
				{
					...
					echo "<font color=red><b>Предмет \"{$dress['name']}\" удачно ".$dodo."заточен на +7! Магия Мусорщика придала этому предмету статус подарка.<b></font> ";
					$bet=1;
					$sbet = 1;
