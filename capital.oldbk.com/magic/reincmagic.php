...
if ($user['battle'] > 0) {
	...
	if(!$_SESSION['scroll']) {
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `id` = '".$_SESSION['scroll']."' AND `owner` = '{$user['id']}' and (cost>0 or ecost>0)  AND `dressed`=0  and labonly=0 and labflag=0 LIMIT 1;"));
...
//					`nlevel` = '{$shop_base['nlevel']}',				
				...
				{
				// перевстраиваем
					$dress['nintel']=$shop_base['nintel'];
					$dress['nlevel']=$shop_base['nlevel'];
					$dress['nmudra']=$shop_base['nmudra'];
					$dress['ngray']=$shop_base['ngray'];
					$dress['ndark']=$shop_base['ndark'];
					$dress['nlight']=$shop_base['nlight'];
...
//							".($dress['nlevel']<=$svitok['nlevel']?"`nlevel`='".$svitok['nlevel']."',":"")."
					$sql="UPDATE oldbk.`inventory`
					 		SET ".($dress['nintel']<=$svitok['nintel']?"`nintel`='".$svitok['nintel']."',":"")."
