...
if ($user['battle'] > 0) {
	...
	if(!$_SESSION['scroll']) {
		...
		if (($svitok['getfrom'] == 43 && $svitok['repcost'] > 0 && $svitok['sowner'] > 0) || ($rowm['getfrom'] == 43 && $rowm['repcost'] > 0 && $rowm['sowner'] > 0)) {
			$sowner = mysql_fetch_assoc(mysql_query('SELECT * FROM users WHERE id = '.$svitok['sowner']));
		?>
			<body onload="showitemschoice('Выберите предмет, в который встраивается свиток.<br><font color=red>Внимание! После встраивания этого свитка предмет станет привязан к персонажу <?=$sowner['login'];?></font>', 'itemssowner', 'main.php?edit=1&use=<?=$_GET['use']?>');">
		<?
		} else {
		?>
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `id` = '".$_SESSION['scroll']."' and (cost>0 or ecost>0 or repcost > 0)  AND `owner` = '{$user['id']}' AND `dressed`=0 AND  (arsenal_klan = '' OR arsenal_owner=1 ) AND `setsale`=0 and labonly=0 and labflag=0 LIMIT 1;"));
		$dress = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE naem = 0 and `owner` = '{$user['id']}' AND present!='Арендная лавка'  AND `id` = '{$_POST['target']}' AND `includemagic` = 0 AND `dressed`=0 AND `setsale`=0 AND (arsenal_klan = '' OR arsenal_owner=1 ) AND `prokat_idp`=0 and labonly=0 and labflag=0 and gmeshok = 0 AND NOT ((`prototype` >= 410001  and `prototype` <= 410030) or (`prototype` >= 55510301  and `prototype` <= 55510401)) LIMIT 1;"));
		...
		if(!$svitok) {
			...
			echo "<font color=red><b>Этот свиток нельзя встраивать в предметы!<b></font>";
				...
				// встраиваем
				$perezar = 150;
				if ($rowm['prototype'] == 9597) $perezar = 250;
				if ($rowm['prototype'] == 9598) $perezar = 500;
				if ($rowm['prototype'] == 9599) $perezar = 750;
				...
				if (($svitok['getfrom'] == 43 && $svitok['repcost'] > 0 && $svitok['sowner'] > 0) and ($dress['arsenal_klan'] != '')) {
				...
				} elseif ($rowm['getfrom'] == 43 && $rowm['repcost'] > 0 && $rowm['sowner'] > 0 && $dress['sowner'] > 0 && $dress['sowner'] != $user['id']) {
					echo "<font color=red><b>Нельзя встраивать в вещь, привязанную не к вам.</b></font>";
				} elseif ($rowm['getfrom'] == 43 && $rowm['repcost'] > 0 && $rowm['sowner'] > 0 && $svitok['sowner'] > 0 && $svitok['sowner'] != $user['id']) {
					echo "<font color=red><b>Нельзя встраивать свиток, привязанный не к вам.</b></font>";
				} else {
					$addsql = "";
					if ($svitok['getfrom'] == 43 && $svitok['repcost'] > 0 && $svitok['sowner'] > 0) {
						$addsql = ', sowner = '.$svitok['sowner'];
					}
					if ($rowm['getfrom'] == 43 && $rowm['repcost'] > 0 && $rowm['sowner'] > 0) {
						$addsql = ', sowner = '.$rowm['sowner'];
					}
					...
					$sql = "UPDATE oldbk.`inventory` SET
						...
						".($dress['nlight']<$svitok['nlight']?"`nlight`='".$svitok['nlight']."',":"")." `massa`=`massa`+1,`cost`=`cost`+'".$svitok['cost']."',
						`includemagic` = '".$svitok['magic']."', `includemagicdex` = '".$svitok['maxdur']."', `includemagicmax` = '".$svitok['maxdur']."',
						`includemagicname` = '".$svitok['name']."', `includemagicuses` = '".$perezar."', 
