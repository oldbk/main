...
if ($user['battle'] > 0) {
	...
	if (rand(1,100) < $int) {
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `name` = 'Заточка на дубины +3' AND `setsale`=0 AND `bs_owner`='{$user[in_tower]}' AND (arsenal_klan = '' OR arsenal_owner=1 ) AND `owner` = '{$user['id']}' LIMIT 1;"));


		if (($dress[otdel]==12) && $svitok) 
		{
			if (mysql_query("UPDATE oldbk.`inventory` SET `sharped` = 1, `name` = CONCAT(`name`,'+3'), `minu` = `minu`+3, `maxu`=`maxu`+3, `cost` = `cost`+18, `ndubina` = `ndubina`+3, `nlovk` = `nlovk`+3 WHERE `id` = {$dress['id']} LIMIT 1;")) {
