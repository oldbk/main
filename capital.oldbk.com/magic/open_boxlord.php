...
function mk_my_item_gift($telo,$proto,$larinfo,$present=false,$ekr_flag=0,$kol=1,$goden=0) {
...
$dress=mysql_fetch_array(mysql_query("select * from oldbk.shop where id='{$proto}' ;"));
	...
	if(mysql_query("INSERT INTO oldbk.`inventory`
		...
		VALUES
			('".time()."','{$dress['ekr_flag']}','130','{$dress['id']}','{$telo[id]}','{$dress['name']}','{$dress['type']}',{$dress['massa']},{$dress['cost']}, {$dress['ecost']}, '{$dress['img']}',{$dress['maxdur']},{$dress['isrep']},'{$dress['letter']}','{$dress['gsila']}','{$dress['glovk']}','{$dress['ginta']}','{$dress['gintel']}','{$dress['ghp']}','{$dress['gnoj']}','{$dress['gtopor']}','{$dress['gdubina']}','{$dress['gmech']}','{$dress['gfire']}','{$dress['gwater']}','{$dress['gair']}','{$dress['gearth']}','{$dress['glight']}','{$dress['ggray']}','{$dress['gdark']}','{$dress['needident']}','{$dress['nsila']}','{$dress['nlovk']}','{$dress['ninta']}','{$dress['nintel']}','{$dress['nmudra']}','{$dress['nvinos']}','{$dress['nnoj']}','{$dress['ntopor']}','{$dress['ndubina']}','{$dress['nmech']}','{$dress['nfire']}','{$dress['nwater']}','{$dress['nair']}','{$dress['nearth']}','{$dress['nlight']}','{$dress['ngray']}','{$dress['ndark']}',
...
$array_box[4016]=50;//В сундук складываем 50 пропусков к Лорду Разрушителю (ID4016)
$array_box[3004053]=5; // Каменный снаряд 5шт PROTO_ID:3004053
...
if (is_array($array_box))
	...
	$out_text='';
				...
				foreach ($array_box as $proto=>$kol)
				{
								...
								$out_text.=mk_my_item_gift($user,$proto,$lar_inf,$present,0,$kol,$rowm['goden']);
