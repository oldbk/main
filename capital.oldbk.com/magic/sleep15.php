...
elseif (rand(1,100) < $int) {
			$nick = nick7($user['id']);
			addch("<img src=i/magic/sleep.gif>Персонаж &quot;{$nick}&quot; наложил заклятие молчания на &quot;{$_POST['target']}&quot;, сроком 15 мин.",$user['room'],$user['id_city']);

			$juser = mysql_fetch_array(mysql_query("SELECT `id` FROM `users` WHERE `login` = '{$_POST['target']}' LIMIT 1;"));
			mysql_query("INSERT INTO `effects` (`owner`,`name`,`time`,`type`) values ('".$juser['id']."','Заклятие молчания',".(time()+900).",2);");
