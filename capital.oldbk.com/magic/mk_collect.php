...
if (($item['id']>0) and  ($item['prototype']==111010)) //Футляр коллекции №1';
	{
		//отработка 1й коллекции
		$get_all_cards=mysql_query("select * from oldbk.inventory where owner = '{$_SESSION['uid']}'  and prototype>=111001 and prototype<111010 group by prototype");
		$ccids=array();
		$ckol=0;
		...
		if ($ckol==9)
			{
				...
				{
				//собралась
				        $dress = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.shop WHERE `id` = '111000' LIMIT 1;"));
					if ($dress['id']>0)
							{
							...
							'{$dress['mfkrit']}','{$dress['mfakrit']}','{$dress['mfuvorot']}','{$dress['mfauvorot']}','{$dress['bron1']}','{$dress['bron2']}','{$dress['bron3']}','{$dress['bron4']}','{$dress['maxu']}','{$dress['minu']}','{$dress['magic']}','{$dress['nlevel']}','{$dress['nalign']}','".$dress['dategoden']."','{$dress['goden']}','{$dress['nsex']}','{$dress['razdel']}','Удача','0','0','{$dress['group']}','{$user['id_city']}','{$dress['letter']}'
							) ;") )
						     	{
						     		$dress['id']=mysql_insert_id();
												...
												$rec['add_info']="Коллекция \"Тайны Лабиринта\"! Из Карт:(".implode(",",$ccids).") Футляр:".$che;
												add_to_new_delo($rec); //юзеру
								 				$rec=array();
				 								//пишем в эффекты
				 						//Собранная коллекция дает право обменивать статуи у торговца с наценкой 20%
				 						mysql_query("INSERT INTO `effects` SET `type`='111010',`name`='{$dress['name']}', add_info='20'  ,`time`='".$dress['dategoden']."', `owner`='{$user[id]}' ");
										if (mysql_affected_rows()>0)
											{
											echo "<font color=red><b>Поздравляем! Удачно собрана коллекция \"Тайны Лабиринта\"!<b></font> ";
											$bet=1;
											$sbet = 1;
											}
							}
