...
if ($user['battle'] > 0) {
	...
	$dress = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE (`mfkrit`> 0 OR `mfakrit` > 0 OR `mfuvorot` > 0 OR `mfauvorot` > 0) AND setsale = 0 AND `dressed`=0 AND `prokat_idp`=0 AND ( `sowner`=0 OR `sowner`='{$_SESSION['uid']}') AND `bs_owner`=0 AND arsenal_klan = '' AND `labonly`=0 AND `type`=9 AND owner = '{$_SESSION['uid']}' AND `name` LIKE '%(мф)%' and id='{$_POST['target']}';"));
	$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `name` = 'Подогнать Перчатки' AND `owner` = '{$user['id']}' LIMIT 1;"));
	if ($dress[id]>0)
	{
	if (($svitok[labflag]>0) AND ($svitok[id]>0))
	   ...
	   if ($svitok[id]>0)
	   {
		...
		{
			echo "<font color=red>Перчатки уже подогнаны до максимального уровня!<b></font>";
		}
		else
		{
		...
		if (mysql_query("UPDATE oldbk.`inventory` SET `ups` =`ups`+1, `mfbonus`=`mfbonus`+'{$ADDMF[$dress[ups]+1]}' , `sowner`='{$user['id']}', `cost` = `cost` + '".$cost_add."'    WHERE `id` = {$dress['id']} LIMIT 1;"))
			{
				echo "<font color=red><b>Перчатки \"{$dress[name]}\" удачно подогнаны.<b></font> ";
				$bet=1;
				$sbet = 1;
				        //new_delo
				  		    		$rec['owner']=$user[id];
