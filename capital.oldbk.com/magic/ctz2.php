...
$hidden2 = mysql_fetch_array(mysql_query("SELECT * FROM `effects` WHERE type=200 and `owner` = '{$us[id]}' LIMIT 1;"));
		if($hidden[0])
		 {
				if($hidden2[0]) {
				addch("<img src=i/magic/zagorod_cure2.gif> ".$angel." &quot;<i>Невидимка</i>&quot; ".$action." от {$travm} травм &quot;<i>Невидимка</i>&quot;",$user['room'],$user['id_city']);
				} else {
				addch("<img src=i/magic/zagorod_cure2.gif> ".$angel." &quot;<i>Невидимка</i>&quot; ".$action." от {$travm} травм &quot;{$_POST['target']}&quot;",$user['room'],$user['id_city']);
				}
}
else
{
				if($hidden2[0]) {
				addch("<img src=i/magic/zagorod_cure2.gif> ".$angel." &quot;{$user['login']}&quot; ".$action." от {$travm} травм &quot;<i>Невидимка</i>&quot;",$user['room'],$user['id_city']);
				} else {
				addch("<img src=i/magic/zagorod_cure2.gif> ".$angel." &quot;{$user['login']}&quot; ".$action." от {$travm} травм &quot;{$_POST['target']}&quot;",$user['room'],$user['id_city']);
				}
}
