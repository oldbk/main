...
echo "<font color=red>Город не найден!</font>";
/*
...
if ( (strtolower($_POST['city'])=='capital') OR ( strtolower($_POST['city'])=='capitalcity') OR ( strtolower($_POST['city'])=='cap') ) 
	...
	$rowm=mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE id = ".$rowm[id]." limit 1 "));
		...
		mysql_query("UPDATE users set odate=0 , id_city=0 where id={$telo[id]} and id_city=1 and battle = 0;");
		...
		mysql_query("INSERT INTO oldbk.effects SELECT NULL,`type`,`name`,`time`,`sila`,`lovk`,`inta`,`vinos`, `intel` , `owner`, `lastup`, `idiluz`,`pal`, `add_info`, `battle` ,`eff_bonus`  from avalon.effects where owner={$telo[id]}");
		//удаляем копии
       ...
       if ($telo[id_city]==0)
		...
		mysql_query("UPDATE users set odate=0 , id_city=1 where id={$telo[id]} and id_city=0 and battle = 0;");
		if (mysql_affected_rows()>0)
		{
		...
		//bug fix
		mysql_query("DELETE from avalon.effects where owner={$telo[id]}");
		mysql_query("INSERT INTO avalon.effects SELECT NULL,`type`,`name`,`time`,`sila`,`lovk`,`inta`, `intel` , `vinos`, `owner`, `lastup`, `idiluz`,`pal`, `add_info`, `battle` ,`eff_bonus`  from oldbk.effects where owner={$telo[id]}");
		//удаляем копии
		mysql_query("DELETE from oldbk.effects where owner={$telo[id]}");
