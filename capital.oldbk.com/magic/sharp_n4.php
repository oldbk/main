...
if ($user['battle'] > 0) {
	...
	if (rand(1,100) < $int) {
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `name` = 'Заточка на ножи +4' AND `setsale`=0 AND `bs_owner`='{$user[in_tower]}' AND (arsenal_klan = '' OR arsenal_owner=1 )  AND  `owner` = '{$user['id']}' LIMIT 1;"));
