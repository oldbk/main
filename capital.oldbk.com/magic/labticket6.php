...
{
 mysql_query("UPDATE oldbk.`labirint_var` SET val=val-21600  WHERE  `var`='labstarttime' and `owner`='".$user[id]."';");
 
// if (mysql_affected_rows()>0)
 	{
		 mysql_query("Insert into ristalka set `owner`='{$user[id]}', labp=1 on DUPLICATE key update labp=1"); //ставим флаг
		 echo "Вы использовали ".link_for_magic($rowm['img'],$rowm['name'])."...<i> время посещения лабиринта уменьшено на 6 часов.</i>";
