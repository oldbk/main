...
$baff_name='Защита Мироздателя';
$baff_type=1731;//815-задержка
...
elseif ($user['trv'] != 0) 
{	
	echo "Вы уже защищены от травм..."; 
}
else {
	$get_imun_baff= mysql_fetch_array(mysql_query("select * from effects where owner='{$user[id]}'  and type='815' ; "));
		...
		else
		{
			...
			mysql_query("INSERT INTO `effects` SET `type`='815',`name`='Задержка на использование Защиты Мироздателя',`time`='".(time()+6*60*60)."',`owner`='{$user[id]}' ;");//6часов задержка
		
			 $mag_gif='<img src=i/magic/6n2.jpg>';
			...
			addch($mag_gif." <B>{$fuser['login']}</B> использовал магию &quot;".$baff_name."&quot;",$user['room'],$user['id_city']);
				$bet=1;
				$sbet = 1;
				echo "Все прошло удачно! Вы защитились от травм на следующий бой!";
				$MAGIC_OK=1;
				}
