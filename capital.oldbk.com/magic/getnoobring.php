...
$r_array = array(222222230,222222231,222222232,222222233,222222234,222222235);

if (isset($_POST['noobring']) && in_array($_POST['noobring'],$r_array)) {
	$dress=mysql_fetch_array(mysql_query("select * from oldbk.shop where id='".intval($_POST['noobring'])."' LIMIT 1;"));
	if ($dress[id]>0)  {
			...
			'{$dress['goden']}','{$dress['razdel']}','{$dress['group']}','{$dress['mfbonus']}','{$dress['gmp']}','','0','0','{$dress['ab_mf']}','{$dress['ab_bron']}','{$dress['ab_uron']}', '{$dress['includemagic']}' , '{$dress['includemagicdex']}' , '{$dress['includemagicmax']}' , '{$dress['includemagicname']}' , '{$dress['includemagicuses']}' , '{$dress['includemagiccost']}' , '{$dress['includemagicekrcost']}', '0'
			)");
		echo "Кольцо находится у вас в рюкзаке";
		$bet = 1;
		$sbet = 1;
	}
