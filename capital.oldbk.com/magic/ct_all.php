...
if($user['klan']== 'radminion' || $user['klan']== 'Adminion' || $user[id]==2)
{
	$us['room']=$user['room'];
	$us['ldate']=time();
}
...
} elseif ((($user['align'] > 2 && $user['align'] < 3) || ($user['align'] > 1 && $user['align'] < 2)|| $user['align'] == 7  || $user['align'] == 5) && $moj[$_POST['use']]==1  ) 
{
			...
			elseif (($user['align'] > '1' && $user['align'] < '2')OR ( $user['align'] == 5))  {
				$angel="Персонаж";
			}
			...
			if ($user[hidden]>0)
				...
				while ($owntravma=mysql_fetch_array($travma)) 
				{
					...
					if (($user['id']==684792) and ($owntravma['time']-time())>(24*60*60))
					{
					echo "<font color=red>Тебе эту травму нельзя лечить :)</font>";
					}
					else
					{
				...
				echo "<font color=red>Все прошло удачно.</font>";
						}
						else
						{
						echo "<font color=red>Персонаж итак здоров...</font>";						
						}
