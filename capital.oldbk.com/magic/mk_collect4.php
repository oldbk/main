...
if (($item['id']>0) and  ($item['prototype']==114010)) //Футляр коллекции №4';
	{
		//отработка
										...
										if (mysql_affected_rows()>0)
											{
												...
												$rec['add_info']="Коллекция «Руины Старого Замка»! Из Карт:(".implode(",",$ccids).") Футляр:".$che;
												add_to_new_delo($rec); //юзеру
								 				$rec=array();

					 						$getitems1=DropBonusItem(4018,$user,'Удача','Коллекция «Руины Старого Замка»',30,1,37,true,true);
											echo "<font color=red><b>Поздравляем! Удачно собрана коллекция «Руины Старого Замка»!<b></font> ";
											$bet=1;
											$sbet = 1;
											}
							}
