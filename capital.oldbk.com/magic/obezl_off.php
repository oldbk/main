		...
		if ($tar['id'])
		{
...

			{
				...
				if ($effect['time']) {
					...
					if ($ok == 1) {
						if (mysql_query("DELETE FROM ".$db_city[$tar[id_city]]."`effects` WHERE `owner` = '{$tar['id']}' and `type` = '5' LIMIT 1 ;")) {
							...
							mysql_query("INSERT INTO oldbk.`paldelo`(`id`,`author`,`text`,`date`,`m_type`) VALUES ('','".$_SESSION['uid']."','$mess','".time()."','7');");
							addch("<img src=i/magic/obezl_off.gif> $mess",$user['room'],$user['id_city']);
							if($tar[room]!=$user[room])
				                        {
				                        	addchp (' <img src=i/magic/obezl_off.gif> '.$mess,'{[]}'.$tar['login'].'{[]}',$tar['room'],$tar['id_city']);
				                        }
