...
//
		...
		if ($tar['id']) 
		{
			...
			if($tar[id_city]==$user[id_city] || ADMIN)
			{
				...
				$effect = mysql_fetch_array(mysql_query("SELECT * FROM ".$db_city[$tar[id_city]]."`effects` WHERE `owner` = '{$tar['id']}' and `type` = '3'  LIMIT 1;"));
				if ($effect['time']) {
					$ok=0;
					...
					if ($ok == 1) {
						if (mysql_query("DELETE FROM ".$db_city[$tar[id_city]]."`effects` WHERE `owner` = '{$tar['id']}' and `type` = '3' and id = ".$effect['id'])) {
							...
							$mess="$angel &quot;{$user['login']}&quot; $action заклятие форумного молчания с &quot;$realnametarget&quot;.";
							$messch="$angel &quot;{$user['login']}&quot; $action заклятие форумного молчания с &quot;$target&quot;.";
							mysql_query("INSERT INTO oldbk.`lichka`(`id`,`pers`,`text`,`date`) VALUES ('','".$tar['id']."','$mess','".time()."');");
							mysql_query("INSERT INTO oldbk.`paldelo`(`id`,`author`,`text`,`date`,`m_type`) VALUES ('','".$_SESSION['uid']."','$mess','".time()."','13');");
							addch("<img src=i/magic/sleepf_off.gif> $messch",$user['room'],$user['id_city']);
							if($tar[room]!=$user[room])
				                        {
				                        	addchp (' <img src=i/magic/sleepf_off.gif> '.$messch,'{[]}'.$tar['login'].'{[]}',$tar['room'],$tar['id_city']);
				                        }
