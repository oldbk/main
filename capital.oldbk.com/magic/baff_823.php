...
elseif($user[hp]<=0) {      err('Для Вас бой окончен!');        }
	...
	$jert = mysql_fetch_array(mysql_query("SELECT * FROM `users_clons` WHERE `login` = '{$targ}' and battle='{$user[battle]}' and battle_t!='{$user[battle_t]}' and hp>0 and mklevel>0   LIMIT 1;"));
	if ($jert[id] >0)
	{
	...
	if (($get_test_baff[id] > 0) )
	{
	 ...
	 if ($get_var_baff[baf_823_use]>0)
		...
		while ($baff_owners = mysql_fetch_array($get_count_baff))
			...
			{
			//никого я начинаю
				mysql_query("INSERT INTO `effects` SET `type`='824',`name`='Начало Светлое изгнание', add_info='{$jert[id]}'  ,`time`='".(time()+60)."', `owner`='{$user[id]}',`battle`='{$user[battle]}';");//1 минута
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>, на '.nick_in_battle($jert,$jert[battle_t]).'. <i>(Начал цепь...)</i> <BR>');
			...
			{
			//есть 1 я продолжаю
				mysql_query("INSERT INTO `effects` SET `type`='824',`name`='Продолжение Светлое изгнание', add_info='{$jert[id]}' ,`time`='".$remem_time[1]."',`owner`='{$user[id]}',`battle`='{$user[battle]}';");//время дублируем с эфекта начала
				if (mysql_affected_rows()>0)
				{
