...
if ($user['battle'] > 0) {
	...
	if (rand(1,100) < $int) {
		...
		$svitok = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`inventory` WHERE `name` = 'Заточка на дубины +5' AND `setsale`=0 AND `bs_owner`='{$user[in_tower]}' AND (arsenal_klan = '' OR arsenal_owner=1 ) AND `owner` = '{$user['id']}' LIMIT 1;"));


		if (($dress[otdel]==12) && $svitok)
		{
			if (mysql_query("UPDATE oldbk.`inventory` SET `sharped` = 1, `name` = CONCAT(`name`,'+5'), `minu` = `minu`+5, `maxu`=`maxu`+5, `cost` = `cost`+30, `ndubina` = `ndubina`+5, `nlovk` = `nlovk`+5 WHERE `id` = {$dress['id']} LIMIT 1;")) {
			...
			if ($user[id]==14897)
			{
			echo "SELECT * FROM oldbk.`inventory` WHERE  present!='Арендная лавка' AND (arsenal_klan = '' OR arsenal_owner=1 ) AND `setsale`=0 AND `bs_owner`='{$user[in_tower]}' AND `owner` = '{$user['id']}' AND `name` = '{$_POST['target']}' AND `sharped` = 0 LIMIT 1;" ;
			print_r($dress);
			echo "<br>";
			print_r($svitok);			
			}
