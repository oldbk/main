...
if ($rowm['duration']+1 == $rowm['maxdur']) {
	...
	//2018-10-31 00:00:00
		$goden_do = mktime(0,0,0,11,1,2018); 
		$goden = round(($goden_do-time())/60/60/24); if ($goden<1) {$goden=1;}

	mysql_query('UPDATE inventory SET maxdur = 1, duration = 0, magic = 0, type = 77, dategoden='.$goden_do.' , goden='.$goden.'  , prototype = 2018411 WHERE id ='.$rowm['id'].' LIMIT 1') or die();
} else {
	$bet = 1;
	$sbet = 1;
