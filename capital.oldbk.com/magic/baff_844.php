...
{	
	...
	$get_imun_baff= mysql_fetch_array(mysql_query("select * from effects where owner='{$user[id]}'  and type='845' ; "));
		...
		else
		{
			...
			if (mysql_affected_rows()>0)
				{
			mysql_query("INSERT INTO `effects` SET `type`='845',`name`='Задержка на использование заклятия Озарение',`time`='".(time()+24*60*60)."',`owner`='{$user[id]}' ;");//24 часов задержка
		
			 $mag_gif='<img src=i/magic/6n3.jpg>';
