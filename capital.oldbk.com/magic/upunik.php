...
$che=(int)($_POST['target']);
	...
	if ($che>0)
	{
	$item = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.inventory WHERE id='{$che}' and owner = '{$_SESSION['uid']}' and (sowner=0 or sowner = '{$user['id']}') AND type in (1,2,3,4,5,8,9,10,11,27,28) AND unik=1  AND `dressed`= 0 AND bs_owner = '".$user[in_tower]."' AND `setsale`=0 AND `prokat_idp` = 0 AND (arsenal_klan = '' OR arsenal_owner=1 ) AND present!='Арендная лавка' ;"));
		...
		if ($item['id']>0)
		{
				...
				if (mysql_query("UPDATE oldbk.`inventory` SET `unik` = 2 ".$str2." , stbonus=stbonus+1 ".$str.", `sebescost` = `sebescost` + ".$rowm['cost']." WHERE `id` = {$item['id']} ;")) 
				{

					echo "<font color=red><b>Предмет \"{$item['name']}\" теперь имеет улучшенную уникальную модификацию.<b></font> ";
					$bet=1;
					$sbet = 1;
