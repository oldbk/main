...
$targ=($_POST['target']);
$baff_name='Светлое Возмездие';
$baff_type=709;//809 , 710-негатив
...
elseif($user[hp]<=0) {      err('Для Вас бой окончен!');        }
	...
	$get_test_baff= mysql_fetch_array(mysql_query("select * from effects where owner='{$user[id]}' and battle='{$user[battle]}' and type='809' ; "));
	if (($get_test_baff[id] > 0) )
	{
	 ...
	 if ($get_var_baff[baf_709_use]>0)
		...
		$get_count_baff=mysql_query("select  * from effects where battle='{$user[battle]}' and type=809 and add_info='{$jert[id]}' and owner in (select id from users where battle='{$user[battle]}' and battle_t='{$user[battle_t]}' and hp >0); ");
		$kow=0;
		while ($baff_owners = mysql_fetch_array($get_count_baff))
			...
			{
			//никого я начинаю
				mysql_query("INSERT INTO `effects` SET `type`='809',`name`='Начало Светлого Возмездия', add_info='{$jert[id]}'  ,`time`='".(time()+60)."', `owner`='{$user[id]}',`battle`='{$user[battle]}';");//1 минута
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>, на персонажа '.nick_in_battle($jert,$jert[battle_t]).'. <i>(Начал цепь...)</i> <BR>');
			...
			{
			//есть 1 я продолжаю
				mysql_query("INSERT INTO `effects` SET `type`='809',`name`='Продолжение Светлого Возмездия', add_info='{$jert[id]}' ,`time`='".$remem_time[1]."',`owner`='{$user[id]}',`battle`='{$user[battle]}';");//время дублируем с эфекта начала
				if (mysql_affected_rows()>0)
				{
					if ($user[hidden]>0 and $user[hiddenlog]=='') 	{ $user[sex]=1;	}
					elseif ($user[hidden]>0 and $user[hiddenlog]!='') {  $fuser=load_perevopl($user); $user[sex]=$fuser[sex]; }
...
//				addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>, на персонажа '.nick_in_battle($jert,$jert[battle_t]).'. <i>(Продолжил цепь...)</i> <BR>');
			...
			mysql_query("INSERT INTO `effects` SET `type`='710',`name`='{$baff_name}',`time`=1999999999,`owner`='{$jert[id]}',`lastup`=5,`battle`='{$jert[battle]}';");//ставим негативный  баф жертве
			//удаляем начало бафа
			foreach($remem_own as $ic=>$owner)
				{
			mysql_query("DELETE from effects where `type`='809' and owner='{$owner}' and battle='{$user[battle]}' and add_info='{$jert[id]}'  ;"); 	
...
//			addlog($user['battle'],'<span class=date>'.date("H:i").'</span> '.nick_in_battle($user,$user[battle_t]).' использовал'.$action.' заклятие <b>"'.$baff_name.'"</b>, на персонажа '.nick_in_battle($jert,$jert[battle_t]).'. <i>(Закрыл цепь...)</i> <BR>');								
			...
			mysql_query("INSERT `battle_vars` (`battle`,`owner`,`update_time`,`baf_709_use`) values('".$jert['battle']."', '".$jert['id']."', '".time()."' , '1' ) ON DUPLICATE KEY UPDATE `baf_709_use` =`baf_709_use`+1;");
