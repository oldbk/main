...
<?php
	if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['nick'])) {
		...
		if ($owner !== FALSE) {
			$item = mysql_fetch_assoc(mysql_query('SELECT * FROM oldbk.shop WHERE id = '.intval($_POST['mid'])));
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['nickok'])) {
		try {
			$dress = mysql_fetch_assoc(mysql_query('SELECT * FROM oldbk.shop WHERE id = '.intval($_POST['mid'])));
			...
			$dress['otdel'] == '' ? $xx = $dress['razdel'] : $xx = $dress['otdel'];
...
<A HREF="?otdel=14&sid=&0.0943516060419363">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;луки и арбалеты</A><BR>
...
<A HREF="?otdel=60&sid=&0.925798340638547">Молитвенные предметы</A><BR>
...
<?php
	if (isset($_GET['otdel'])) {
		...
		$data = mysql_query('SELECT * FROM oldbk.`shop` WHERE `razdel` = '.intval($_GET['otdel']).' AND name NOT LIKE "%прокат%" and img not LIKE "%art_%" and type != 200 and cost > 1 ORDER by `cost` ASC');
