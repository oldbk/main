...
$rep_kof=2; // множитель репутации на 1 бой
	   ...
	   if ($_SERVER["SERVER_NAME"]=='capitalcity.oldbk.com') { $cityname='capitalcity' ;  }
		...
		include ("labconfig_s.php"); // настройки
...
////////////////////////////////////////////load map file//////////////////////////////////////////////////
		...
		if (file_exists($mfile)) 
			...
			{
			copy('/www/capitalcity.oldbk.com/labmaps/default_lab.map', $mfile);
			$map=file($mfile);
			}
...
*/
/////////////////IPUT VAR USERS///////////////////////////////////////////////////////////////////////////
if (($_REQUEST[talk]==73)and($map[$labpoz[x]][$labpoz[y]]=='F'))
	{
...
if (($_POST[gotolab2])and($map[$labpoz[x]][$labpoz[y]]=='F'))
{
//1. проверить все ли тут со мной на одной клетке
 $kol_kom=$Tcount+1; // команда + 1я
 $laba_rooma=$mapid+1000;
 $tut_vse=mysql_fetch_array(mysql_query("select count(*) from labirint_users where map='{$mapid}' and x='{$labpoz[x]}' and y='{$labpoz[y]}' ;"));
 $have_her=mysql_fetch_array(mysql_query("select count(*) from users where room='{$laba_rooma}' and medals like '%011;%'"));
 ...
 if ( ($kol_kom==$tut_vse[0]) and ($have_her[0]>0))
 	{
 	//тут все
	$msg="Все тут....поехали...";
	// очищаем отработаные итемы
	...
	//ставим всех чаров в старт
	mysql_query("update `labirint_users` SET x=0,y=0 where `map`='".$mapid."'; ");
	// ставим флаг всем чарам которые в этой лабе флаг лабы 2

	mysql_query("update `users` SET lab=2 where lab=1 and `room`='".$laba_rooma."'; ");
	//обновляем карту
	$gen_map=true;
	include ('labogenerator2.php');
	// чистим файлы квестов
	// они потом создадутся сами новые и актуальные
	$delfiles='/www/capitalcity.oldbk.com/labmapsq/'.$mapid.'-*.qst';
	...
	echo "
 	...
 	{
	$msg="<font color=red><b>Тут собрались не все... либо среди вас нет ни одного героя...</b></font>";
 	}
...
if ($_GET[getitem])
				{
			           ...
			           if ($getitem[0]>0)
			           {
						...
						if (($row['setsale'] ==0 )  AND  ($row['dressed'] ==0 )  )
		           		...
		           		//all is good
		           		//del item from map
		          		mysql_query("UPDATE `labirint_items` SET `active`=1 where `x`='".$labpoz[x]."' and `y`='".$labpoz[y]."' and `map`='".$mapid."' and `active`=0 and `item`='I' and `val`='".$getitem[val]."' LIMIT 1;");
				...
				VALUES
				('{$dress['id']}','{$user['id']}','{$dress['name']}','{$dress['type']}',{$dress['massa']},{$dress['cost']},'{$dress['img']}', '{$dress['img_big']}' , '11','{$dress['rareitem']}','{$dress['ekr_flag']}' ,{$dress['duration']},{$dress['maxdur']},{$dress['isrep']},'{$dress['nclass']}','{$dress['gsila']}','{$dress['glovk']}','{$dress['ginta']}','{$dress['gintel']}','{$dress['ghp']}','{$dress['gnoj']}','{$dress['gtopor']}','{$dress['gdubina']}','{$dress['gmech']}','{$dress['gfire']}','{$dress['gwater']}','{$dress['gair']}','{$dress['gearth']}','{$dress['glight']}','{$dress['ggray']}','{$dress['gdark']}','{$dress['needident']}','{$dress['nsila']}','{$dress['nlovk']}','{$dress['ninta']}','{$dress['nintel']}','{$dress['nmudra']}','{$dress['nvinos']}','{$dress['nnoj']}','{$dress['ntopor']}','{$dress['ndubina']}','{$dress['nmech']}','{$dress['nfire']}','{$dress['nwater']}','{$dress['nair']}','{$dress['nearth']}','{$dress['nlight']}','{$dress['ngray']}','{$dress['ndark']}',
...
if ($_GET[sunduk])
			...
			if(mysql_affected_rows()>0)			
				...
				VALUES
				('{$dress['id']}','{$user['id']}','{$dress['name']}','{$dress['type']}',{$dress['massa']},{$dress['cost']},'{$dress['img']}', '{$dress['img_big']}' , '11','{$dress['rareitem']}','{$dress['ekr_flag']}',{$dress['duration']},{$param['maxdur']},{$dress['isrep']},'{$dress['nclass']}','{$dress['gsila']}','{$dress['glovk']}','{$dress['ginta']}','{$dress['gintel']}','{$dress['ghp']}','{$dress['gnoj']}','{$dress['gtopor']}','{$dress['gdubina']}','{$dress['gmech']}','{$dress['gfire']}','{$dress['gwater']}','{$dress['gair']}','{$dress['gearth']}','{$dress['glight']}','{$dress['ggray']}','{$dress['gdark']}','{$dress['needident']}','{$dress['nsila']}','{$dress['nlovk']}','{$dress['ninta']}','{$dress['nintel']}','{$dress['nmudra']}','{$dress['nvinos']}','{$dress['nnoj']}','{$dress['ntopor']}','{$dress['ndubina']}','{$dress['nmech']}','{$dress['nfire']}','{$dress['nwater']}','{$dress['nair']}','{$dress['nearth']}','{$dress['nlight']}','{$dress['ngray']}','{$dress['ndark']}',
...
<form method=POST >
    <div class="btn-control">
        <input class="button-big btn" type=Submit name=exit value='Выйти и потерять все найденное!' onClick="return confirmSubmit('Действительно хотите Выйти и потерять все найденное?')" >
    </div>
</form>
<?
	...
	foreach ($id_bot as $k=>$v)
		...
		*/
				...
				//	addlog($id_battl,"Часы показывали <span class=date>".date("Y.m.d H.i")."</span>, когда ".$rr." бросили вызов друг другу. <BR>");
					...
					// перенес выше mysql_query("UPDATE users SET `battle` ={$id_battl},`zayavka`=0, `battle_t`=1 WHERE `id`= '".$user[id]."';");
...
//////////////////////
		...
		if (($room_map=="F"))
			...
			*/
			...
			echo "<br>";
			//if ( ($user[id]==14897) OR ($user[id]==188) OR ($user[id]==11899))
				{
			echo '<b><i>Вы можете войти в Героический лабиринт, для этого вам надо собраться тут вместе...<br>и только один из вас сможет переместить всех...</i></b>';

				$labro=1000+$mapid;
				$i_have_button=mysql_fetch_array(mysql_query("select *  from labirint_users where map='{$mapid}' ORDER BY start LIMIT 1;"));

				if ($user[id]==$i_have_button[owner])
				{
				echo "<form method=POST action=lab.php><input type=submit name=gotolab2 value='Перейти в Героический лабиринт!' onClick=\"return confirmSubmit('Действительно хотите перейти?')\" ></form>";
				}
				  }
...
?>
  ...
  <tr>
    ...
    <td width="12" ></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab.php?gto=p1";'><img src="http://i.oldbk.com/i/laba/arr1.gif" border="0" title="Вверх" alt="Вверх" /></a></td>
    <td width="13"></td>
    <td width="26"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="26" height="26"><a href="#" onclick='location.href="lab.php?gto=p4";'><img src="http://i.oldbk.com/i/laba/arr4.gif" border="0" title="Налево" alt="Налево" /></a></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab.php?refresh=13.4";'><img src="http://i.oldbk.com/i/laba/refresh.gif" border="0" title="Обновить" alt="Обновить" /></a></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab.php?gto=p2";'><img src="http://i.oldbk.com/i/laba/arr2.gif" border="0" title="Направо" alt="Направо" /></a></td>
  </tr>
  <tr>
    <td ></td>
    <td></td>
  ...
  <tr>
    <td></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab.php?gto=p3";'><img src="http://i.oldbk.com/i/laba/arr3.gif" border="0" title="Вниз" alt="Вниз" /></a></td>
    <td></td>
    <td></td>
  </tr>
</table>    </td>
