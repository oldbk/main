...
function callcgold(kol,kurs)
{
document.getElementById('awmz').value=Math.floor(kol*kurs);
}
...
$k=document.getElementById('qbil').value;
	if (kurs==1)
	{
	document.getElementById('qrub').value=($tt*$k);	
	}
	else
	{
	document.getElementById('qrub').value=Math.ceil(kurs*$tt*$k);
...
function showpages(p,pmax)
{
        for (var i = 0; i <= pmax; i++) 
	        {
			if (p==i)
	        		  {
      		  			document.getElementById("bp"+i).style.display='block';
	        		  }
	        		  else
	        		  {
