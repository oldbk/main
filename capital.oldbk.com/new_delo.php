...
function add_to_new_delo($rec)
{
		...
		if  ((($rec['item_type']>0 && $rec['item_type']<12 || $rec['item_type']==28 || $rec['item_type']==555 || $rec['item_type']==27 || (in_array($rec['item_proto'],$sert))) && $rec['type']!=32 && $rec['type']!=33) OR ($rec['type']==1179 || $rec['type']==1180 ||  $rec['type']==1181 ||  $rec['type']==179 || $rec['type'] == 1303 || $rec['type'] == 1305 || $rec['type'] == 1306) )
...
function get_delo_rec($rec,$al,$sql_str="",$add_info_off)
{
     ...
     //креды/екры/репа
	{
	    //креды
	 	{
	 		//получил
		 	{
				...
				if ($rec['type'] == 1009) {
					if ($rec['sum_kr'] > 0) {
						$delo_type[1009]="\"{$rec['owner_login']}\" купил наёмника \"{$rec['item_name']}\" за ".$rec['sum_kr']." кр.";
					} elseif ($rec['sum_ekr'] > 0) {
						$delo_type[1009]="\"{$rec['owner_login']}\" купил наёмника \"{$rec['item_name']}\" за ".$rec['sum_ekr']." екр.";
					} elseif (strlen($rec['add_info'])) {
						$t = explode("/",$rec['add_info']);
						$delo_type[1009]="\"{$rec['owner_login']}\" купил наёмника \"{$rec['item_name']}\" за ".$t[0]." монет.";
					} else {
						$delo_type[1009]="\"{$rec['owner_login']}\" купил наёмника \"{$rec['item_name']}\" за ".$rec['sum_rep']." реп.";
					}
				}
				$delo_type[1010]="Наемник \"{$rec['target_login']}\" взял следующий ап {$rec['add_info']} опыта.";
				$delo_type[1011]="Наемник \"{$rec['target_login']}\" перешел на следующий {$rec['add_info']} уровень.";
				...
				$delo_type[1515]="\"{$rec['owner_login']}\" получил {$rec['sum_kr']} кр. поединке №{$rec['battle']}";
				...
				$delo_type[50]="\"{$rec['owner_login']}\" получил {$rec['sum_kr']} кр. на свой счет №.{$rec['bank_id']} в банке со счета №{$rec['add_info']} персонажа \"{$rec['target_login']}\" комисия {$rec['sum_kom']} кр.";
			    	...
			    	$delo_type[101]="\"{$rec['owner_login']}\" получил {$rec['sum_kr']} кр., {$rec['sum_rep']} репутации за выйгрыш в турнире Башни смерти";
			        ...
			        $delo_type[115]="\"{$rec['owner_login']}\" получен выигрыш на Фонтане Удачи {$rec['sum_ekr']} екр.";

				$delo_type[1104]="\"{$rec['owner_login']}\" получено {$rec['sum_kr']} кр. от  \"{$rec['target_login']}\".";
				$delo_type[1201]="\"{$rec['owner_login']}\" получено {$rec['sum_kr']} кр. от  \"{$rec['target_login']}\" за принудетельный сброс АПА на \"{$rec['item_name']}\".";
				...
				$delo_type[113]="\"{$rec['owner_login']}\" совершил переход на 14й уровень. Получен бонус +100% опыта пока не будет набрано:{$rec['add_info']} опыта ";
				$delo_type[114]="\"{$rec['owner_login']}\" Бонус +100% опыта окончен, набрано:{$rec['add_info']} опыта.";
				...
				$delo_type[368]="\"{$rec['owner_login']}\" получил {$rec['sum_kr']} кр. возврат за свиток \"Улучшение вещи {$rec['item_ups']}\" ";
				$delo_type[511]="\"{$rec['owner_login']}\" получил  {$rec['sum_kr']} кр. на счет {$rec['bank_id']} за продажу  {$rec['sum_ekr']} екр. на бирже";
				$delo_type[512]="\"{$rec['owner_login']}\" изьято  {$rec['add_info']} золотых монет с перечислением на счет персонажа.";

		        }
	        //заплатил
		        {
      	                	$delo_type[2510]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} кр. со счета №{$rec['bank_id']},  за увеличение вместимости сундука!";
			        $delo_type[289]="\"{$rec['owner_login']}\" пожертвовал {$rec['sum_kr']} кр., {$rec['sum_ekr']} екр., {$rec['sum_rep']} репутации на строительство Замка Лорда Разрушителя.";
			        $delo_type[290]="\"{$rec['owner_login']}\" пожертвовал \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. на строительство Замка Лорда Разрушителя.";
				...
				$delo_type[134]="\"{$rec['owner_login']}\" перевел {$rec['sum_kr']} кр. в казну клана {$rec['target_login']}. ({$rec['add_info']})";
				$delo_type[135]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} кр. за приглашение персонажа {$rec['target_login']} в клан.";
				...
				$delo_type[1710]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} кр. за сброс класса у Знахаря.";
				$delo_type[1711]="\"{$rec['owner_login']}\" установил класс {$rec['add_info']} у Знахаря.";
				$delo_type[5010]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} кр. за сброс умений у Знахаря.";

				$delo_type[1105]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} кр. к  \"{$rec['target_login']}\".";

				$delo_type[175]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']}кр. за лечение травмы в БС.";
				$delo_type[1175]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']}кр. за лечение травмы в Лабиринте.";
				...
				if ($rec['type'] == 191) {
					if ($rec['sum_kr'] > 0) {
						$delo_type[191]="\"{$rec['owner_login']}\" отремонтировал предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$rec['sum_kr']." кр.";
					} elseif ($rec['sum_ekr'] > 0) {
						$delo_type[191]="\"{$rec['owner_login']}\" отремонтировал предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$rec['sum_ekr']." екр.";
					} elseif (strlen($rec['add_info'])) {
						$t = explode("/",$rec['add_info']);
						$delo_type[191]="\"{$rec['owner_login']}\" отремонтировал предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$t[0]." монет.";
					} else {
						$delo_type[191]="\"{$rec['owner_login']}\" отремонтировал предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";
					}
				}

				if ($rec['type'] == 1991) {
					if ($rec['sum_kr'] > 0) {
						$delo_type[1991]="\"{$rec['owner_login']}\" участвовал в Колесе Фортуны и получил предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$rec['sum_kr']." кр.";
					} elseif ($rec['sum_ekr'] > 0) {
						$delo_type[1991]="\"{$rec['owner_login']}\" участвовал в Колесе Фортуны и получил предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$rec['sum_ekr']." екр.";
					} elseif (strlen($rec['add_info'])) {
						$t = explode("/",$rec['add_info']);
						$delo_type[1991]="\"{$rec['owner_login']}\" участвовал в Колесе Фортуны и получил предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".$t[0]." монет.";
					} else {
						$delo_type[1991]="\"{$rec['owner_login']}\" участвовал в Колесе Фортуны и получил предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";
					}
				}

				$delo_type[192]="\"{$rec['owner_login']}\" сбросил АП предмета \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".($rec['sum_kr'] > 0 ? $rec['sum_kr']." кр. ":"")."".($rec['sum_ekr'] > 0 ? $rec['sum_ekr']." екр. ":"");
				$delo_type[193]="\"{$rec['owner_login']}\" сбросил СТАТЫ предмета \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за ".($rec['sum_kr'] > 0 ? $rec['sum_kr']." кр. ":"")."".($rec['sum_ekr'] > 0 ? $rec['sum_ekr']." екр. ":"");
				...
				$delo_type[3368]="\"{$rec['owner_login']}\" перевел {$rec['sum_kr']} кр. на счет, к персонажу \"{$rec['target_login']}\".";
				$delo_type[3373]="\"{$rec['owner_login']}\" получил ".if_have("",$rec['sum_kr']," кр.")." ".if_have("",$rec['sum_ekr']," екр.")."  на счет, со счета №{$rec['bank_id']} от персонажа \"{$rec['target_login']}\".";


				if ($rec['type'] == 506) { $t = explode("/",$rec['add_info']); $delo_type[506]="\"{$rec['owner_login']}\" возврат {$t[0]} монет за отказ услуги КО.";}
				if ($rec['type'] == 507) { $t = explode("/",$rec['add_info']); $delo_type[507]="\"{$rec['owner_login']}\" заплатил {$t[0]} монет за услуги КО.";}
				if ($rec['type'] == 577) { $t = explode("/",$rec['add_info']); $delo_type[577]="\"{$rec['owner_login']}\" заплатил {$t[0]} монет за услуги КО. (оплата штрафа)";}

				$delo_type[578]="\"{$rec['owner_login']}\" заплатил  1$ (оплата неизлечимой травмы)";

				if ($rec['type'] == 509) { $t = explode("/",$rec['add_info']); $delo_type[509]="\"{$rec['owner_login']}\" заплатил {$t[0]} монет за \"{$rec['item_name']}\" {$rec['item_count']} шт. на Ярмарке.";}

				if ($rec['type'] == 599) { $t = explode("/",$rec['add_info']); $delo_type[599]="\"{$rec['owner_login']}\" обменял {$t[0]} монет на \"{$rec['sum_kr']}\" кр. в Банке.";}

				if ($rec['type'] == 508) { $t = explode("/",$rec['add_info']); $delo_type[508]="\"{$rec['owner_login']}\" получил {$t[0]} монет за возврат \"{$rec['item_name']}\" {$rec['item_count']} шт. на Ярмарке.";}

				if ($rec['type'] == 3636) { $t = explode("/",$rec['add_info']); $delo_type[3636]="\"{$rec['owner_login']}\" получил {$t[0]} монет от персонажа \"{$rec['target_login']}\" заплатил   \"{$rec['sum_kr']}\" кр.";}
				if ($rec['type'] == 3737) { $t = explode("/",$rec['add_info']); $delo_type[3737]="\"{$rec['owner_login']}\" отдал {$t[0]} монет персонажу \"{$rec['target_login']}\" получил  \"{$rec['sum_kr']}\" кр.";}


				$delo_type[588]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации за возврат \"{$rec['item_name']}\" {$rec['item_count']} шт. в храм. лавке.";
				if ($rec['type'] == 519) { $t = explode("/",$rec['add_info']); $delo_type[519]="\"{$rec['owner_login']}\" получил {$t[0]} монет за возврат \"{$rec['item_name']}\" {$rec['item_count']} шт.";}
				if ($rec['type'] == 521) { $t = explode("/",$rec['add_info']); $delo_type[521]="\"{$rec['owner_login']}\" получил {$t[0]} монет.";}
	            	}
		}
		//екры
		{
		    //получил
		    	{
			    	...
			    	$delo_type[355]="\"{$rec['owner_login']}\" получил бонус за покупку {$rec['sum_ekr']} екр. на свой счет №{$rec['bank_id']} в банке, от дилера \"{$rec['target_login']}\" ";
			    	$delo_type[356]="\"{$rec['owner_login']}\" получил бонус за установку изображения {$rec['sum_ekr']} екр. на свой счет №{$rec['bank_id']} в банке.";
			    	$delo_type[357]="\"{$rec['owner_login']}\" получил бонус по акции {$rec['sum_ekr']} екр. на свой счет №{$rec['bank_id']} в банке.";
				...
				$delo_type[370]="\"{$rec['owner_login']}\" получил {$rec['sum_ekr']} екр. на счет {$rec['bank_id']} за акцию \"{$rec['target_login']}\".";
				$delo_type[3369]="\"{$rec['owner_login']}\" перевел {$rec['sum_ekr']} екр. на счет Коммерческого отдела, к персонажу \"{$rec['target_login']}\".";
				$delo_type[3370]="\"{$rec['owner_login']}\" получил {$rec['sum_ekr']} екр. на счет КО, со счета №{$rec['bank_id']} от персонажа \"{$rec['target_login']}\".";
				$delo_type[3371]="\"{$rec['owner_login']}\" перевел {$rec['sum_ekr']} екр. на счет №{$rec['bank_id']} к персонажу \"{$rec['target_login']}\" (возврат).";
				$delo_type[3372]="\"{$rec['owner_login']}\" получил {$rec['sum_ekr']} екр. на счет №{$rec['bank_id']} от персонажа \"{$rec['target_login']}\" (возврат).";
				$delo_type[510]="\"{$rec['owner_login']}\" купил {$rec['sum_ekr']} екр. на счет {$rec['bank_id']} за {$rec['sum_kr']} кр.  на бирже";
				$delo_type[1106]="\"{$rec['owner_login']}\" получено {$rec['sum_ekr']} екр. от  \"{$rec['target_login']}\", со счета №{$rec['bank_id']}.";
				$delo_type[1324]="\"{$rec['owner_login']}\" получил {$rec['sum_ekr']} eкр. на счет №{$rec['bank_id']}, за возврат оплаты мест в ристалище!";
				$delo_type[514]="\"{$rec['owner_login']}\" вернул с биржи {$rec['sum_ekr']} екр. на свой счет {$rec['bank_id']}";
		    	}
			//заплатил
			{
	                	$delo_type[323]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} eкр. со счета №{$rec['bank_id']},  за покупку  магии стихий у Знахаря!";
	                	$delo_type[1323]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} eкр. со счета №{$rec['bank_id']},  за оплату одного места в ристалище!";
	                	$delo_type[2520]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} eкр. со счета №{$rec['bank_id']},  за увеличение вместимости сундука!";


				$delo_type[513]="\"{$rec['owner_login']}\" выставил на биржу {$rec['sum_ekr']} екр. со счета {$rec['bank_id']}";
				...
				$delo_type[5858]="\"{$rec['owner_login']}\" оплата услуг партнера ({$rec['add_info']}) {$rec['sum_ekr']} екр, через дилера \"{$rec['target_login']}\".";

 				$delo_type[59]="\"{$rec['owner_login']}\" приобрел/продлил Silver account за {$rec['sum_ekr']} екр,  сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\"."; //5 екр на счет №{$rec['bank_id']},
 				$delo_type[459]="\"{$rec['owner_login']}\" приобрел/продлил Silver account за {$rec['sum_ekr']} екр, сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\".";

 				$delo_type[359]="\"{$rec['owner_login']}\" приобрел/продлил Gold account за {$rec['sum_ekr']} екр, сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\"."; //5 екр на счет №{$rec['bank_id']},
 				$delo_type[559]="\"{$rec['owner_login']}\" приобрел/продлил Gold account за {$rec['sum_ekr']} екр, сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\".";

 				$delo_type[358]="\"{$rec['owner_login']}\" приобрел/продлил Platinum account за {$rec['sum_ekr']} екр, сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\"."; //5 екр на счет №{$rec['bank_id']},
 				$delo_type[558]="\"{$rec['owner_login']}\" приобрел/продлил Platinum account за {$rec['sum_ekr']} екр, сроком до {$rec['add_info']}, через  \"{$rec['target_login']}\".";
				...
				$delo_type[2283]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} екр. со своего счета №.{$rec['bank_id']} в банке ".if_have(" дополнительно уплатил компенсацию: ",$rec['sum_kr']," кр.")." за услуги Коммерческого отдела. {$rec['add_info']}";
				...
				$delo_type[77]="\"{$rec['owner_login']}\" потратил {$rec['sum_ekr']} екр. со своего счета №{$rec['bank_id']} в банке, за использование абилки: \"Восстановление энергии +180\". (Остаток на счету:{$rec['add_info']} екр.) ";
	                	...
	                	$delo_type[199]="\"{$rec['owner_login']}\" перезарядил предмет \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. за {$rec['sum_rep']} репутации.";
				$delo_type[1107]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} екр. к  \"{$rec['target_login']}\", со счета №{$rec['bank_id']}.";
				$delo_type[1108]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} екр. к  \"{$rec['target_login']}\", со счета №{$rec['bank_id']}.";
				$delo_type[2221]="\"{$rec['owner_login']}\" перевел {$rec['sum_ekr']} екр. со счета №{$rec['bank_id']}, {$rec['add_info']}  .";
		...
		//gold
		//получил
		    	$delo_type[3001]="\"{$rec['owner_login']}\" получил {$rec['item_count']} шт. Золотых Монет, от дилера \"{$rec['target_login']}\" ";
			$delo_type[3011]="\"{$rec['owner_login']}\" получил в подарок {$rec['item_count']} шт. Золотых Монет.";
		//3002
		//3003
		//3004
		//3005
	    	...
	    	//получил
		    	{
	                	...
	                	$delo_type[2829]="\"{$rec['owner_login']}\" получил бонус {$rec['sum_rep']} репутации.";
				...
				$delo_type[1183]="\"{$rec['owner_login']}\" получил {$rec['add_info']} опыта победу в турнире.";
				...
				$delo_type[204]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации за проигрыш в турнире на руинах";
	                	...
	                	$delo_type[2260]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации за обмен  {$rec['item_name']}  в банке";
				...
				$delo_type[384]="\"{$rec['owner_login']}\" заплатил {$rec['sum_rep']} репутации за сдачу артефакта в ком отдел";
				...
				$delo_type[3823]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации в бонус по свитку";
				$delo_type[383]="\"{$rec['owner_login']}\" - {$rec['add_info']}.";
	                	$delo_type[385]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации (возврат разницы в цене) за предмет {$rec['item_name']}.";
				$delo_type[371]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} репутации за акцию \"{$rec['target_login']}\".";
				$delo_type[392]="\"{$rec['owner_login']}\" получил {$rec['add_info']} Великих Побед за акцию \"{$rec['target_login']}\".";

					//тут еще и шмотки....  победы разные
				{
					$delo_type[3]="\"{$rec['owner_login']}\" получил: \"{$rec['item_name']}\" и \"{$rec['sum_rep']}\" репутации за победу на ристалище в \"Сражениях отрядов\".";
					$delo_type[4]="\"{$rec['owner_login']}\" получил: ".if_have(" За участие:",$rec['sum_kr']," кр.")." ".if_have(" В награду:",$rec['item_name']," и ")."  ".if_have("",$rec['sum_rep']," репутации ")." на ристалище в \"Одиночных сражениях\".";
				...
				$delo_type[5008]="Получен предмет {$rec['item_name']}, на сумму {$rec['sum_ekr']} екр. Сдача за возврат долга сертификатом.";
		    		$delo_type[5020]="Заплатил {$rec['sum_ekr']} екр с помощью {$rec['add_info']}, в счет долга.";
		               	$delo_type[386]="\"{$rec['owner_login']}\" заплатил {$rec['sum_rep']} репутации  за {$rec['add_info']} для артефакта {$rec['item_name']}.";
		               	$delo_type[387]="\"{$rec['owner_login']}\" заплатил {$rec['sum_rep']} репутации  за {$rec['add_info']} для артефакта {$rec['item_name']}.";
		               	$delo_type[395]="\"{$rec['owner_login']}\" заплатил {$rec['sum_rep']} репутации квестовому боту.";
...
//Предметы
	{
		//купил
		{
			$delo_type[1]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ".if_have(" ",$rec['sum_kr']," кр.")."  ".if_have(" ",$rec['sum_ekr']," екр.")."  ".if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
			$delo_type[401]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ".if_have(" ",$rec['sum_kr']," кр.")."  ".if_have(" ",$rec['sum_ekr']," екр.")."  ".if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
			...
			$delo_type[40]="\"{$rec['owner_login']}\" купил  предмет:\"{$rec['item_name']}\" ".if_have(" (x",$rec['item_count'],") ")." за {$rec['sum_kr']} кр у \"{$rec['target_login']}\"";
			...
			$delo_type[8282]="\"{$rec['owner_login']}\" получил \"{$rec['item_name']}\" за выполнение квеста на  Хэллоуин";
			...
			$delo_type[377]="\"{$rec['owner_login']}\" обменял  личный артефакт {$rec['add_info']} на новый: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] ";

			$delo_type[378]="\"{$rec['owner_login']}\" обменял  артефакт {$rec['add_info']} на новый: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_ekr']} екр(ваучерами).";

			if ($rec['type'] == 1378) {
				$t = 'Оплата монетами:';
				$pos = strpos($rec['add_info'],$t);
				if ($pos !== false) {
					$t = substr($rec['add_info'],$pos+strlen($t));
					$t = explode(":",$t);
					$t1 = substr($rec['add_info'],0,$pos-2);
					$delo_type[1378]="\"{$rec['owner_login']}\" обменял артефакт {$t1} на новый: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$t[1]} монет.";
				} else {
					$delo_type[1378]="\"{$rec['owner_login']}\" обменял артефакт {$rec['add_info']} на новый: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_ekr']} екр со счета №{$rec['bank_id']}.";
				}
			}
			$delo_type[379]="\"{$rec['owner_login']}\" отдал  артефакт \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] на обмен в коммерческий отдел за {$rec['sum_ekr']} екр(ваучерами).";
			$delo_type[1379]="\"{$rec['owner_login']}\" отдал  артефакт \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] на обмен в коммерческий отдел за {$rec['sum_ekr']} екр .";
			$delo_type[380]="\"{$rec['owner_login']}\" получил обратно артефакт \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] приотказе в обмене.";
			$delo_type[390]="\"{$rec['owner_login']}\" получил личный артефакт \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] при обмене.";


		}
		//получил/подарили
		{
			$delo_type[454]="\"{$rec['owner_login']}\" получил \"{$rec['item_name']}\" за {$rec['sum_ekr']} екр, от \"{$rec['target_login']}\".";
		    	$delo_type[452]="\"{$rec['owner_login']}\" получил \"{$rec['item_name']}\" ".if_have(" на ",$rec['sum_ekr']," екр.").", от \"{$rec['target_login']}\".";
			$delo_type[455]="\"{$rec['owner_login']}\" получил \"{$rec['item_name']}\" , от \"{$rec['target_login']}\".";
			...
			$delo_type[6060]="\"{$rec['owner_login']}\" получил в подарок предмет:\"{$rec['item_name']}\" от \"{$rec['target_login']}\", использовал \"Ленточка на Дерево Желаний\".";

			$delo_type[6062]="\"{$rec['owner_login']}\" вернул премет:\"{$rec['item_name']}\" (\"{$rec['item_id']}\") через КО.";

			$delo_type[410]="\"{$rec['owner_login']}\" получил в подарок упакованный предмет ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." от \"{$rec['target_login']}\".";
			$delo_type[14]="\"{$rec['owner_login']}\" получил в подарок предмет:\"{$rec['item_name']}\" от \"{$rec['target_login']}\" (обмен в лабиринте ).";
			...
			$delo_type[699]="\"{$rec['owner_login']}\" получил предмет: \"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." ";
			$delo_type[700]="\"{$rec['owner_login']}\" подобрал предмет: \"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." в Лабиринте Хаоса.";
			$delo_type[701]="\"{$rec['owner_login']}\" забрал предмет: \"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." из сундука, в Лабиринте Хаоса.";

			$delo_type[6044]="\"{$rec['owner_login']}\" получил в бою \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";

			$delo_type[1110]="\"{$rec['owner_login']}\" собрал коллекцию \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. и получил бонус на 90 дней";
			$delo_type[1120]="\"{$rec['owner_login']}\" собрал коллекцию \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. ";
			$delo_type[1130]="\"{$rec['owner_login']}\" собрал коллекцию \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. ";
			$delo_type[1144]="\"{$rec['owner_login']}\" собрал коллекцию \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. ";
			...
			$delo_type[2888]="\"{$rec['owner_login']}\" получил в бонус предмет: \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. при достижении 8го уровня.";
			...
			$delo_type[169]="\"{$rec['owner_login']}\" почтой получил предмет:\"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." от \"{$rec['target_login']}\"";

			$delo_type[419]="\"{$rec['owner_login']}\" получил из {$rec['add_info']}, предмет: \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";
			$delo_type[420]="\"{$rec['owner_login']}\" получил из {$rec['add_info']}, личную абилити: \"{$rec['item_name']}\"  {$rec['item_count']}шт.";

			$delo_type[1419]="\"{$rec['owner_login']}\" получил из {$rec['add_info']}, предмет: \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";
			$delo_type[1420]="\"{$rec['owner_login']}\" получил из {$rec['add_info']}, личную абилити: \"{$rec['item_name']}\"  {$rec['item_count']}шт.";


			$delo_type[421]="\"{$rec['owner_login']}\" получил из {$rec['add_info']}, предмет: \"{$rec['item_name']}\"  {$rec['item_count']}шт.";
			$delo_type[422]="\"{$rec['owner_login']}\" получил из {$rec['add_info']} \"{$rec['sum_kr']}\" кр.";

			$delo_type[98]="\"{$rec['owner_login']}\" получил в подарок предмет:\"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." от персонажа \"{$rec['target_login']}\"";
		    	$delo_type[140]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_ekr']} eкр.";
		    	$delo_type[1140]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_ekr']} eкр. со счета №{$rec['bank_id']}   ";
			$delo_type[393]="\"{$rec['owner_login']}\" получил предмет:\"{$rec['item_name']}\" (x{$rec['item_count']}) шт. по акции \"{$rec['target_login']}\"";
			...
			$delo_type[185]="\"{$rec['owner_login']}\" выиграл предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. в Лотерею.";
			$delo_type[1185]="\"{$rec['owner_login']}\" выиграл предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. в Пасхальную лотерею.";
			...
			$delo_type[1206]="\"{$rec['owner_login']}\" собрал предмет {$rec['item_name']} в цветочном магазине.";
			$delo_type[202]="\"{$rec['owner_login']}\" получил {$rec['item_name']} за победу в турнире на руинах";
			$delo_type[209]="\"{$rec['owner_login']}\" получил подарок из цветочного магазина: \"{$rec['item_name']}\" ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." от персонажа \"{$rec['target_login']}\"";
			...
			$delo_type[267]="У \"{$rec['owner_login']}\" украли лошадь за городом.";
		    	...
		    	$delo_type[1280]="\"{$rec['owner_login']}\" получил возврат на сумму {$rec['sum_ekr']} екр. от Коммерческого отдела на банковский счет №{$rec['bank_id']} ";
			...
			if ($rec['type'] == 1281)
			{
				...
				if (count($t) == 3)
					{
				    	$delo_type[1281]="\"{$rec['owner_login']}\" заплатил  {$t[2]} монет за покупку личных реликтов ";
				    	}
				    	else
				    	{
				    	$delo_type[1281]="\"{$rec['owner_login']}\" заплатил  {$rec['sum_ekr']} екр. за покупку личных реликтов со счета №{$rec['bank_id']}  ";
				    	}
		    	}


		    	$delo_type[282]="\"{$rec['owner_login']}\" заплатил ваучеры за оплату услуг Коммерческого отдела";
		    	$delo_type[2282]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} екр.  за оплату услуг Коммерческого отдела со счета №{$rec['bank_id']}";
		    	if ($rec['type'] == 2284) { $t = explode("/",$rec['add_info']); $delo_type[2284]="\"{$rec['owner_login']}\" заплатил {$t[1]} монет за оплату услуг Коммерческого отдела";}
			...
			if ($rec['type'] == 1284)
			{
				...
				if (count($t) == 3)
					{
					$delo_type[1284]="\"{$rec['owner_login']}\" заплатил  {$t[2]} монет  за покупку клановых реликтов ";
					}
					else
					{
				    	$delo_type[1284]="\"{$rec['owner_login']}\" заплатил  {$rec['sum_ekr']} екр.  за покупку клановых реликтов  со счета №{$rec['bank_id']}  ";
					}
			}

		    	$delo_type[285]="\"{$rec['owner_login']}\" заплатил ваучеры за покупку поинтов для рекламы.";
		    	$delo_type[388]="\"{$rec['owner_login']}\" заплатил сертификатом за услуги Коммерческого отдела";
		    	$delo_type[389]="\"{$rec['owner_login']}\" получил сертификат (возврат) от Коммерческого отдела";

			$delo_type[306]="\"{$rec['owner_login']}\" получил при обмене предмет \"{$rec['item_name']}\" в ремотной мастерской";

			$delo_type[307]="\"{$rec['owner_login']}\" получил предмет:\"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." за участие в рейтинге \"{$rec['add_info']}\" ";
		}
		//продал
		{
			$delo_type[34]="\"{$rec['owner_login']}\" продал в магазин товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ". ($rec['sum_kr']>0?$rec['sum_kr']." кр.":"").($rec['sum_ekr']>0?$rec['sum_ekr']." екр.":"").($rec['bank_id']>0?" номер счета №:".$rec['bank_id']:"");
			$delo_type[3434]="\"{$rec['owner_login']}\" продал в Березку товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ". ($rec['sum_kr']>0?$rec['sum_kr']." кр.":"").($rec['sum_ekr']>0?$rec['sum_ekr']." екр.":"").($rec['bank_id']>0?" номер счета №:".$rec['bank_id']:"");
			$delo_type[41]="\"{$rec['owner_login']}\" продал предмет:\"{$rec['item_name']}\" ".if_have(" (x",$rec['item_count'],") ")." за {$rec['sum_kr']} кр, персонажу \"{$rec['target_login']}\" (комиссия:{$rec['sum_kom']} кр.)";
		...
		//вернул, подарил, передал, сдал (избавился)
		{
			...
			$delo_type[707]="\"{$rec['owner_login']}\" был изъят предмет: \"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." из-за бегства из Лабиринта Хаоса.";
			...
			$delo_type[6061]="\"{$rec['owner_login']}\" использовал предмет:\"{$rec['item_name']}\" у \"{$rec['target_login']}\" (неудачно).";

			$delo_type[1101]="\"{$rec['owner_login']}\" обменял 3 карты на \"{$rec['item_name']}\" (x{$rec['item_count']}) шт. ".($rec['item_count']==1?"(".$rec['item_id'].")":"")." у \"{$rec['target_login']}\"".if_have(", уплатил за обмен:",$rec['sum_kr']," кр.");
			...
			$delo_type[186]="Закончился срок аренды предмета:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}].";
			...
			$delo_type[305]="\"{$rec['owner_login']}\" обменял предмет \"{$rec['item_name']}\" в ремотной мастерской";
			...
			$delo_type[412]="У \"{$rec['owner_login']}\" получил предмет \"{$rec['item_name']}\" (".$rec['item_id'].") из ремонтной мастеркой";
        		...
        		$delo_type[414]="У \"{$rec['owner_login']}\" сдал купюры на сумму \"{$rec['sum_ekr']}\" (".$rec['item_id'].") запрос на вывод средств.".$rec['add_info'];
        		$delo_type[415]="У \"{$rec['owner_login']}\" отказ на вывод средств на сумму \"{$rec['sum_ekr']}\". Возврат купюр:".$rec['item_id'].".".$rec['add_info'];

        	}
		//забрал, вернул (себе или хозяину)
		{

			$delo_type[344]="\"{$rec['owner_login']}\" получил ваучер \"{$rec['item_name']}\" при размене.";
			$delo_type[2566]="\"{$rec['owner_login']}\" получил обратно квестовый предмет: \"{$rec['item_name']}\" от \"{$rec['target_login']}\"";
		...
		{//разные действия с предметами
			...
			$delo_type[3223]="\"{$rec['owner_login']}\" удачно использовал предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] и получил {$rec['add_info']}";
			$delo_type[33]="\"{$rec['owner_login']}\" неудачно использовал предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}]";

			$delo_type[3232]="\"{$rec['owner_login']}\" удачно  использовал личный реликт:\"{$rec['item_name']}\" ";
			...
			$delo_type[1179]="\"{$rec['owner_login']}\" зачаровал  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. свитком  {$rec['add_info']} ";
			$delo_type[1180]="\"{$rec['owner_login']}\" улучшил (  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. свитком  {$rec['add_info']} ";
			$delo_type[1181]="\"{$rec['owner_login']}\" улучшил (  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. свитком  {$rec['add_info']} ";
			$delo_type[313]="\"{$rec['owner_login']}\" поднял страницу магической книги в замке";
			$delo_type[314]="\"{$rec['owner_login']}\" получил при обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт.";
			if ($rec['type'] == 1314) {
				$t = explode(":",$rec['add_info']);
				if (count($t) == 7) {
					$delo_type[1314]="\"{$rec['owner_login']}\" получил при обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт., оплатил {$t[6]} монет";
				} else {
					$delo_type[1314]="\"{$rec['owner_login']}\" получил при обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт., оплатил {$rec['sum_ekr']} екр. со счета № {$rec['bank_id']}";
				}
			}
			if ($rec['type'] == 1315) {
				$t = explode(":",$rec['add_info']);
				if (count($t) == 5) {
					if ($rec['sum_kr'] > 0) {
						$delo_type[1315]="\"{$rec['owner_login']}\" получил при уникальном обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт., оплатил {$t[4]} монет и {$rec['sum_kr']} кр.";
					} else {
						$delo_type[1315]="\"{$rec['owner_login']}\" получил при уникальном обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт., оплатил {$t[4]} монет.";
					}
				} else {
					$delo_type[1315]="\"{$rec['owner_login']}\" получил при уникальном обмене предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт., оплатил {$rec['sum_ekr']} екр. со счета № {$rec['bank_id']} и {$rec['sum_kr']} кр.";
				}
			}


			$delo_type[315]="\"{$rec['owner_login']}\" {$rec['add_info']} на предмет  \"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. персонажу {$rec['target_login']}.";
			$delo_type[316]="Изъят \"{$rec['item_name']}\" {$rec['item_count']}шт. Администрацией и возвращено \"{$rec['add_info']}\" монет";
        	}
	    //аренда
 ...
 //Аукцион
	{
		...
		if ($rec['type'] == 222) { $t = explode("/",$rec['add_info']); $delo_type[222]="\"{$rec['owner_login']}\" выставил вещь из арсенала на аукцион: \"{$rec['item_name']}\". Сроком на ".$t[0]." дней и начальной стоимостью ".$t[1]." кр. из клана {$rec['item_arsenal']} ".(isset($t[2]) ? "Блиц цена: ".$t[2]." кр. " : "")." (снято 1 кр налога из казны)"; }
		if ($rec['type'] == 223) { $t = explode("/",$rec['add_info']); $delo_type[223]="\"{$rec['owner_login']}\" выставил вещь на аукцион: \"{$rec['item_name']}\". Сроком на ".$t[0]." дней и начальной стоимостью ".$t[1]." кр. ".(isset($t[2]) ? "Блиц цена: ".$t[2]." кр. " : "")." (снято 1 кр налога)"; }
		...
		$delo_type[238]="\"{$rec['owner_login']}\" получил возврат от Регистратуры кланов";

		$delo_type[332]="\"{$rec['owner_login']}\" поднял свою ставку {$rec['sum_rep']} реп., на аукционе за \"{$rec['item_name']}\""; // за репу
		$delo_type[352]="\"{$rec['owner_login']}\" поднял свою ставку ваучерами на сумму {$rec['sum_ekr']} екр, на аукционе за \"{$rec['item_name']}\"";

		$delo_type[331]="\"{$rec['owner_login']}\" сделал ставку {$rec['sum_rep']} реп., на аукционе за \"{$rec['item_name']}\""; //за репу
		$delo_type[351]="\"{$rec['owner_login']}\" сделал ставку  ваучерами на сумму {$rec['sum_ekr']} екр, на аукционе за \"{$rec['item_name']}\"";

		$delo_type[330]="\"{$rec['owner_login']}\" получил возврат ставки {$rec['sum_rep']} реп.,  с аукциона за \"{$rec['item_name']}\"";	// за репу
		$delo_type[350]="\"{$rec['owner_login']}\" получил возврат ставки ваучерами на сумму {$rec['sum_ekr']} екр,  с аукциона за \"{$rec['item_name']}\"";

		$delo_type[333]="\"{$rec['owner_login']}\" выкупил сразу лот \"{$rec['item_name']}\" на аукционе за {$rec['sum_rep']} реп."; //за репу

		$delo_type[334]="\"{$rec['owner_login']}\" выиграл на аукционе \"{$rec['item_name']}\" за {$rec['sum_rep']} реп."; //за репу
		$delo_type[353]="\"{$rec['owner_login']}\" выиграл на аукционе \"{$rec['item_name']}\", за {$rec['sum_ekr']} екр ваучерами.";
	...
	$delo_type[274]="\"{$rec['owner_login']}\" получил награду \"{$rec['item_name']}\" (х{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за квест \"{$rec['add_info']}\"";
	$delo_type[275]="\"{$rec['owner_login']}\" получил квестовый предмет: \"{$rec['item_name']}\" от \"{$rec['target_login']}\"";
	$delo_type[276]="У \"{$rec['owner_login']}\" квестовый предмет \"{$rec['item_name']}\" был изъят в квесте \"{$rec['add_info']}\"";
	$delo_type[277]="\"{$rec['owner_login']}\" получил квест \"{$rec['add_info']}\".";
	$delo_type[278]="\"{$rec['owner_login']}\" завершил квест \"{$rec['add_info']}\".";
	$delo_type[279]="\"{$rec['owner_login']}\" получил {$rec['sum_kr']} кр. в квесте \"{$rec['add_info']}\".";
	$delo_type[291]="\"{$rec['owner_login']}\" получил {$rec['sum_ekr']} екр. в квесте \"{$rec['add_info']}\".";
	$delo_type[292]="\"{$rec['owner_login']}\" получил {$rec['item_count']} опыта в квесте \"{$rec['add_info']}\".";
	$delo_type[293]="\"{$rec['owner_login']}\" получил {$rec['item_count']} веса для квесте \"{$rec['add_info']}\".";

	// 1300-1310 - крафт
	$delo_type[1300]="\"{$rec['owner_login']}\" запустил производство рецепта \"{$rec['item_name']}\" в количестве {$rec['item_count']} шт.";
	$delo_type[1301]="\"{$rec['owner_login']}\" отменил процесс производства.";
	$delo_type[1302]="\"{$rec['owner_login']}\" ускорил производство \"{$rec['item_name']}\" за {$rec['sum_ekr']} екр.";
	$delo_type[1303]="\"{$rec['owner_login']}\" закончил производство и получил \"{$rec['item_name']}\" в количестве {$rec['item_count']} шт.";
	$delo_type[1304]="\"{$rec['owner_login']}\" неудачно закончил производство \"{$rec['item_name']}\" в количестве {$rec['item_count']} шт.";
	$delo_type[1305]="\"{$rec['owner_login']}\" удачно собрал \"{$rec['item_name']}\" в количестве {$rec['item_count']} шт.";
	$delo_type[1306]="\"{$rec['owner_login']}\" удачно разбил предмет \"{$rec['item_name']}\" и получил предметы в количестве {$rec['item_count']} шт.";

	//Чемп мира 1340-1350
	$delo_type[1340]="\"{$rec['owner_login']}\" получил предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. ха выигрышную ставку в матче \"{$rec['add_info']}\"";
	$delo_type[1341]="Сделана ставка {$rec['add_info']}, изъят предмет «{$rec['item_name']}»";

	//Бук 1351-1375
	$delo_type[1351]="\"{$rec['owner_login']}\" получено {$rec['sum_kr']} мон. от  \"{$rec['target_login']}\".";
	$delo_type[1352]="\"{$rec['owner_login']}\" заплатил {$rec['sum_kr']} мон. к  \"{$rec['target_login']}\".";

    	//Плагин 6хх
    	$delo_type[600]="\"{$rec['add_info']}\" x\"{$rec['item_count']}\"";

	//Рейтинги 1380-1400
	$delo_type[1380]="\"{$rec['owner_login']}\" получил награду \"{$rec['item_name']}\" (х{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за рейтинг \"{$rec['add_info']}\"";
	$delo_type[1381]="\"{$rec['owner_login']}\" получил {$rec['sum_rep']} реп. за рейтинг \"{$rec['add_info']}\".";
...
if ($rec['type'] == 7777) {
  		$t = explode("/",$rec['add_info']);
			...
			if ($tname[1]!='')
					...
					$rename['Укус Гидры']='Отравление ядом';
					$rename['Обман Химеры']='Подлый удар';
					$rename['Вой Грифона']='Потрясение';
					$rename['Гнев Ареса']='Огненный ожог';
					if ($rename[$tname]!='') $tname=$rename[$tname];

			$delo_type[7777]="\"{$rec['owner_login']}\" отменил эффект \"{$tname}\" (остаток ".round($t[1]/60)." минут) ";
			}
	...
	if ($al['perevodi']>=5 && $add_info_off==2)
	{
	  ...
	  //админское время
		...
		if ($rec['type']!=19)
		{
		$out.=if_have(", использован предмет id:",$rec['aitem_id']);
		}
		
		$out.=if_have(", стоимость предмета:",$rec['item_cost']);

		if (($rec['type']==32 || $rec['type']==33) && !empty($rec['aitem_id'])) {
			$out .= if_have(", магия id:",$rec['item_id']);
		} else {
		...
		($rec['item_unic'] > 0 ? ", <font color=red>Уник: ".$rec['item_unic']."</font>" : "").
		if_have(", Привязка:",$rec['item_sowner']).
		...
		$out .= if_have(", Встройка:",$rec['item_incmagic']).
		...
		if ($rec['type'] == 509 || $rec['type'] == 599  || $rec['type'] == 3737  || $rec['type'] == 507 || ($rec['type'] == 191 && strlen($rec['add_info'])) || ($rec['type'] == 1991 && strlen($rec['add_info']) && strpos('/',$rec['add_info']) !== false)) {
			$t = explode("/",$rec['add_info']);
			if (!isset($t[1])) $t[1] = 0;
			$out .= ", монеты до:".($t[0]+$t[1])." после: ".$t[1];
		}

		if ($rec['type'] == 3001 || $rec['type'] == 3011 || $rec['type'] == 3636) {
			$t = explode("/",$rec['add_info']);
			$t[1] = intval($t[1]);
			$out .= ", монеты до:".($t[1]-$t[0])." после: ".$t[1];
		}

		if ($rec['type'] == 1314) {
			$t = explode(":",$rec['add_info']);
			if (count($t) == 7) {
				$out .= ", монеты до:".($t[5]+$t[6])." после: ".$t[5];
			}
		}
		...
		if ($rec['type'] == 2284) {
			$t = explode("/",$rec['add_info']);
			if (count($t) == 2) {
				$out .= ", монеты до:".($t[0]+$t[1])." после:".$t[0];
			}
		}
		...
		if ($rec['type'] == 1315) {
			$t = explode(":",$rec['add_info']);
			if (count($t) == 5) {
				$out .= ", монеты до:".($t[4]+$t[3])." после:".$t[3];
			}
		}
		...
		if ($rec['type'] == 1281) {
			$t = explode(":",$rec['add_info']);
			if (count($t) == 3) {
				$out .= ", монеты до:".($t[2]+$t[1])." после:".$t[1];
			}
		}
		...
		if ($rec['type'] == 508 || $rec['type'] == 506 || $rec['type'] == 519 || $rec['type'] == 521) {
			$t = explode("/",$rec['add_info']);
			$out .= ", монеты до:".($t[1])." после:".($t[0]+$t[1]);
		}


		$out .= if_have(", доп инфо:",$rec['add_info'])."   )";
