...
// Настройки
   ...
   $SECURITY_WIDTH = 240+mt_rand(0,10) ;            // Ширина изображения
   $SECURITY_HEIGHT = 66+mt_rand(0,10);            // Высота изображения
  

  $SECURITY_NUM_GENSIGN = mt_rand(3,5);        
 $LETTERS = array('0','1','2','3','4','5','6','8','9','A','B','C','D','E','F','J','H','K','L','M','N','O','P','W','R','T','Y','U','S','G','Z','X','V');
...
// Ядро
	...
	if (isset($_SESSION['EsecurityCode']) && isset($_SESSION['EsecurityCodedata'])) {
		header ("Content-type: image/gif");
		echo $_SESSION['EsecurityCodedata'];
		die();
	}

   $SECURITY_FONT_SIZE = intval($SECURITY_HEIGHT/(($SECURITY_HEIGHT/$SECURITY_WIDTH)*mt_rand(10,15)  ));
   $SECURITY_NUM_SIGN = intval(($SECURITY_WIDTH*$SECURITY_HEIGHT)/150);
   $CODE = array();

   $FIGURES = array('100' ,'107','70','90','110','130','150','115','55','75');
...
// Получаем код
   ...
   $_SESSION['EsecurityCode'] = implode('',$CODE);
...
function mv ($im,$w,$h) {
  ...
  return $im2; // возвращаем новый рисунок
   ...
   else
   {
       ...
       ob_start();
       imagegif($src);
       $_SESSION["EsecurityCodedata"] = ob_get_flush();
   }
