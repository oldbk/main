...
// переключение на инвентарь наёмника
if (isset($_GET['naeminv'])) {
	$_SESSION['naeminv'] = 1;
} elseif (isset($_GET['normalinv'])) {
	$_SESSION['naeminv'] = 0;
} else {
	if (!isset($_SESSION['naeminv'])) $_SESSION['naeminv'] = 0;
}

if (isset($_GET['bot_attack'])) // нападаение надоба из чата
	{
	$hbotid=(int)($_GET['bot_attack']);
	$hbot=mysql_fetch_array(mysql_query("select * from users_clons where id='{$hbotid}' and bot=3 and hp>0"));

		if ($hbot['id'] >0)
			{
			$_POST['target']=$hbot['login'];
			$HBOT_ATTACK=true;
...
if (($_SESSION['razdel']==6) OR ($_GET['razdel']==6))
{


				if  (isset($_GET['sub']) AND ($_GET['sub'] == 1)) {  $_SESSION['invsub6']=-1; }
				elseif  (isset($_GET['sub']) AND ($_GET['sub'] == 0)) {  $_SESSION['invsub6']=0; }
...
if (isset($_GET['bankauth'])) {
	...
	if (isset($_GET['bankpass'],$_GET['bankid'])) {
		...
		if (mysql_num_rows($q) > 0) {
			...
			echo '<script>closeinfo();bankauth = true;</script>';
			die();
		} else {
			$error = 1;
		}
	...
	$auth =  '<table border=0 width="100%" height=100><tr><td  valign=top align="center" height=5 colspan="4"><font style="COLOR:#8f0000;FONT-SIZE:12pt">';
...
if (($op!=true) and ($user['room']==17 OR $user['room']==36 OR $user['room']==54) )
	{
	can_goto_room_in_club(1);
...
/////////////////////////////////////////эмулятор инвентаря для режима турнира
	...
	?>
	<body leftmargin=0 topmargin=0 marginwidth=0 marginheight=0 bgcolor=#e2e0e0 onLoad="top.setHP(<?=$hhp?>,<?=$hhpmax?>);">
...
{
	header('Location: class_armory.php'); die();
} elseif(400 <= $user['room'] && $user['room'] <= 450 && ($app->request->isQueryEmpty() || (isset($_GET['goto']) && $_GET['goto'] == 'plo'))) {
	header('Location: /action/street/clan/location'); die();
}
...
if ( (isset($_GET['room17']) OR isset($_GET['room36']) OR isset($_GET['room54'])) AND ($op!=true) )
	{
	unset($_GET['room17']);
	unset($_GET['room36']);
	unset($_GET['room54']);
	err("Вы не можете попасть в эту комнату, когда нет дня противостояния.");
	}
...
if($_POST['gallery'] || ((int)$_GET['pic']&&(int)$_GET['dressimg']>=0) || (int)$_GET['del_pick']>0) //  Галерея
{
	$_GET['pic']=(int)($_GET['pic']);
				...
				if($_GET['pic']>0&&$_GET['dressimg']==0)
				{
				$ring_num=(int)$_POST['target'];
				if ($ring_num<=0)
						{
						$ring_num=1; //default
						}

					$piccha= mysql_fetch_assoc(mysql_query('select * from oldbk.gellery where id="'.$_GET['pic'].'" AND owner = '.$user['id'].' AND dressed > 0;'));
					if($piccha['id']==$_GET['pic'])
						{

										mysql_query('update oldbk.gellery set dressed=0 WHERE  id='.$piccha['id'].' AND owner = '.$user['id'].';');
										if (mysql_affected_rows()>0)
										{
											$mess = 'Картинка снята!';
										}
				...
				elseif($_GET['pic']>0&&$_GET['dressimg']==1&&(!(isset($_REQUEST['clearstored']))) )
				{
					...
					if ($ring_num<=0)
						...
						if ($ring_num>3) {$ring_num=3;}

					$piccha= mysql_fetch_assoc(mysql_query('select * from oldbk.gellery where id="'.$_GET['pic'].'" AND exp_date>'.time().' AND owner = '.$user['id'].' AND dressed = 0;'));
					...
					if($piccha['id']==$_GET['pic'])
					{
							...
							if ($piccha['otdel']==42)
									{
									mysql_query('update oldbk.gellery set dressed=0 WHERE otdel=42 and dressed='.$ring_num.' AND owner = '.$user['id'].';');
									mysql_query('update oldbk.gellery set dressed='.$ring_num.' WHERE id='.$piccha['id'].' AND owner = '.$user['id'].';');
									}
									elseif (($piccha['otdel']==1) OR ($piccha['otdel']==11) OR ($piccha['otdel']==12) OR ($piccha['otdel']==13) OR ($piccha['otdel']==14))
									{
									mysql_query('update oldbk.gellery set dressed=0 WHERE otdel in (1,11,12,13,14) AND owner = '.$user['id'].';');
									mysql_query('update oldbk.gellery set dressed=1 WHERE id='.$piccha['id'].' AND owner = '.$user['id'].';');
									}
									else
									{
									mysql_query('update oldbk.gellery set dressed=0 WHERE otdel='.$piccha['otdel'].' AND owner = '.$user['id'].';');
									mysql_query('update oldbk.gellery set dressed=1 WHERE id='.$piccha['id'].' AND owner = '.$user['id'].';');
									}
							$mess = 'Картинка одета.';
						}
			...
			for ($ii=0;$ii<$maxitems;$ii++)   		// делаем строки по максимальному количеству картинок
			{
				...
				foreach($colum_array as $k=>$v)
				{
					...
					if (is_array($img_array[$v]))
					{
						...
						if ($elem['otdel']==42)
						{
						echo ($elem['dressed']==0?"<div class=\"button-mid btn\" onclick=\"showitemschoice('Выберите слот для картинки кольца', 'select_ring', '?pic=".$elem['id']."&amp;dressimg=1');\">Надеть</div>".$del_link:"<div class=\"button-mid btn\" OnClick=\"location.href='?pic=".$elem['id']."&dressimg=0';\">Снять</div>")."";
						}
						else
						{
						echo ($elem['dressed']==0?"<div class=\"button-mid btn\" onclick=\"location.href='?pic=".$elem['id']."&dressimg=1';\">Надеть</div>".$del_link:"<div class=\"button-mid btn\" OnClick=\"location.href='?pic=".$elem['id']."&dressimg=0';\">Снять</div>")."";
						}
...
$security_actions = ['set_2fa', 'unset_2fa', 'prepare_2fa'];
...
if($_security_action || (@$_POST['changepsw'] || @$_POST['second_password'] || isset($_POST['unset_second_password']) || isset($_POST['btn_set_advises'])) OR ($_POST['ipsetup']) || ($_POST['setautobank']) || ($_POST['unsetbank']) || ($_POST['scansubmit'])  || isset($_FILES['upfile']))
{
	...
	<FORM ACTION="main.php" METHOD=POST>
		<table width=100%><tr><td><h3>Безопасность</h3></td><td align=right><INPUT TYPE=button value="Вернуться" onClick="location.href='main.php?edit=1&tmp=<?=mt_rand(1111,9999);?>';"></td></tr>
		</table>
				...
				<td valign="top" width="50%" valign=top>
					<fieldset style="text-align:justify;">
						...
						if ($_POST['oldpass'] && $_POST['npass'] && $_POST['npass2']) {
						    /** @var \components\models\User $UserOps */
						    $UserOps = \components\models\User::where('id', '=', $_SESSION['uid'])->first();
                            if ($UserOps->validatePassword($_POST['oldpass']) === true) {
                                if (in_smdp($_POST['npass']) === mysql_real_escape_string($user['oldpass'])) {
                                    ...
                                    if($_POST['npass'] == $_POST['npass2']) {
                                        $_new_password = \components\models\User::generatePassword($_POST['npass'], $UserOps->salt);

                                        if(mysql_query("UPDATE `users` SET `pass` = '".$_new_password."' WHERE `id` = '{$_SESSION['uid']}' LIMIT 1;"))
                                        {
                                            //установка времени обновления пароля
                                            mysql_query("INSERT oldbk.`users_pas_ch` (`owner`,`login` ,`last`) values('".$_SESSION['uid']."','".$UserOps->login."' , '".time()."') ON DUPLICATE KEY UPDATE `last`='".time()."' ; ");
...
<br>
					<fieldset style="text-align:justify;">
						...
						if(isset($_POST['second_password'])) {
							...
							echo "<font color=red><b>Ваш второй пароль: {$second_password}.</b></font>";
						...
						} else if(isset($_POST['unset_second_password'])) {
						...
						<table width="100%">
							...
							<tr>
								<td>
									<input type="submit" name="second_password" value="Установить" /> <?php if($user['second_password']) {?><input type="submit" name="unset_second_password" value="Снять второй пароль" /> <?php } ?>
                            ...
                            <tr>
                                <td>
									<?php if(!$user2fa || $user2fa->isDisabled()): ?>
                                        <input type="submit" name="prepare_2fa" value="Включить" />
									<?php else: ?>
                                        <table>
                                            <tr>
                                                <td>
                                                    ...
                                                    <input type="submit" name="unset_2fa" value="Выключить" />
                                                </td>
                                                <td>
													<?php if($user2fa->isPrepared()): ?>
                                                        <input name="2fa_code" placeholder="Введите 6 цифр кода">
                                                        <input type="submit" name="set_2fa" value="Подтвердить" />
														<?php if($_2fa_error): ?>
				...
				<td valign="top">
					<fieldset style="text-align:justify;">
						...
						echo "<legend><b>Настройки IP адреса</b></legend>
		...
		<input type=submit name=ipsetup value='Сохранить'>";
					...
					<br>
						...
						<form method="post" name="banksetauto">
							<table border=0  style="text-align:left;">
								...
								<tr><td colspan="2" align=center><input type=hidden name='unsetbank' id='unsetbank' value=''><input type='submit' class='submitinput' name='setautobank' value='ДА '> <input type='submit' class='submitinput' name='cansel' value='НЕТ'></td></tr>
							</table>
						</form>
						</div>
...
if(isset($_REQUEST['naem']) && $user['battle'] == 0) {
	if (isset($_GET['buy'])) {
		$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_id = '.intval($_GET['buy']));
		if (!mysql_num_rows($q)) {
			// покупаем наёмника
			$q = mysql_query('SELECT * FROM naem_proto WHERE is_enabled = 1 and id = '.intval($_GET['buy']));
			if (mysql_num_rows($q)) {
				...
				unset($flds["id"]);
				unset($flds["price"]);
				unset($flds["pricetype"]);
				unset($flds["img"]);
				unset($flds["is_enabled"]);

				if ($n !== false) {
					// 0 - кр, 1 - екр, 2 - репа, 3 - голда
					$pay = true;
					$bank = false;
					if ($n['pricetype'] == 0 && $user['money'] < $n['price']) {
						$msg = "Недостаточно кр для покупки наёмника";
						$pay = false;
					}

					if ($n['pricetype'] == 1) {
						if (!isset($_SESSION['bankid'])) {
							$msg = "Не выполнен вход в банковский счёт";
							$pay = false;
						}
						if (isset($_SESSION['bankid'])) {
							...
							if ($bank) {
								if ($bank['ekr'] < $n['price']) {
									$msg = "Недостаточно екр для покупки наёмника";
									$pay = false;
								}
							} else {
								$pay = false;
					...
					if ($n['pricetype'] == 2 && $user['repmoney'] < $n['price']) {
						$msg = "Недостаточно репутации покупки для покупки наёмника";
						$pay  = false;
					}
					if ($n['pricetype'] == 3 && $user['gold'] < $n['price']) {
						$msg = "Недостаточно монет для покупки наёмника";
						$pay = false;
					}

					if ($pay) {
						// сниманием бабки и добавляем наёмника
						$price = $n['price'];
						if ($n['pricetype'] == 0) {
							mysql_query("UPDATE `users` set `money` = money-'".$price."' WHERE id = ".$user['id'].' LIMIT 1') or die();
						}
						if ($n['pricetype'] == 1) {
							mysqL_query("UPDATE oldbk.`bank` set `ekr` = ekr-'".$price."' WHERE id = ".$bank['id'].' LIMIT 1') or die();
						}
						if ($n['pricetype'] == 2) {
							mysql_query("UPDATE `users` set `repmoney` = repmoney-'".$price."' WHERE id = ".$user['id'].' LIMIT 1') or die();
						}
						if ($n['pricetype'] == 3) {
							mysql_query("UPDATE `users` set `gold` = gold-'".$price."' WHERE id = ".$user['id'].' LIMIT 1') or die();
						}
						...
						$rec['target']=0;
						$rec['target_login'] = "Покупка Наёмника";
						$rec['item_name'] = $n['login'];
						$rec['type'] = 1009;
						if ($n['pricetype'] == 0) {
							$rec['sum_kr'] = $price;
							$rec['sum_ekr'] = 0;
						...
						$rec['sum_kom'] = 0;
						if ($n['pricetype'] == 1) {
							$qb = mysql_query('SELECT * FROM oldbk.bank WHERE id = '.$bank['id']) or die();
							$qb = mysql_fetch_assoc($qb);
							$rec['add_info'] = 'Баланс до '.($qb['ekr']+$price). ' после ' .$qb['ekr'];
							mysql_query("INSERT INTO `oldbk`.`bankhistory`(`date`, `text` , `bankid`) VALUES ('".time()."','Куплен наёмника за ".$price." екр.</b>, <i>(Итого: {$qb['cr']} кр., {$qb['ekr']} екр.)</i>','{$bank['id']}');") or die();
						}
						add_to_new_delo($rec) or die();

						$qs = 'INSERT INTO users_clons (exp,nextup,owner,naem_id,stats,master,';
						foreach($flds as $k => $v) {
							$qs .= $k.",";
						}
						$qs = substr($qs,0,-1);
						...
						$qs .= ') VALUES ("'.$exp.'","'.$nextup.'","'.$user['id'].'","'.$n['id'].'","'.$stats.'","'.$master.'",';
						reset($flds);
						while(list($k,$v) = each($flds)) {
							$qs .= '"'.mysql_real_escape_string($n[$k]).'",';
						}
						$qs = substr($qs,0,-1);
	                                        $qs .= ')';
						if (mysql_query($qs)) {
							mysql_query('COMMIT');
							$msg = "Удачно куплен наёмник &quot;".$n['login']."&quot;";
						} else {
							$msg = "Ошибка";
						}
					}
	...
	if (isset($_GET['activate'])) {
		// активация/дезактивация наёмника
		$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_id = '.intval($_GET['activate']));
		if (mysql_num_rows($q)) {
			$n = mysql_fetch_assoc($q);
			if ($n !== false) {
				if ($n['naem_status']) {
					mysql_query('UPDATE users_clons SET naem_status = 0 WHERE id = '.$n['id']);
					header('Location: ?naem=1'); die();
				} else {
					$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_status = 1');
					if (!mysql_num_rows($q)) {
						mysql_query('UPDATE users_clons SET naem_status = 1 WHERE id = '.$n['id']);
						header('Location: ?naem=1'); die();
					} else {
						$msg = "У вас уже есть активный наёмник";
					}
				}
			}
		}
	...
	<script>
	var bankauth = <?php echo (isset($_SESSION['bankauth']) ? "true" : "false"); ?>;
	...
	function checkbank() {
		if (bankauth) {
			...
			$.get('<?=$_SERVER['PHP_SELF']; ?>?bankauth', function(data) {
				$('#pl').html(data);
			 	$('#pl').css({ position:'absolute',left: ($(window).width()-$('#pl').outerWidth())/2, top: ($(window).scrollTop()+120)+'px'  });
				$('#pl').show(200);
	...
	function doauth() {
		...
		$.get('<?=$_SERVER['PHP_SELF']?>?bankauth=1&bankid='+bankid+'&bankpass='+bankpass, function(data) {
			$('#pl').show(200);
			$('#pl').html(data);
			$("input, select, button").bind('mousedown.ui-disableSelection selectstart.ui-disableSelection', function(e){
			    e.stopImmediatePropagation();
	...
	<div id="page-wrapper" style="text-align:center;">
		...
		<div class="btn-control" style="text-align:right;">
			...
			<div class="button-mid btn" onClick="location.href='<?=$_SERVER['PHP_SELF']?>?edit=1&naem=<? echo mt_rand(1111,9999);?>';" >Обновить</div>
			<div class="button-mid btn" onClick="location.href='<?=$_SERVER['PHP_SELF']?>?edit=1';">Вернуться</div>

		</div>


		<fieldset style="text-align:center;display: inline-block;padding: 30px;">
			<legend align="center" class="legend">НАЁМНИКИ</legend>
			<?php
			// выгребаем купленных наёмников
			$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id']);
			$nowner = [];
			...
			$q = mysql_query('SELECT * FROM naem_proto WHERE is_enabled = 1');
			while($n = mysql_fetch_assoc($q)) {
			 ?>
			<div class="naem">
				<div class="title">
					<div class="knopka_text">
						<?=$n['login']; ?>
					</div>
				</div>

				<img src="http://i.oldbk.com/i/naem/naemniki_ramka.png">
				<img class="naem_ava" src="http://i.oldbk.com/i/naem/img/<?=$n['img']; ?>">
				<div class="clear">
					<img class="knopka" src="http://i.oldbk.com/i/naem/naemniki_knopka.jpg">
					<div class="knopka_text">
						<?php
						if (!isset($nowner[$n['id']])) {
							if ($n['pricetype'] == 1) {
								echo '<a OnClick="return checkbank();"; href="?naem&buy='.$n['id'].'"><span style="color: #b12321;">купить за '.round($n['price'],2).' ';
							} else {
								echo '<a href="?naem&buy='.$n['id'].'"><span style="color: #b12321;">купить за '.round($n['price'],2).' ';
							}
							switch($n['pricetype']) {
								case 0: echo 'кр '; break;
								case 1: echo 'екр '; break;
								case 2: echo 'реп '; break;
								case 3: echo '<img src="http://i.oldbk.com/i/icon/coin_icon.png" style="margin-bottom: -5px;">'; break;
							}
							echo '</a></span>';
						} elseif ($nowner[$n['id']]['naem_status'] == 0) {
							?><a href="?naem&activate=<?=$n['id'];?>"><span style="color: #535252;">неактивен</span></a><?php
						} elseif ($nowner[$n['id']]['naem_status'] == 1) {
							?><a href="?naem&activate=<?=$n['id'];?>"><span style="color: green;">активен</span></a><?php
						}
				...
				<div class="clear">
					<img class="knopka" src="http://i.oldbk.com/i/naem/naemniki_knopka.jpg">
					<div class="knopka_text">
						<span style="color: #535252;">ХАРАКТЕРИСТИКИ</span>
					</div>
				</div>
				<div class="naemstatus">
					<?php if (!isset($nowner[$n['id']])) { ?><img src="http://i.oldbk.com/i/naem/naemniki_close.png"> <? } ?>
				</div>
			</div>
			<? } ?>
		</fieldset>
...
if($_GET['effects'])
{
	...
	<div id="page-wrapper">
		...
		if ( (($_GET['cancel']>=10901) and ($_GET['cancel']<=10904)) OR ($_GET['cancel']==7777) OR ($_GET['cancel']==920) OR ($_GET['cancel']==930) OR ($_GET['cancel']==656) OR ($_GET['cancel']==420) OR ($_GET['cancel']==150) OR ($_GET['cancel']==130) OR ($_GET['cancel']==420420) OR ($_GET['cancel']==440) )
		{
			$cncl=$_GET['cancel'];
		...
		elseif ($_GET['cancel']==555)
		{
			...
			if ($eff)
			{
				...
				if (mysql_affected_rows()>0)
				{
					mysql_query("DELETE from effects where owner = ".$user['id']." and type = '{$cncl}' and id = ".$eff['id']." LIMIT 1;");
					log_cancel_delo($user,$eff);
					$out_buff="Эффект \"".($eff['name'])."\" - Удален!<br>";
				}
			}
		}
		elseif (($_GET['cancel']==102) OR ($_GET['cancel']==160))
		{
		// отмены по опыту
			$cncl=$_GET['cancel'];
			//удаляем эффект если он есть
		...
		{
		$efid=(int)$_GET['cancel'];
		$todel_effect = mysql_fetch_array(mysql_query("SELECT * FROM `effects` WHERE `owner` = '{$user['id']}'  and `type` = '{$efid}' LIMIT 1;"));
			...
			if (mysql_affected_rows()>0)
			{
				...
				log_cancel_delo($user,$todel_effect);
			}
		}
		elseif  (($_GET['cancel']>=900) AND ($_GET['cancel']<=908) )
		{
			...
			$todel_effect_name=explode(":",$todel_effect['name']);
			if ($todel_effect_name[1]!="")
				{
				$todel_effect_name=$todel_effect_name[1];
				}
				else
				{
				$todel_effect_name=$todel_effect['name'];
				}
			...
			if ($_GET['cancel']==907)
			{
				...
				if (mysql_affected_rows()>0)
				{
				$out_buff="Эффект \"{$todel_effect_name}\" - будет удален  в течении минуты!<br>";
				log_cancel_delo($user,$todel_effect);
				}
			}
			else
			{
				mysql_query("DELETE from effects where owner='{$user['id']}' and type=".$efid." LIMIT 1;");
				if (mysql_affected_rows()>0)
				{
				$out_buff="Эффект \"{$todel_effect_name}\" - Удален!<br>";
				telepost_new($user,'<font color=red>Внимание!</font> Окончилось действие эффекта <b>«'.$todel_effect_name.'»</b>.');
				log_cancel_delo($user,$todel_effect);
				}
			}
		}elseif ($_GET['cancel']==20180601)
		{
			$efid=(int)$_GET['cancel'];
			$todel_effect = mysql_fetch_array(mysql_query("SELECT * FROM `effects` WHERE `owner` = '{$user['id']}'  and `type` = '{$efid}' LIMIT 1;"));
			$todel_effect_name=explode(":",$todel_effect['name']);
			if ($todel_effect_name[1]!="")
				...
				//делаем - апдейт времени для того чтоб крон сам снес все нужное - для возврата опыта
				mysql_query("UPDATE effects  set `time`=1 where owner='{$user['id']}' and type={$efid} LIMIT 1;");
				if (mysql_affected_rows()>0)
				{
				$out_buff="Эффект \"{$todel_effect_name}\" - будет удален  в течении минуты!<br>";
				log_cancel_delo($user,$todel_effect);
		...
		elseif 	($_GET['cancel'] == 60006000) {
			$q = mysql_query('SELECT * FROM oldbk.ny_quest_var WHERE owner = '.$user['id'].' AND var = "q1"');
			if (mysql_num_rows($q) > 0) {
				mysql_query('DELETE FROM oldbk.ny_quest_var WHERE owner = '.$user['id'].' AND var = "q1"');
			}
		}
			...
			*/
			...
			if ($user['lab']==0)
			{
				...
				if ((($labc['val']+$regulyator) > time()  ) and ($labc['val']>0))
				{
					...
					$lab_need_bat=10;
					if (($user['level']>=4) and ($user['level']<=9) ) { $lab_need_bat=0; }
            ...
            <tr class="separate">
                <td class="row-left" style="display: table-cell;">
                    <div class="separate"></div>
                </td>
                <td class="row-center" style="display: table-cell;">
                    <div class="separate"></div>
                </td>
                <td class="row-right" style="display: table-cell;">
                    <div class="separate"></div>
                </td>
            </tr>
            <tr class="element">
                <td class="row-left" style="display: table-cell;">
                    Фонтан Удачи<br><img src="http://i.oldbk.com/i/city/sub/osen_fontan.png" height="30px">
                </td>
                <td class="row-center" style="display: table-cell;">
                    Возможность испытать удачу, бросив монетку!
                </td>
                <td class="row-right" style="display: table-cell;">
                    <?= sprintf('Доступно %d\%d', $DailyFree->getAvailable(), $DailyFree->limit_uses); ?>
                    <?php
					$nextAddedTimestamp = $DailyFree->getNextAddedTimestamp();
					$start_fontan = (new DateTime())->getTimestamp();
					$end_fontan = (new DateTime())->setTime(23,59,59)->getTimestamp();

                    if($nextAddedTimestamp > 0): ?>
                        , осталось <?= \components\Helper\TimeHelper::prettyTime($start_fontan, $nextAddedTimestamp) ?>
                    <?php elseif($DailyFree->getAvailable() == 0): ?>
                        , осталось <?= \components\Helper\TimeHelper::prettyTime($start_fontan, $end_fontan) ?>
                    <?php endif; ?>
                </td>
            </tr>
     ...
     Таймер остатка времени бонуса в мм., если в остатке меньше 60 минут. Пример: Осталось 14 мин.
			*/
			...
			for($i=0;$i<count($eff);$i++)
			{
				...
				elseif($eff[$i]['type']==9100) {
					...
					echo  render_sost_row('<img src='.$ksimg.' alt=""><br>Репутация +'.$kr_a.'%','<STRONG>'.(isset($pr[2]) ? $pr[2].": " : "" ).'Добавляет +'.$kr_a.'% к получаемой репутации.</STRONG>','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин.</strong>'.$cncl_repli);
				...
				{
					$add_info='';
					$hiimg='http://i.oldbk.com/i/magic/hidden.gif';
					if ($eff[$i]['add_info']!='')
					{
					$add_info=explode("::",$eff[$i]['add_info']);
					$hiimg='http://i.oldbk.com/i/sh/'.$add_info[1];
					}

					echo  render_sost_row('<IMG height=25 src="'.$hiimg.'" width=40><br>Невидимость','<strong>'.$add_info[0].'</strong>','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong><br><a href="?hiddenoff=1&effects=1">Развеять иллюзию</a>');
				}
				 elseif ($eff[$i]['type'] == 656)
				{
					$add_info=explode("::",$eff[$i]['add_info']);
					$hiimg='http://i.oldbk.com/i/sh/'.$add_info[1];
					...
					echo  render_sost_row('<IMG height=25 src="'.$hiimg.'" width=40><br>'.$eff[$i]['name'],'<strong>'.$add_info[0].'</strong>','Осталось <strong>'.prettyTime(null,$eff[$i]['time']).'</strong>'.$offvar);
				}
				...
				} elseif ($eff[$i]['type'] == 555)
				{
				...
				} elseif ($eff[$i]['type'] == 20005) {

					$addinfo=explode(':',$eff[$i]['add_info']);

					if ($addinfo[0]=='cooldown')
						{
						echo  render_sost_row($eff[$i]['name'],'Возможно использовать.','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong>'.$offvar);
						}
						else
						{
						echo  render_sost_row($eff[$i]['name'],'Опыт +'.(int)($addinfo[0]*100).'%','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong>'.$offvar);
						}
				} elseif ($eff[$i]['type'] == 20006) {
					...
					if ($addinfo[0]=='cooldown')
						...
						{
						echo  render_sost_row($eff[$i]['name'],'Репутация +'.(int)($addinfo[0]*100).'%','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong>'.$offvar);
						}
				} elseif ($eff[$i]['type'] == 20007) {
					...
					if ($addinfo[0]=='cooldown')
						...
						{
						echo  render_sost_row($eff[$i]['name'],'Рунный опыт +'.(int)($addinfo[0]*100).'%','Осталось <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong>'.$offvar);
						}
				}
				elseif ($eff[$i]['type'] == 20) {
					...
					if ($eff[$i]['type'] == 9102)
						{
						$tempname=explode(":",$eff[$i]['name']);
						if (count($tempname)>1)
							{
							$eff[$i]['name']=(isset($tempname[2]) ? $tempname[2].": " : "").$tempname[1];
							$bonimgs[9102]=$tempname[0];
							$bonname[9102]='Опыт +'.($eff[$i]['add_info']*100).'%';
							}
					...
					if ($eff[$i]['type'] == 9103)
						...
						if (count($tempname)>1)
							{
							$eff[$i]['name']=(isset($tempname[2]) ? $tempname[2].": " : "").$tempname[1];
							$bonimgs[9103]=$tempname[0];
							$bonname[9103]='Рунный опыт +'.($eff[$i]['add_info']*100).'%';
							}
					...
					$addi=explode(":",$eff[$i]['name']);
					if ($addi[1]!='')
					{
					echo  render_sost_row('<img style="width:60px;height:60px"  src="http://i.oldbk.com/i/chm2018/'.$addi[0].'" alt="'.$addi[1].'" title="'.$addi[1].'">',$addi[1],' Осталось  <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong><br><a href="main.php?effects=1&cancel='.$eff[$i]['type'].'">Отказаться</a>');
					}
					else
					{
					echo  render_sost_row('Cупер-валентинка',' '.$eff[$i]['name'].':+'.$eff[$i]['add_info'].' %.',' Осталось  <strong>'.floor(($eff[$i]['time']-time())/60/60).' ч. '.round((($eff[$i]['time']-time())/60)-(floor(($eff[$i]['time']-time())/3600)*60)).' мин. </strong><br><a href="main.php?effects=1&cancel='.$eff[$i]['type'].'">Отказаться</a>');
		...
		<table align="center" class="table-list sostoyanie" cellspacing="0" cellpadding="0">
			...
			if (mysql_num_rows($q) > 0) {
				...
				echo render_sost_row('<img src="http://i.oldbk.com/i/fighttype7.gif" alt=""><br> <b>Ёлочное безумие</b></a>','Вам необходимо одержать победу в 25-ти елочных хаотичных боях или уровневых елочных автозаявках в полном комплекте (13 вещей, не считая рун). Бой будет засчитан, если вами будет нанесен урон. <b>('.$nyq['val'].'/25)</b> <br>Награда: 6000 репутации, Волшебная шляпа.','<a href="?effects=1&cancel=60006000">Отказаться</a>');
			}
		...
		<div data-tab-body="4" class="tab" style="display:none;">
			<TABLE class="main">
				<tr><td colspan=3>
						...
						<font color=#003388><b>Каждый персонаж, зарегистрировавшийся в ОлдБК по Вашей реферальной ссылке, по достижению им 10го уровня начнет приносить Вам еврокредиты</b></font>.<br><br>
...
if (@$_GET['setch'])
{
	...
	<body id="body" leftmargin=0 topmargin=0 marginwidth=0 marginheight=0  style="background-image:url(http://i.oldbk.com/i/fon_kartamirov.png); background-repeat:no-repeat;"   bgcolor=#e2e0e0 onLoad="top.setHP(<?=$hhp?>,<?=$hhpmax?>)">
	<!--555-->
	<?
	make_quest_div();
	?>
        ...
        <tbody>
                    ...
                    <div class="map-block">
                        ...
                        <div class="map-block-mid novi4ki">
                            <div class="map-block-head">
                                Комната для новичков <div class="head-level">(Уровень 0)</div>
				'.$active[1].'
                            </div>
                ...
                <td>
                    <div class="map-block">
                        ...
                        <div class="map-block-mid voinov">
                            <div class="map-block-head">
                                Зал воинов <div class="head-level">(Уровень 1-3)</div>
				'.$active[5].'
                            </div>
                            <div class="map-block-bottom">
            ...
            <tr>
				...
				'.$active[17].'
                            ...
                            <div class="map-block-bottom">
                                <div class="button-mid btn" name="setch" title="Войти в комнату" onClick="'.($op!=true?'alert(\'Вы не можете попасть в эту комнату, когда нет дня противостояния.\')':'location.href=\'main.php?setch=1&got=1&room17='.mt_rand(1111,9999).'\';').'">Войти</div><div class="room-amount">&nbsp;('.(int)($or[17]).')</div>
                                <div class="room-info">
				...
				'.$active[36].'
                            ...
                            <div class="map-block-bottom">
                                <div class="button-mid btn" name="setch" title="Войти в комнату" onClick="'.($op!=true?'alert(\'Вы не можете попасть в эту комнату, когда нет дня противостояния.\')':'location.href=\'main.php?setch=1&got=1&room36='.mt_rand(1111,9999).'\';').'">Войти</div><div class="room-amount">&nbsp;('.(int)($or[36]).')</div>
                                <div class="room-info">
				...
				'.$active[54].'
                            ...
                            <div class="map-block-bottom">
                                <div class="button-mid btn" name="setch" title="Войти в комнату" onClick="'.($op!=true?'alert(\'Вы не можете попасть в эту комнату, когда нет дня противостояния.\')':'location.href=\'main.php?setch=1&got=1&room54='.mt_rand(1111,9999).'\';').'">Войти</div><div class="room-amount">&nbsp;('.(int)($or[54]).')</div>
                                <div class="room-info">
...
if ((@$_GET['edit'])AND($_SESSION['boxisopen']=='open')) {
	...
	if ($_SESSION['naeminv'] && isset($_GET['up']) && !empty($_GET['up'])) {
		$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_status = 1');

		if(isset($_GET['up']) && !empty($_GET['up']) && mysql_num_rows($q)) {
			...
			if($_GET['up'] == 1) {
				...
				if ($add['sila']>0) { setup_naem_stats("sila",$add['sila'],$naem);}
				elseif  ($add['lovk']>0)  { setup_naem_stats("lovk",$add['lovk'],$naem);}
				elseif  ($add['inta']>0)  { setup_naem_stats("inta",$add['inta'],$naem);}
				elseif  ($add['vinos']>0)  { setup_naem_stats("vinos",$add['vinos'],$naem);}
				elseif  ($add['intel']>0)  { setup_naem_stats("intel",$add['intel'],$naem);}
				elseif  ($add['mudra']>0)  { setup_naem_stats("mudra",$add['mudra'],$naem);}
			}

			switch ($_GET['up']) {
				case 21 :
					if ($naem['master'] >0 && $naem['noj'] < 5)	mysql_query("UPDATE `users_clons` SET `noj` = `noj`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' AND noj < 5 LIMIT 1;");
					break;
				case 22 :
					if ($naem['master'] >0 && $naem['mec'] < 5)	mysql_query("UPDATE `users_clons` SET `mec` = `mec`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' AND mec < 5 LIMIT 1;");
					break;
				case 23 :
					if ($naem['master'] >0 && $naem['dubina'] < 5)	mysql_query("UPDATE `users_clons` SET `dubina` = `dubina`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' AND dubina < 5 LIMIT 1;");
					break;
				case 24 :
					if ($naem['master'] >0 && $naem['topor'] < 5)	mysql_query("UPDATE `users_clons` SET `topor` = `topor`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' AND topor < 5 LIMIT 1;");
					break;
				case 25 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mfire` = `mfire`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 26 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mwater` = `mwater`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 27 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mair` = `mair`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 28 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mearth` = `mearth`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 29 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mlight` = `mlight`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 210 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mgray` = `mgray`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
				case 211 :
					if ($naem['master'] >0)	mysql_query("UPDATE `users_clons` SET `mdark` = `mdark`+1, `master`=`master`-1 WHERE `id` = '{$naem['id']}' LIMIT 1;");
					break;
			}
		}
	} else {
	...
	function UnCraft($item,$rcid) {
		global $user;

		$q = mysql_query('SELECT * FROM craft_formula WHERE craftid = '.$rcid);
		if (mysql_num_rows($q) > 0) {
			$rc = mysql_fetch_assoc($q);
			$res = unserialize($rc['craftnres']);
			...
			if ($resallcount) {
			 	// посчитали количество ресурсов
				shuffle($resprotolist);
				$resallcount = floor($resallcount * 0.75);
				if ($resallcount < 1) $resallcount = 1;

				$prototogive = [];

				for ($i = 0; $i < $resallcount; $i++) {
					$r = array_pop($resprotolist);
					$prototogive[$r]++;
				}
				...
				$showlist = [];
				mysql_query('START TRANSACTION') or die();
				foreach($prototogive as $k => $v) {
					// выдаём предметы
					$dress = mysql_fetch_assoc(mysql_query('SELECT * FROM shop WHERE id = '.$k));

					$dress['present'] = "Мироздатель";
					$dress['dategoden'] = time()+(3*24*3600);
					$dress['goden'] = 3;
					...
					for ($i = 0; $i < $v; $i++) {
						$q = mysql_query("INSERT INTO oldbk.`inventory`
								(`prototype`,`sowner`,`present`,`owner`,`name`,`type`,`massa`,`cost`,`img`,`maxdur`,`isrep`,
									...
									`otdel`,`gmp`,`gmeshok`, `group`,`letter`,`rareitem`,`stbonus`,`mfbonus`,`unik`,`notsell`,`craftspeedup`,`craftbonus`
									)
									VALUES
									...
									,'{$dress['razdel']}','{$dress['gmp']}','{$dress['gmeshok']}','{$dress['group']}','{$dress['letter']}','{$dress['rareitem']}','{$dress['stbonus']}','{$dress['mfbonus']}','{$dress['unik']}','{$dress['notsell']}','{$dress['craftspeedup']}','{$dress['craftbonus']}')
						") or die();
					}
					$ids[$k] = "cap".mysql_insert_id()."/".$dress["name"]."/x".$v;
					$idstxt .= '&quot;'.link_for_item($dress).'&quot; '.$v.' шт.,';

					$showlist[] = ['name' => $dress['name'], 'img' => $dress['img'], 'count' => $v, 'link' => link_for_item($dress,true)];
				}

				$_SESSION['uncraftitems'] = $showlist;
				...
				$rec['owner_balans_posle'] = $user['money'];
				$rec['target']=0;
				$rec['target_login']="Разбитие предмета";
				$rec['type']=1306;
				$rec['sum_kr']=0;
				$rec['sum_ekr']=0;
				$rec['sum_kom']=0;
				$rec['item_id']=implode(",",$ids);
				$rec['aitem_id']=get_item_fid($item);
				$rec['item_name']=$item['name'];
				$rec['item_count']=$resallcount;
				$rec['item_type']=$item['type'];
				$rec['item_cost']=$item['cost'];
				$rec['item_dur']=$item['duration'];
				$rec['item_maxdur']=$item['maxdur'];
				$rec['add_info'] = $item['name'];
				add_to_new_delo($rec) or die();

				addchp ('<font color=red>Внимание!</font> Удачно разбито &quot;'.link_for_item($item).'&quot;, вы получили '.substr($idstxt,0,-1),'{[]}'.$user['login'].'{[]}',-1,$user['id_city']) or die();

				mysql_query('DELETE FROM inventory WHERE id = '.$item['id']) or die();
	...
	if (isset($_GET['uncraft'])) {
		$q = mysql_query('SELECT * FROM inventory WHERE owner = '.$user['id'].' and setsale = 0 and dressed = 0 and naem > 0 AND bs_owner = 0 and id = '.intval($_GET['uncraft']));
		if (mysql_num_rows($q)) {
			$row = mysql_fetch_assoc($q);
			if ($row && strlen($row['craftedby'])) {
				$t = explode("#",$row['craftedby']);
				if (count($t) == 2) {
					$t = explode("|",$t[0]);
					if (isset($t[5])) {
						$t[5] = intval($t[5]);
	...
	elseif (isset($_GET['drop']))
	{
				...
				if ($_SESSION['naeminv'] && $user['in_tower'] == 0) {
					$q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_status = 1');

					if(mysql_num_rows($q)) {
						$naem = mysql_fetch_assoc($q);
						dropitemnaem($naem,$_GET['drop']);
					}
				} else {
					dropitem($_GET['drop']);
				}
	}
	elseif (isset($_GET['dress']))
	{
						if ($user['ruines'] > 0) {
					...
					if ($_SESSION['naeminv'] && $user['in_tower'] == 0) {
							...
							if(mysql_num_rows($q)) {
								$naem = mysql_fetch_assoc($q);
								dressitemnaem($naem,$_GET['dress']);
							}
						} else {
							dressitem($_GET['dress']);
						}
	}
	elseif(isset($_GET['dressall']))
	{

	if ($user['level'] > 3)
		{
			$count_free=0;
			for ($i = 1; $i <= 10; $i++) //станадртные слоты
				{
					if ($user['m'.$i]==0) { $count_free++; }
				}

			$repcount = (int)$user['rep']/ 20000; //дополнительные слоты за репу 11-15
			if($repcount < 0) $repcount = 0;
			if($repcount > 5) $repcount = 5;
			for($i = 1; $i <= $repcount; $i++)
			{
			if ($user['m1'.$i]==0) { $count_free++; }
			}

			for ($i = 16; $i <= getMaximumSlot($user['prem']); $i++) //доп слоты за прем. акк 16-20
			{
			if ($user['m'.$i]==0) { $count_free++; }
			}
				...
				if ($count_free>0)
				{
						$dress_all_proto=(int)($_GET['dressall']);
						$get_dress_all=mysql_query("SELECT id from oldbk.inventory where owner='{$user['id']}' and prototype='{$dress_all_proto}' and type=12 and dressed=0 and setsale=0 limit {$count_free} ");
						while ($todress=mysql_fetch_array($get_dress_all))
									{
									dressitem($todress['id']);
									}
				}
				else
				{
					$noty_mesg='Нет свободных слотов!';
					$typet='e';
				}
		}
	}
	elseif (isset($_GET['pocket']))
	{
		$in_out = (int)$_GET['pocket'];
		$item = (int)$_GET['item'];
		$result = put_into_pocket($in_out, $item);
	}
	elseif (isset($_GET['pocketall']))
	{
		$put_proto=(int)$_GET['pocketall'];
		mysql_query("UPDATE oldbk.`inventory` SET `karman` = 1 WHERE `prototype` = {$put_proto}  and  owner={$user['id']} and dressed=0 and setsale=0;");

		if (mysql_affected_rows()>0)
			{
			$noty_mesg='Предметы перенесены в карман.';
			}
	}
	elseif (isset($_GET['getpocketall']))
	{
		$get_proto=(int)$_GET['getpocketall'];
		mysql_query("UPDATE oldbk.`inventory` SET `karman` = 0 WHERE `prototype` = {$get_proto}  and  owner={$user['id']} and dressed=0 and setsale=0;");
		if (mysql_affected_rows()>0)
			{
			$noty_mesg='Предметы перенесены из кармана.';
			}
	}
	elseif ($_GET['setup'])  //start edit bf fred for setup stats
	{
	...
	$shmot=array(1,2,3,4,5,8,9,10,11,22,27,28,30,34,35);
			...
			$sk='SELECT * FROM oldbk.inventory WHERE 	`owner` = '.$user['id'].' AND `prototype` = '.$del_proto.' AND `dressed` = 0 AND `setsale` = 0 AND present != "Арендная лавка" AND bs_owner='.$user['in_tower'];
			$cc=0;
			$drop=mysql_query($sk);

			//имена полей для копирования в таблик удаления
			$copy_sql_f='INSERT INTO oldbk.`inventory_aftr_del`(`deltime`, ';
			for($i=0;$i<mysql_num_fields($drop);$i++)
			{
				$fields[$i]=mysql_field_name($drop, $i);
				$copy_sql_f.='`' . mysql_field_name($drop, $i) . '`, ';
			}
			$copy_sql_f=substr($copy_sql_f,0,-2). ') VALUES ("'.time().'", ';
			...
			$copy_sql='';
				...
				$issart = 0;
				if ((($row['ab_uron'] > 0 || $row['ab_bron'] > 0 || $row['ab_mf'] > 0 || $row['art_param'] != "")  AND $row['type'] != 30) || ($row['type'] == 30 && $row['up_level'] > 5) || $row['prototype'] == 3003092 || $row['prototype'] == 3003093 || $row['prototype'] == 111000 || $row['prototype'] == 509 || $row['prototype'] == 3003227 || $row['prototype'] == 12802 || $row['prototype'] == 3003226 || $row['prototype'] == 3003228)  { 	$issart = 1; 	}

				if (!$issart || ($issart && ($user['in_tower'] > 0 || $row['labonly'] > 0))) { 			} else {		continue; 	}

				$delo_str.=get_item_fid($row).',';

				if ($cc==0)
				{
				$item['id']=get_item_fid($row);
				$item['name']=$row['name'];
				$item['type']=$row['type'];
				$item['proto']=$row['prototype'];
				$item['cost']=$row['cost'];
				$item['ecost']=$row['ecost'];
				$item['dur']=$row['duration'];
				$item['maxdur']=$row['maxdur'];
				$item['ups']=$row['ups'];
				$item['unic']=$row['unic'];
				$item['incmagic']=$row['includemagicname'];
				$item['incmagic_id']=$row['includemagic'];
				$item['incmagic_count']=$row['includemagicuses'];
				$dress_bs_owner=$row['bs_owner'];
				$dress_labonly=$row['labonly'];
				}

				$del_sql.=$row['id'].', ';
				$cc++;

				if((in_array($row['type'],$shmot) || $row['prototype'] == 3003092 || $row['prototype'] == 3003093) && $row['bs_owner']==0 && $row['labonly'] == 0)
				{
				//готовим для копирования в таблик удаления

						for($iii=0;$iii<count($fields);$iii++)
						    {
						     $copy_sql.=" '".$row[$iii]."', ";
						    }
						      $copy_sql=substr($copy_sql,0,-2). ');';
									if ($copy_sql!='')
									{
						      mysql_query($copy_sql_f.$copy_sql);
									}
									else
									{
									//echo "NE";
			...
			if (strlen($del_sql))
			{
				...
				if(mysql_query('DELETE FROM oldbk.inventory WHERE id in ('.$del_sql.')'))
				{
					if (($dress_bs_owner==1 || $dress_bs_owner==2 || $dress_bs_owner==3 ||	$dress_labonly==1))
					{
					...
					else
					{
						...
						$rec['aitem_id']=$delo_str;
						$rec['item_name']=$item['name'];
						$rec['item_count']=$cc;
						$rec['item_type']=$item['type'];
						$rec['item_proto']=$item['proto'];
						$rec['item_cost']=$item['cost'];
						...
						$rec['item_incmagic_id']=$item['incmagic_id'];
						$rec['item_incmagic_count']=$item['includemagicuses'];
						$rec['item_arsenal']='';
						add_to_new_delo($rec); //

						$noty_mesg='Успешно выброшено: '.$item['name'].' в кол-ве '.$cc.' шт.';
					}
				}
			}
			else
			{
			//echo "NO ITEMS FROM DELETE";
			}
	}
	elseif (isset($_GET['destruct'])) //single item drop
	{
		$_GET['destruct']=(int)$_GET['destruct'];
			...
			if($dress['id'] && $okk)
			{
				...
				if($dress['arsenal_klan'] == '' && $okk) {
					...
					if($dress['present'] == 'Арендная лавка')
					{
						$noty_mesg="<b>Этот предмет нельзя выбросить.</b>";
						$typet='e';
					}
					elseif ($okk==1)
					{
						//echo "<font color=red><b>Предмет \"".$dress['name']."\" выброшен.</b></font>";
						$noty_mesg="<b>Предмет \"".$dress['name']."\" выброшен.</b>";
					}
				}
				else
				{
					$noty_mesg="<b>Нельзя выкинуть вещь из Арсенала Клана!</b>";
					$typet='e';
				}
			}
	...
	<HTML><HEAD>
		...
		<script>
			...
			function showhiddeninv(proto,id,otdel) {
				...
				// ajax load
				$.ajax({
					url: "<?=$_SERVER['PHP_SELF']?>?invload2=1&prototype="+proto+"&id="+id+"&otdel="+otdel,
					cache: false,
					async: true,
		...
		<TABLE border=0 width=100% cellspacing="0" cellpadding="0">
			<TR>
				<td valign=top align=left width=250><?php
					...
					if ($user['in_tower'] == 0) {
	                                        $q = mysql_query('SELECT * FROM users_clons WHERE owner = '.$user['id'].' and naem_status = 1');
						if ($_SESSION['naeminv'] && mysql_num_rows($q)) {
							$isnaem = true;
							$naem = mysql_fetch_assoc($q);
					...
					$part2 = "<div id='uncraftitems' style='z-index: 300; position: absolute; left: 155px; top: 120px;width:600px; background-color: #eeeeee; border: 1px solid black; display: none;'></div><center>";
					$part2 .= "<a href='".$_SERVER['PHP_SELF']."?edit=1&undress=all'>Снять все</a><BR>";

					if ($user['in_tower'] != 3 && !$isnaem) {
					...
					if (!$isnaem) {
						...
						$part2 .= '</form>';
						$part2 .= '</td></tr><tr><td>&nbsp;</td><td></td></tr></table>';
					}

					$part2 .= '</td><TD valign=top width=207>';
					...
					if (!$isnaem) {
						...
						if($user['uclass']>0) {
							...
							$sub_class_name='Антикрит';
							if ($USER_MF_DATA['mfakrit']<$USER_MF_DATA['mfauvorot']) {
							$sub_class_name='Антиуворот';
							}
							$part2 .= 'Подкласс: <b>'.$sub_class_name.'</b><BR>' ;
						}
					}
					...
					$master = get_mast_wt($WEP_TYPE);
					if ($WEP_TYPE2!='') { $master2 = get_mast_wt($WEP_TYPE2); }
					...
					if (!$isnaem) {
						...
						if ($WEP_TYPE2!='')
							{
						$min_damage2 = round((floor($user['sila']/3) + 1) + $user_level + $USER_MF_DATA['min_u2'] * (1 + 0.07 * $master2));
						$max_damage2 = round((floor($user['sila']/3) + 4) + $user_level + $USER_MF_DATA['max_u2'] * (1 + 0.07 * $master2));
							}
						...
						if ($prof_data['smithlevel']>0)	{
							...
							if ($min_damage2>0) {$min_damage2 += (int)($prof_data['smithlevel']*1) ; }
							if ($max_damage2>0) { $max_damage2 += (int)($prof_data['smithlevel']*2) ; }
						...
						if ($USER_MF_DATA['ab_uron']>0 || isset($aeff[792])) {
							...
							if($USER_MF_DATA['ab_uron']>0) {
								...
								if ($min_damage2>0) { $min_damage2+=(int)($min_damage2*($USER_MF_DATA['ab_uron']/100)); }
								if ($max_damage2>0) { $max_damage2+=(int)($max_damage2*($USER_MF_DATA['ab_uron']/100)); }
							...
							if (isset($aeff[792])) {
								...
								if ($min_damage2>0) { $min_damage2+=(int)($min_damage2*(5/100)); }
								if ($max_damage2>0) { $max_damage2+=(int)($max_damage2*(5/100)); }
						...
						$min_damage = round((floor($user['sila']/3) + 1) + $user_level + $user['sum_minu'] * (1 + 0.07 * $master));
						$max_damage = round((floor($user['sila']/3) + 4) + $user_level + $user['sum_maxu'] * (1 + 0.07 * $master));
						$gree_out_uron = "";
						$arrmf['uvorota'] = $user['sum_mfuvorot']+($user['lovk']*5);
						$arrmf['auvorota'] = $user['sum_mfauvorot']+($user['lovk']*5)+($user['inta']*2);
						$arrmf['krita'] = $user['sum_mfkrit']+($user['inta']*5);
						$arrmf['akrita'] = $user['sum_mfakrit']+($user['inta']*5)+($user['lovk']*2);

						$USER_MF_DATA['bron1'] = $user['sum_bron1'];
						$USER_MF_DATA['bron2'] = $user['sum_bron2'];
						$USER_MF_DATA['bron3'] = $user['sum_bron3'];
						$USER_MF_DATA['bron4'] = $user['sum_bron4'];

					}


					$part2 .= "Урон: ".$min_damage." - ".$max_damage.$gree_out_uron;

					if (($min_damage2>0) and ($max_damage2>0))
						{
						$part2 .= "<br>Доп. урон: ".$min_damage2." - ".$max_damage2.$gree_out_uron;
						}

					$part2 .="<br>Модификаторы<br>";
	...
	<INPUT class="button-mid btn" TYPE="button" onclick="location.href=\'?edit=1&naem=1\'" value="Наёмники" title="Наёмники">
	...
	<INPUT class="button-mid btn" TYPE="button" onClick="location.href=\'main.php\';" value="Вернуться" title="Вернуться">
					...
					if ($isnaem)
					{
					if (isset($_SESSION['naem_razdel'])) { 	$_SESSION['razdel']=$_SESSION['naem_razdel']; }


					if ((isset($_GET['razdel'])) AND ($_GET['razdel']==101 OR $_GET['razdel']==102 OR $_GET['razdel']==0))
						{
						$_SESSION['razdel'] = (int)$_GET['razdel'];
						$_SESSION['naem_razdel']=$_SESSION['razdel'];
						}
						else
						if (!(($_SESSION['razdel']==0) OR ($_SESSION['razdel']==101) OR ($_SESSION['razdel']==102)))
						{
						$_SESSION['razdel'] =0;
						}
					}
					...
					{
					if (isset($_SESSION['user_razdel'])) { 	$_SESSION['razdel']=$_SESSION['user_razdel']; }

					if ($_SESSION['razdel']>100) { $_SESSION['razdel'] = 0; }
					$_SESSION['razdel'] = isset($_GET['razdel']) ? max(0, min(7, intval($_GET['razdel']))) : (isset($_SESSION['razdel']) ? $_SESSION['razdel'] : 0);

					$_SESSION['user_razdel']=$_SESSION['razdel'];
					}
		...
		';
				...
				<TD align=center bgcolor="'.(($_SESSION['razdel'] === 6 && ((isset($_GET['sub']) && $_GET['sub'] == 1) OR ($_SESSION['invsub6']==-1) )   ) ? "#A5A5A5":"#C7C7C7").'"><A HREF="?edit=1&razdel=6&sub=1">&nbsp;Квестовое</A></TD>
				<TD align=center bgcolor="'.(($_SESSION['razdel'] === 6 && ((isset($_GET['sub']) && $_GET['sub'] == 0) OR ($_SESSION['invsub6']==0) )) ? "#A5A5A5":"#C7C7C7").'"><A HREF="?edit=1&razdel=6&sub=0">&nbsp;Ресурсы</A></TD>
		...
		';
					...
					{
					$part2 .= '	<TD align=center bgcolor="'.(($_SESSION['razdel'] === 101) ? "#A5A5A5":"#C7C7C7").'"><A HREF="?edit=1&razdel=101">&nbsp;Боевые приемы</A></TD>
							<TD align=center bgcolor="'.(($_SESSION['razdel'] === 102) ? "#A5A5A5":"#C7C7C7").'"><A HREF="?edit=1&razdel=102">&nbsp;Пассивные умения</A></TD>';
					}
					$part2 .= '
	...
	';
					...
					$_SESSION['cache_inv_massa']="Рюкзак (Вес: ".$my_massa."/<span id=maxinvmassa>".get_meshok()."</span>)</B>";
					...
					switch($_SESSION['razdel']) {
						...
						break;
						case 2: //pro4ee
							$where = "AND `type` > 12 AND `type` NOT IN (34,35,99,200,555,556) AND `type` !=27 AND `type` !=28 AND `type` !=30 AND `type` !=33 and otdel != 62 AND `type` !=77  AND ( (`prototype` < 3001 or `prototype` > 3030) and (`prototype` < 103001 or `prototype` > 103030) and (`prototype` < 15551 or `prototype` > 15568)) ";
						...
						default: //abmundir
							if ($filt==4) {
								...
								$where = "AND (`type` < 12 OR `type`=27 OR `type`=28  OR `type`=555 OR `type`=556 OR  `type`=30 OR  `type`=33 OR  `type`=34  OR  `type`=35  ) ";
							}
					...
					if($grrr == 1) {
						if ($isnaem) {
							$sql = "SELECT *, count(*) as `itemscount` FROM (SELECT *, IF (dategoden = 0, 2052691200, dategoden) as dategoden2 FROM oldbk.`inventory` WHERE `owner` = '".$_SESSION['uid']."' ".$where." AND naem = ".$naem['naem_id']." AND dressed = 0 AND `setsale` = 0 AND bs_owner = ".$user['in_tower']." ORDER by `dategoden2` ASC,`update` DESC) as `inv` GROUP BY `prototype` ORDER BY ".$podarok_order." `update` DESC";
						} else {
							$sql = "SELECT *, count(*) as `itemscount` FROM (SELECT *, IF (dategoden = 0, 2052691200, dategoden) as dategoden2 FROM oldbk.`inventory` WHERE `owner` = '".$_SESSION['uid']."' ".$where." AND dressed = 0 AND `setsale` = 0 AND bs_owner = ".$user['in_tower']." ORDER by `dategoden2` ASC,`update` DESC) as `inv` GROUP BY `prototype` ORDER BY ".$podarok_order." `update` DESC";
						}
					} else {
						if ($isnaem) {
							$sql = "SELECT * FROM oldbk.`inventory` WHERE `owner` = '".$_SESSION['uid']."' ".$where." AND naem = ".$naem['naem_id']." AND dressed = 0 AND `setsale` = 0 AND bs_owner = ".$user['in_tower']." ".$giftfilter." ORDER by ".$podarok_order." `update` DESC";
						} else {
							$sql = "SELECT * FROM oldbk.`inventory` WHERE `owner` = '".$_SESSION['uid']."' ".$where." AND dressed = 0 AND `setsale` = 0 AND bs_owner = ".$user['in_tower']." ".$giftfilter." ORDER by ".$podarok_order." `update` DESC";
						}
					}

					if ($_SESSION['razdel'] != 4 && $_SESSION['razdel'] != 6 && $_SESSION['razdel'] != 7 && $_SESSION['razdel'] != 101 && $_SESSION['razdel'] != 102 )  {
			...
			<TR><TD align=center><!--Рюкзак-->
					...
					<TABLE BORDER=0 WIDTH=100% CELLSPACING="0" CELLPADDING="1" BGCOLOR="#A5A5A5">
						...
						else if ($_SESSION['razdel'] == 101)
						{
						//бОЕВЫЕ
						require_once('abiltxt.php');
						$nv[1]='power';
						$nv[2]='chance';

								if ($naem['skills']!='')
									{
										$skill=unserialize($naem['skills']);


										if (($naem['skills_point']>0) AND  isset($_GET['skill']) AND  isset($skill[$_GET['skill']]) and isset($_GET['upp']) and isset($nv[$_GET['upp']])  )
											{
											if (($nv[$_GET['upp']]=='chance') AND ($skill[$_GET['skill']][$nv[$_GET['upp']]]>=50) )
												{
												//максумум 50% шанса
												}
											elseif (($nv[$_GET['upp']]=='power') AND ($skill[$_GET['skill']][$nv[$_GET['upp']]]>=100) )
												{
												//максумум 100% мощи
												}
												else
												{
												$skill[$_GET['skill']][$nv[$_GET['upp']]]+=$skill_templ[$_GET['skill']]['chance_by_point'];

												mysql_query("UPDATE `users_clons` SET `skills_point`=`skills_point`-1, skills='".serialize($skill)."'  WHERE `id`='{$naem['id']}' and owner='{$user['id']}' limit 1;");
												$naem['skills_point']-=1;
												}
											}
											...
											foreach($skill as $ss => $skd)
											{
											echo '<tr><td bgcolor="#C7C7C7" colspan=2 align="center"><hr></td></tr>';
											echo '<tr><td bgcolor="#C7C7C7" height=30 align="center">';
											echo '<img src="http://i.oldbk.com/i/naem/active_skill_'.$skd['id'].'.gif" alt="'.$skd['name'].'" title="'.$skd['name'].'">';
											echo '</td>';
											echo '<td bgcolor="#C7C7C7" align="left" valign="top">&nbsp;&nbsp;&nbsp;'.$atext[$skd['id']].'<br>';
											echo "<br>• Мощность:{$skd['power']}%";
											if ($naem['skills_point']>0 and ($skd['power']<100) ) { echo  "<a href='?skill={$skd['id']}&upp=1&edit=1'><img src='http://i.oldbk.com/i/up.gif' alt=\"Увеличить\" title=\"Увеличить\"></a>";}
											echo "<br>• Вероятность:{$skd['chance']}%";
											if (($naem['skills_point']>0) and ($skd['chance']<50)) { echo  "<a href='?skill={$skd['id']}&upp=2&edit=1'><img src='http://i.oldbk.com/i/up.gif' alt=\"Увеличить\" title=\"Увеличить\"></a>";}

											echo '</td></tr>';
											}
											echo '<tr><td bgcolor="#C7C7C7" colspan=2 align="center"><hr>';
												if ($naem['skills_point']>0)
												{

												echo "<font color=\"#333399\">Возможных увеличений: {$naem['skills_point']}</font>";
												}
											echo '</td></tr>';
								...
								if ($naem['passkills']!='')
									{
										...
										if (($naem['passkills_points']>0) AND  isset($_GET['skill']) AND  isset($paskill[$_GET['skill']]) and isset($_GET['upp']) and isset($nv[$_GET['upp']])  )
										{
										//прокачка пассивного скила
											if ($paskill[$_GET['skill']]['id']==20001)
											{
											//нет разкачки
											}
											else
											if (($nv[$_GET['upp']]=='procent') AND ($paskill[$_GET['skill']][$nv[$_GET['upp']]] < 5*($paskill[$_GET['skill']]['by_point']) ) )
												{
												$paskill[$_GET['skill']][$nv[$_GET['upp']]]+=$paskill[$_GET['skill']]['by_point'];
												mysql_query("UPDATE `users_clons` SET `passkills_points`=`passkills_points`-1, passkills='".serialize($paskill)."'  WHERE `id`='{$naem['id']}' and owner='{$user['id']}' limit 1;");
												$naem['passkills_points']-=1;

												if ($paskill[$_GET['skill']]['id']==20002)
													{
													$_SESSION['active_passkill_massa']+=$paskill[$_GET['skill']]['by_point'];
													echo "<script>document.getElementById('maxinvmassa').innerHTML=parseInt(document.getElementById('maxinvmassa').innerHTML)+{$paskill[$_GET['skill']]['by_point']} ;</script>";
													}
										...
										if (isset($_GET['skill']) AND isset($paskill[$_GET['skill']]) and isset($_GET['active']) )
											{
											//вкл/выкл
												if ( (($paskill[$_GET['skill']]['active']==0) AND ($_GET['active']==1) ) OR (($paskill[$_GET['skill']]['active']==1) AND ($_GET['active']==0)) )
												{
												$to_active=true;

													if ($paskill[$_GET['skill']]['cooldown']>0)
													{
													//есть кулдаун проверяем
													$get_eff_coold=mysql_fetch_array(mysql_query("select * from effects where owner='{$user['id']}' and type='{$paskill[$_GET['skill']]['id']}'"));
													if (($get_eff_coold['id']>0))
														{
														$to_active=false;
														}
													}
													...
													if ($to_active==true)
													{
														...
														if (($paskill[$_GET['skill']]['time']>0) and ($paskill[$_GET['skill']]['active']==0) and ($paskill[$_GET['skill']]['procent']>0) )
														{
														//есть время для эффекта и нет кулдауна вешаем эффект и идет активация

															$add_time_eff=time()+($paskill[$_GET['skill']]['time']*60*60); //2ч
//															$addinfo_kof=round($paskill[$_GET['skill']]['bylevel']*$naem['level']*0.01,2);
															//$addinfo_kof=round(2*$naem['level']*0.01,2);
															$addinfo_kof=round($paskill[$_GET['skill']]['procent']*0.01,2); // новая формула только проценты

															mysql_query("INSERT INTO `effects` SET `type`= '{$paskill[$_GET['skill']]['id']}',`name`='{$paskill[$_GET['skill']]['name']}',`time`='{$add_time_eff}',`owner`='{$user[id]}', add_info='{$addinfo_kof}:{$paskill[$_GET['skill']]['cooldown']}:{$naem['id']}' ;");


															if ($paskill[$_GET['skill']]['id']==20005)
																{
																mysql_query("UPDATE users set expbonus=expbonus+'{$addinfo_kof}' where id='{$user[id]}' ; ");
																}
															elseif ($paskill[$_GET['skill']]['id']==20006)
																{
																mysql_query("UPDATE users set rep_bonus=rep_bonus+'{$addinfo_kof}' where id='{$user[id]}' ; ");
																}

														$paskill[$_GET['skill']]['active']=$paskill[$_GET['skill']]['active']==0?1:0;

														}

														if ($paskill[$_GET['skill']]['id']==20004)
														{
															if ($_GET['active']==1)
																{
														...
														if ($paskill[$_GET['skill']]['id']==20003)
														{
															if ($_GET['active']==1)
																{
														...
														if (($paskill[$_GET['skill']]['id']==20002) and ($paskill[$_GET['skill']]['procent']>0) )
														{

															if ($_GET['active']==1)
																{
																$_SESSION['active_passkill_massa']=round($paskill[$_GET['skill']]['procent']);

																echo "<script>document.getElementById('maxinvmassa').innerHTML=parseInt(document.getElementById('maxinvmassa').innerHTML)+{$_SESSION['active_passkill_massa']} ;</script>";
																}
																else
																{
																echo "<script>document.getElementById('maxinvmassa').innerHTML=parseInt(document.getElementById('maxinvmassa').innerHTML)-{$_SESSION['active_passkill_massa']} ;</script>";
													...
													mysql_query("UPDATE `users_clons` SET passkills='".serialize($paskill)."'  WHERE `id`='{$naem['id']}' and owner='{$user['id']}' limit 1;");
													}
												}
											}


											foreach($paskill as $ss => $skd)
											{
												echo '<tr><td bgcolor="#C7C7C7" colspan=2 align="center"><hr></td></tr>';
												echo '<tr><td bgcolor="#C7C7C7" height=30 align="center">';
												...
												echo '<a href="?skill='.$skd['id'].'&active='.$sactive.'&edit=1"><img '.$active.' src="http://i.oldbk.com/i/naem/pass_skill_'.$skd['id'].'.gif" alt="'.$skd['name'].'" title="'.$skd['name'].'"></a>';
												...
												if ($sactive==0)
													{
													echo "<br>• Состояние: <b>Активен</b>";
													}
													else
													{
													echo "<br>• Состояние: <b>Не активен</b>";
													}



												if ((($skd['by_point']>0) and ($skd['bylevel']==0)) OR ($skd['id']==20001) )
												{
												if ($skd['id']==20001)
													 {
														 /* нет разброса */
	 													echo "<br>• Всего:".round($naem['level'])." %";
													 }
													 else
													 {
														echo "<br>• Всего:".(int)$skd['procent']." %";
														if ($naem['passkills_points']>0 and ($skd['procent']<5*($skd['by_point'])) )
														{
														echo  "<a href='?skill={$skd['id']}&upp=1&edit=1'><img src='http://i.oldbk.com/i/up.gif' alt=\"Увеличить\" title=\"Увеличить\"></a>";
														}
													 }

												}
												elseif (($skd['by_point']>0) and ($skd['bylevel']>0))
												{
												echo "<br>• Всего:".(int)$skd['procent']." ед.";
												if ($naem['passkills_points']>0 and ($skd['procent']< 5*($skd['by_point']) ) ) { echo  "<a href='?skill={$skd['id']}&upp=1&edit=1'><img src='http://i.oldbk.com/i/up.gif' alt=\"Увеличить\" title=\"Увеличить\"></a>";}
												}


												if ($skd['time']>0)
												{
												echo "<br>• Время действия:{$skd['time']} ч.";
												}

												if ($skd['cooldown']>0)
												{
												echo "<br>• Возможность активации каждые:{$skd['cooldown']} ч.";
												}
											...
											echo '<tr><td bgcolor="#C7C7C7" colspan=2 align="center"><hr>';
												...
												if ($naem['passkills_points']>0)
												{
												echo "<font color=\"#333399\">Возможных увеличений: {$naem['passkills_points']}</font>";
												}
								...
								foreach ($inv_shmot as $key => $row) {
									if ((($_SESSION['allp'] != 1) and ($displc >= $_GET['page']*$_SESSION['lim']) AND ($displc < $_GET['page']*$_SESSION['lim']+$_SESSION['lim'])) || $_SESSION['allp'] == 1) {
										...
										if ($row['itemscount'] == 1) {
											...
											$ret .= "<tr BGCOLOR='".$color."' ><td WIDTH=150>";
											$ret .= '<div  id=txt_'.$row['prototype'].' style="display: block; text-align: center;">';
											...
											$ret .= '<div  id="txt1_'.$row['prototype'].'" style="display: none; text-align: center;">';
											$ret .= "<a href=\"#".$row['prototype']."\" Onclick=\"closehiddeninv(".$row['prototype'].");\">скрыть</a></div>";

											$ret .= "</td><td>";

											if (($row['type']==12)&&($can_dress_item==true))
													{
													$ret.= "  <span><a href=\"?edit=1&dressall={$row['prototype']}\">надеть все</a></span> | ";
													}

											 if ((int)$row['karman']==0)
												 	{
													$ret .= "<sapn><a href=\"?edit=1&pocketall={$row['prototype']}\">положить все</a></span>";
													}
													else
													{
													$ret .= "<span><a href=\"?edit=1&getpocketall={$row['prototype']}\">достать все</a></span>";
													}

											if ($can_drop_item==true)
											{
												$ret .= " | <span><a style=\"float: right;\" OnClick=\"if (!confirm('Вы уверены, что хотите выкинуть ".($row['name'])." в количестве ".($row['itemscount'])." шт?')) { return false; } \" href='?edit=1&delall=".$row['prototype']."'>удалить все</span>";
											}
...
//echo $displc  ;
	...
	if (isset($_SESSION['uncraftitems'])) {
		?>
		<script>
		obj = $("#uncraftitems");
		ht = '<table border=0 width=600 height=100><tr><td valign=top align="center" height=5><font style="COLOR:#8f0000;FONT-SIZE:12pt"><div style="width:25px;height:2px;position:relative;top:-15px;right:-300px;"><a onClick=\'$("#uncraftitems").hide();\' title="Закрыть" style="cursor: pointer;"><img src="http://i.oldbk.com/i/bank/bclose.png" border=0 title="Закрыть"></a></div>';
		ht += 'Вы разбили предмет и получили</font><br><br></td></tr>';
		<?php
		foreach($_SESSION['uncraftitems'] as $k => $v) {
			echo "ht += '<tr><td align=\"center\"><a href=\"http://oldbk.com/encicl/".addslashes($v['link']).".html\" target=\"_blank\"><img align=\"middle\" title=\"".$v['name']."\" alt=\"".$v['name']."\" src=\"http://i.oldbk.com/i/sh/".$v['img']."\"></a> <b> x ".$v['count']."</b></td></tr>';";
		}
		?>
		ht += '</table><br><br>';
		obj.html(ht);
	 	obj.css({ position:'absolute',left: ($(window).width()-$('#uncraftitems').outerWidth())/2, top: ($(window).scrollTop()+120)+'px'  });
		obj.show(200);
		</script>
		<?php

		unset($_SESSION['uncraftitems']);
	}
