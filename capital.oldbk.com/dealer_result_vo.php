...
class wmMerchantOldbk{
	var $course = array('WMZ'=>1);
	var $comission = array('WMZ'=>10);

	//var $myPurses = array('WMZ'=>'Z132092281531');
	//private $merchantkey = '94kdjf90834jfmrcxrt4w0xzx32x43r';

	var $myPurses = array('WMZ'=>'Z755383101103');
	private $merchantkey = 'ASs12fUDfd175WdfSDHFadfssdpo';	


	//var $myPurses = array('WMZ'=>'Z206515604818');
	//private $merchantkey = 'DsrnjhYtesKetq56Yf[eq';
	...
	function checkpost(){
		...
		addchp ('<font color=red>Внимание!</font>Debug1: '.$traderid,'{[]}Bred{[]}');
	...
	global $akks_types,$vaucher,$new_akks_prise, $larec_param,$artup_param , $artup_prise, $artup_name, $exprun_param,$exprun_prise,$exprun_name,$larec_prise,$larec_type,$larec_name, $podar , $podarvl,$podar23, $podar8,$podarny_img,$podarvl_img,$podar23_img,$podar8_img,$podar_prise,$podar_ekr,$bukets,$bukets_prise,$leto_bukets,$leto_bukets_prise;
	...
	$sub_trx=(int)($indata[4]);
		...
		else if (($owner>0) and ($param==81) and ($bankid >0)) //покупка коробок лорда
		{
			$q = $this->query("SELECT * FROM oldbk.`bank` WHERE `id` = '$bankid' and `owner` = '$owner' LIMIT 1;");
			if($s = mysql_fetch_array($q))
			{
		...
		else if (($owner>0) and ($param==91  or $param==51 or $param==52 or $param==53 ) and ($bankid >0)) //покупка свитков
		{
			$q = $this->query("SELECT * FROM oldbk.`bank` WHERE `id` = '$bankid' and `owner` = '$owner' LIMIT 1;");
			if($s = mysql_fetch_array($q))
			{
	...
	*/
	...
	include "ny_events.php";
		...
		$amountUsd = isset($this->course[$currency])?$amount*$this->course[$currency]:0;
		$comis = isset($this->comission[$currency])?$this->comission[$currency]:0;
		$balamount = $amount;
		addchp ('<font color=red>Внимание!</font>Debug2: (WMZ) sum:'.$balamount,'{[]}Bred{[]}');
		//if ($traderid==3154)
		//{
		//$balamount = ($balamount*0.13)+$balamount;	   //сумма / 90 * 10 + сумма = сумма /90*100 ??????
		//}
		//else
		if ($traderid > 0 )
		{
		...
		*/
		...
		if ($traderid > 0)
				...
				$balamount=$balamount/$usd_kurs; // сколько екр
			...
			//пишем в дилдело
				...
				mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,bank,owner,ekr) values ('wmz','8383','Банкир','$bankid','{$tonick['login']}','{$balamount}');");
			...
			mysql_query("INSERT INTO oldbk.`bankhistory`(`date`, `text` , `bankid`) VALUES ('".time()."','Вы купили <b>{$balamount} екр.</b> за WMZ','{$bankid}');");
			...
			mysql_query("INSERT INTO oldbk.`bankhistory`(`date`, `text` , `bankid`) VALUES ('".time()."','Бонус за покупку еврокредитов <b>{$ko_bonus} екр.</b> ','{$bankid}');");
					 ...
					 // Партнерка
					CheckRealPartners($tonick['id'],$amount,$_SESSION['uid']);
					 $p_user = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`partners_users` WHERE `id` = '{$tonick['id']}' LIMIT 1;"));
				       ...
				       {
				       	 mysql_query("INSERT INTO oldbk.`partners_delo` (dealer_id,partner_id,owner_id,ekr,bank,transfer_time) values ('{$_SESSION['uid']}','{$partner['id']}','{$tonick['id']}','{$amount}','{$bank['id']}','".time()."');");
				      	 $bonus=round(($amount/100*$partner['percent']),2);
				      	 mysql_query("UPDATE oldbk.`partners` set `all_ekr` = `all_ekr`+'{$amount}', `money`=`money`+'{$bonus}' WHERE `id` = '{$partner['id']}' LIMIT 1;");
				      }
				...
				/// проверить соответсвие стоимости которая подтверждена от платежки и стоимости предмета
				...
				$test_ekr=number_format($get_test_vau[ecost],2,'.','');

				 if ($test_ekr==$balamount)
				 	{
					$q = $this->query("select id from oldbk.trader_balance WHERE pid= '$pid' AND bankid = $bankid AND method='$method'");
					if(mysql_num_rows($q) == 0)
					{
					...
					{
					mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ стоимость предмета != стоимости оплаты: Оплачено {$balamount} / должно {$test_ekr}, Param {$param} , UserID {$get_bankid[owner]} ');");
					}
				}
				...
				$test_ekr=number_format($new_akks_prise[$param][$sub_type],2,'.','');
				 ...
				 if ($test_ekr==$balamount)
						...
						addchp ('<font color=red>Внимание!</font>Debug2 wmz: PREM EXP:'.$exp,'{[]}Bred{[]}');
							if ($exp>0)
							{
							$REZ=by_prem_from_bank($tonick,$param,$bank,$exp,$dill,$sub_type);
							addchp ('<font color=red>Внимание!</font>Debug2 wmz: PREM REZ:'.$REZ,'{[]}Bred{[]}');
						...
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMR Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка установки премиум акк(1). ');");
				 		...
				 		{
					addchp ('<font color=red>Внимание!</font>WMZ ERROR AKK: '.$traderid,'{[]}Bred{[]}');
				 		}
				...
				mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,bank,owner,ekr) values ('wmz','8383','Банкир','0','{$tonick['login']}','{$balamount}');");
				mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$balamount}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира
				...
				if  ($sum_ekr>=$DOLG_EKR)
					{
					///типа все окей
					$mess = "Досрочно выпущен из хаоса, долг погашен через КО. {ee}WMZ{ee}.";
					mysql_query("INSERT INTO oldbk.`lichka`(`id`,`pers`,`text`,`date`) VALUES ('','".$tonick['id']."','".$mess."','".time()."');");

					mysql_query("update {$dbc}users set align = 0, palcom = 0 where id='".$tonick[id]."' ");
					...
					telepost_new($tonick,'<font color=red>Внимание!</font> Частично погашен долг за хаос '.($balamount).' екр. ');
					//запись в дело
											//запись в дело!
											$rec['owner']=$tonick[id];
											$rec['owner_login']=$tonick[login];
											...
											$rec['item_arsenal']='';
											$rec['bank_id']=0;
											$rec['add_info']='WMZ (частичное погашение) ';
											add_to_new_delo($rec); //юзеру
				...
				if ($kkazna)
				{
					...
					if ((time()>$KO_start_time) and (time()<$KO_fin_time))
					{
					$add_klan_kazna_ekr=round(($klan_kazna_ekr*(1+$KO_A_BONUS)) ,2); //+%
				...
				if (put_to_kazna($klan_name[id],2,$add_klan_kazna_ekr,$klan_name[short],$tonick,$coment='Пополнение через WMZ, от персонажа '.$tonick[login]))
			   	{
				$fc_nom=100000000+$klan_name[id];
				$fc_name='Клан-Казна:«'.$klan_name[short].'»';
				mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,bank,owner,ekr) values ('wmz','8383','Банкир','{$fc_nom}','".$fc_name."','{$balamount}');");
				mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$balamount}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира
				...
				telepost_new($tonick,'<font color=red>Внимание!</font> Пополнен баланс клановой казны на '.($add_klan_kazna_ekr).' екр. ');
		   			...
		   			{
					addchp ('<font color=red>Внимание!</font>Пополнение казны, пришла оплата WMZ, но ошибка зачсления, teloid:'.$tonick[id],'{[]}Bred{[]}',-1,-1);
		   			}
				}
			else
				{
				addchp ('<font color=red>Внимание!</font>Пополнение казны, пришла оплата WMZ, но чел не имееет/заморожена казны, teloid:'.$tonick[id],'{[]}Bred{[]}',-1,-1);
				}
			...
			//give_bonus_meshok($tonick,$balamount);
			  ...
			  //покупка репутации
				...
				{
						mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr) values ('wmz','8383','Банкир','300','{$tonick[login]}','{$balamount}');");
   						mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$balamount}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира

					      // Партнерка
						CheckRealPartners($tonick['id'],$amount,0);
					     $p_user = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`partners_users` WHERE `id` = '{$tonick['id']}' LIMIT 1;"));
					     $partner = mysql_fetch_array(mysql_query("SELECT * FROM oldbk.`partners` WHERE `id` = '{$p_user['partner']}' LIMIT 1;"));
					     if ($p_user['partner']!='' and $p_user['fraud']!=1)
					      {
					       mysql_query("INSERT INTO oldbk.`partners_delo` (dealer_id,partner_id,owner_id,ekr,bank,transfer_time) values ('8383','{$partner['id']}','{$tonick['id']}','{$amount}','0','".time()."');");
			  ...
			  else if ( ($owner>0) and ($param==2018||($param==2118 and ($sub_trx>=1 and $sub_trx<=5))) )
			  {
				...
				if (mysql_affected_rows()>0)
				{
					...
					{
					mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr) values ('wmz','8383','Банкир','{$param}','{$tonick[login]}','{$balamount}');");
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
					}
				}
				...
				if ($good)
					{
					mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,bank,owner,ekr,addition) values ('wmz','8383','Банкир','0','{$tonick['login']}','{$bilprice}','33333');");
				...
				/// проверить соответсвие стоимости которая подтверждена от платежки и стоимости предмета
				/// если все ок то продолжаем - если нет то пишем мне сообщение
				$test_ekr=number_format($larec_prise[$param],2,'.','');
				 if ($test_ekr==$balamount)
				{
						...
						if (mysql_affected_rows() > 0) {
							...
							$goden = round(($goden_do-time())/60/60/24); if ($goden<1) {$goden=1;}
								...
								$laprice=$larec_prise[$param];
									...
									mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,bank,owner,ekr,addition) values ('wmz','8383','Банкир','0','{$tonick['login']}','$laprice','2013');");
			  ...
			  {
			  //  работа сброса WMZ кошелька при выводе
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
				$balamount = number_format($balamount,2,'.','');
  			  ...
  			  else if (($owner>0) and ($param==98)) //покупка чаши триумфа
			  {
				...
				for ($i=0;$i<$kol;$i++)
					...
					{
						mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr) values ('wmz','8383','Банкир','{$fid}','{$tonick[login]}','{$dress['ecost']}');");
					...
					if ($werrcount>0)
						{
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка покупки чаш werrcount:{$werrcount} . ');");
						}
					//give_bonus_meshok($tonick,$balamount);
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
				}
  			  ...
  			  else if (($owner>0) and ($param==60))
			  {
													     ...
													     if ($p_user['partner']!='' and $p_user['fraud']!=1)
													      {
													       ...
													       mysql_query("UPDATE oldbk.`partners` set `all_ekr` = `all_ekr`+'{$balamount}', `money`=`money`+'{$bonus}' WHERE `id` = '{$partner['id']}' LIMIT 1;");
																			...
																			$rec['sum_kom']=0;
																			$rec['item_id']='';
																			$rec['add_info'] = iconv("CP1251","CP1251",'Оплата счета №'.$get_sub_trx['id'].' - '.$ger_par_info['par_serv_desc'].',  через WMZ '.$balamount);
																			add_to_new_delo($rec);
				...
				{
				addchp ('<font color=red>Внимание!</font>'.$param.': (WMZ) add ekr:'.$balamount,'{[]}Bred{[]}');
						...
						if ($tonick['id']>0)
							...
							mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','".iconv("CP1251","CP1251",'Банкир')."','{$fid}','{$tonick[login]}','{$balamount}','{$bankid}' );");
							mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$balamount}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира
							     ...
							     if ($p_user['partner']!='' and $p_user['fraud']!=1)
							      {
							       ...
							       mysql_query("UPDATE oldbk.`partners` set `all_ekr` = `all_ekr`+'{$balamount}', `money`=`money`+'{$bonus}' WHERE `id` = '{$partner['id']}' LIMIT 1;");
													...
													$rec['sum_kom']=0;
													$rec['item_id']='';
													$rec['add_info'] = 'Оплата лечения травмы WMZ '.$balamount;
													add_to_new_delo($rec);
				   ...
				   else if (($owner>0) and ($param==88100||$param==88000||$param==88500||$param==881000))
					//обработка  покупка пакетов монет 100-500-1000
					  {
			  			include "config_ko.php";
						$tonick = check_users_city_data($owner);
						$currency = mysql_escape_string($currency);
						...
						if (($tonick['id']) and  (round($amount,2)>=round($eprice,2) ) and $kol>0)
							{
							$q = $this->query("select id from oldbk.trader_balance WHERE pid= '$pid' AND traderid='$owner'  AND method='$method'");
							if(mysql_num_rows($q) == 0)
							{
							...
							return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
								...
								{
								mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Warning!</font> wmz:{$amount} Param {$param} , UserID {$tonick[id]}, werrcount:{$werrcount} . ');");
								}
			...
			mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Warning!</font> WMR: Pay:{$amount} Param {$param} , UserID {$tonick[id]}, new-align start ');");
				...
				if (($tonick['id']>0) and  ($amount>=$ali_cos)  )
					{
					$q = $this->query("select id from oldbk.trader_balance WHERE pid= '$pid' AND traderid='$owner'  AND method='$method'");
					if(mysql_num_rows($q) == 0)
					{
					...
					$ins_id=mysql_insert_id();
							...
							if (mysql_affected_rows()>0)
								{
									//лог
									mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','".iconv("CP1251","CP1251",'Банкир')."','{$sklonka}','{$tonick[login]}','{$ali_cos}','{$bankid}' );");
									//баланс
			   ...
			   else if (($owner>0) and (in_array($param,$leto_bukets)) and ($bankid>0) )   //цветочки
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
				...
				$k=0;
					...
					{
					$k++;
						mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','Банкир','{$fid}','{$tonick[login]}','{$dress['ecost']}','{$bankid}');");
						mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$dress['ecost']}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира
					}
						else
						{
				...
				telepost_new($tonick,'<font color=red>Внимание!</font> Вам передан предмет <b>'.$dress[name].'</b> (x'.$k.'). Спасибо за покупку!');
					...
					if ($werrcount>0)
						{
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка покупки букета  werrcount:{$werrcount} . ');");
						}
					//give_bonus_meshok($tonick,$balamount);
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
				}
			   ...
			   else if (($owner>0) and ($param==94) and ($bankid>0)) //покупка
			  {
				...
				telepost_new($tonick,'<font color=red>Внимание!</font> Вам передан предмет <b>'.$dress[name].'</b> (x'.$k.'). Спасибо за покупку!');
					...
					if ($werrcount>0)
						{
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка покупки тыквы  werrcount:{$werrcount} . ');");
						}
					//give_bonus_meshok($tonick,$balamount);
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
				}
			...
			else if (($owner>0) and ($param==90||$param==89) and ($bankid>0)) //покупка  пропусков
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
				...
				for ($i=0;$i<$kol;$i++)
					...
					{
					$k++;
						mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','Банкир','{$fid}','{$tonick[login]}','{$prise}','{$bankid}');");
				...
				telepost_new($tonick,'<font color=red>Внимание!</font> Вам передан предмет <b>'.$dress[name].'</b> (x'.$k.'). Спасибо за покупку!');
					...
					if ($werrcount>0)
						{
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка покупки   werrcount:{$werrcount} . ');");
						}
					//give_bonus_meshok($tonick,$balamount);
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
				}
			...
			else if (($owner>0) and ($param==81) and ($bankid>0)) //покупка валентинки
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
			...
			else if (($owner>0) and ($param==91) and ($bankid>0)) //покупка валентинки
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
			...
			else if (($owner>0) and ($param==51 or $param==52 or $param==53 ) and ($bankid>0)) //покупка
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
			  ...
			  else if (($owner>0) and ($param==92) and ($bankid>0)) //покупка валентинки
			  {
				$tonick = check_users_city_data($owner);
				$currency = mysql_escape_string($currency);
				$desc = mysql_escape_string(htmlspecialchars($desc));
				...
				for ($i=0;$i<$kol;$i++)
					...
					{
					$k++;
						mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','Банкир','555','{$tonick[login]}','{$dress['ecost']}','{$bankid}');");
						mysql_query("UPDATE oldbk.`dealers` set `sumekr` = sumekr-'{$dress['ecost']}' WHERE `owner` = '8383' LIMIT 1;"); //  снимаем  из баланса  Банкира
				...
				telepost_new($tonick,'<font color=red>Внимание!</font> Вам передан предмет <b>'.$dress[name].'</b> (x'.$k.'). Спасибо за покупку!');
					...
					if ($werrcount>0)
						{
						mysql_query("INSERT INTO oldbk.`telegraph` (`owner`,`date`,`text`) values ('14897','','<font color=red>Внимание!</font> WMZ Оплачено {$amount} Param {$param} , UserID {$tonick[id]}, ошибка покупки снежинки  werrcount:{$werrcount} . ');");
						}
					//give_bonus_meshok($tonick,$balamount);
					return array('status'=>1,'id'=>$ins_id,'desc'=>'Платеж успешно добавлен');
				}
  			  ...
  			  else if (($owner>0) and ($param==95) and ($bankid>0)) //покупка снежинки
			  {
				...
				$bank=mysql_fetch_array(mysql_query("select * from oldbk.bank where id='{$bankid}' and owner='{$owner}'  LIMIT 1;"));
				 //$bank = mysql_fetch_array(mysql_query("select * from oldbk.bank where owner='{$owner}' order by def desc,id limit 1"));
				...
				$kol=round($balamount/$prise);
										...
										$dress=mysql_fetch_array(mysql_query("select * from oldbk.eshop where id='{$param}' ;"));
											...
											if (mysql_affected_rows()>0)
												...
												mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','Банкир','55','{$tonick[login]}','{$prise}','{$bankid}');");
			...
			if (mysql_affected_rows()>0)
				{
					...
					mysql_query("INSERT INTO oldbk.`dilerdelo` (paysyst,dilerid,dilername,addition,owner,ekr,bank) values ('wmz','8383','Банкир','2011','{$tonick[login]}','{$prise}','{$bankid}');");
