...
//в наградах отключена репа за победу
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
foreach ($otwin as $k =>$wrow)
	{
			...
			if (($repa > 0)and ($expa >0 ))
				...
				/*			
				       ...
				       */
				     	 mysql_query("UPDATE ".$bci."`users` SET `exp`=`exp`+".$expa."  WHERE `id`='".$winrow['id']."' LIMIT 1; ");				  
...
//	        		addchp ('<font color=red>Поздравляем!</font> Вы получили <b>'.$addrepa.'</b> репутации и  <b>'.$expa.'</b> опыта за победу в турнире!','{[]}'.$realwin['login'].'{[]}',$realwin['room'],$realwin['id_city']);
	        		addchp ('<font color=red>Поздравляем!</font> Вы получили <b>'.$expa.'</b> опыта за победу в турнире!','{[]}'.$realwin['login'].'{[]}',$realwin['room'],$realwin['id_city']);
