...
$VER='6.2';
//старт мега боя - запуск по расписанию! из крона для 2-х стороннего боя
     ...
     while($al = mysql_fetch_array($get_all_align)) 
     	{
	     	...
	     	if (($align==3) OR ($align==6)) // OR ($align==2))
	     	{
	     	$count[$align]+=$al[kol];
	     	}
     	}
...
if ($zayava[active]==1) //заявка активная
 ...
 echo "Заявка активна \n";
   ...
   ///
    ...
    {
	 echo "Заявка winN - никто не пришел  \n";    
    winN();
    }
    else
    if (($zayava[team1]=='')and($zayava[team2]!='') and ($zayava[team3]==''))
	{
	 echo "Заявка winT - никто не пришел за свет \n";    	
	winT();
	}
    else
    if (($zayava[team1]!='')and($zayava[team2]=='') and ($zayava[team3]==''))
	{
	 echo "Заявка winS - никто не пришел за тьму \n";    		
	winS();
	}
    else
    if (($zayava[team3]!='')and($zayava[team2]=='') and ($zayava[team1]==''))
	{
	 echo "Заявка winNETRAL - никто не пришел за тьму и за свет\n";    			
	winNETRAL();
	}	
    else
    {
    
   	 echo "Заявка собралась делаем бой! \n";    			
    //все есть
  ...
  //// можем стартовать бой  
     		{
				...
				mysql_query_100("update users  set `battle`='{$id}', `battle_t`=`bpstor`, bpzay=0 where bpzay>0 AND room=60 AND ldate >= (".(time()-60).") AND bpstor>0 AND hidden=0 AND battle=0 AND zayavka=0 AND hp>1 AND sila>0 AND lovk>0 AND inta>0;");
...
//				addlog($id,"Часы показывали <span class=date>".date("Y.m.d H.i")."</span>, когда Свет, Тьма и Нейтралы бросили вызов друг другу. \n");
				addlog($id,"Часы показывали <span class=date>".date("Y.m.d H.i")."</span>, когда Свет, Тьма бросили вызов друг другу. \n");
   ...
   else if (($zayava[sys_mess_10m]==0) and (($zayava['start']-600) <= $t) )
   {
	...
	$TEXT='<font color=red>Внимание!</font> На <a href=http://oldbk.com/encicl/arena.html target=_blank>Арене Богов</a> открыт прием заявок на бой "'.$zayava[coment].'", который начнется через 10 минут! Награда за победу: 250% опыта, 250% РК. Награда за поражение: 140% опыта, 250% РК. Ограничение участников для каждой стороны: '.$maxusers;
	addch2levels($TEXT,$zayava[t1min],$zayava[t1max],$bot_city);
   }         
   else if (($zayava[sys_mess_5min]==0) and (($zayava['start']-300) <= $t) )
   {
 	mysql_query("UPDATE `place_zay` SET `sys_mess_5min`=1 WHERE id='".$zayava[id]."' ; ");
 	
 	$cc=mysql_fetch_array(mysql_query("select * from place_battle where var='maxusers'"));
	$maxusers=$cc['val'];
 	
	$TEXT='<font color=red>Внимание!</font> На <a href=http://oldbk.com/encicl/arena.html target=_blank>Арене Богов</a> открыт прием заявок на бой "'.$zayava[coment].'", который начнется через 5 минут! Награда за победу: 250% опыта, 250% РК. Награда за поражение: 140% опыта, 250% РК. Ограничение участников для каждой стороны: '.$maxusers;
	addch2levels($TEXT,$zayava[t1min],$zayava[t1max],$bot_city);
   }
...
//проверяем не порали ее активировать
   if ( ($zayava[id]>0) and (($zayava['start']-3600) <= $t) ) // за час открываем
   {
				...
				if ($maxusers==1000)
				{
				mysql_query("UPDATE `place_battle` SET `val`='".($zayava['start']+10800)."' WHERE `var`='close';");   //close 3 ч  http://tickets.oldbk.com/issue/oldbk-2605

				$TEXT='<font color=red>Внимание!</font> На <a href=http://oldbk.com/encicl/arena.html target=_blank>Арене Богов</a> открыт прием заявок на бой "'.$zayava[coment].'", который начнется через 1 час! Награда за победу: 250% опыта, 250% РК. Награда за поражение: 140% опыта, 250% РК. Ограничение участников для каждой стороны: 1000';
				//$TEXT='<font color=red>Внимание!</font> На "Арене Богов" через 1 час состоится бой "'.$zayava[coment].'", открыт прием заявок на бой! ';
				}
				else
				{
				mysql_query("UPDATE `place_battle` SET `val`='".($zayava['start']+10800)."' WHERE `var`='close';");   //close 3 ч  http://tickets.oldbk.com/issue/oldbk-2605
				$TEXT='<font color=red>Внимание!</font> На <a href=http://oldbk.com/encicl/arena.html target=_blank>Арене Богов</a> открыт прием заявок на бой "'.$zayava[coment].'", который начнется через 1 час! Награда за победу: 250% опыта, 250% РК. Награда за поражение: 140% опыта, 250% РК. Ограничение участников для каждой стороны: '.$maxusers;
				//$TEXT='<font color=red>Внимание!</font> На "Арене Богов" через 1 час состоится бой "'.$zayava[coment].'", открыт прием заявок на бой! Максимальное количество участников для каждой стороны:'.$maxusers;
				}

				addch2levels($TEXT,$zayava[t1min],$zayava[t1max],$bot_city);				
   }
      else if ( ($zayava[id]>0) and ($zayava[sys_mess_4h]==0) and (($zayava['start']-14400) <= $t) )
   {
   
			 	mysql_query("UPDATE `place_zay` SET `sys_mess_4h`=1 WHERE id='".$zayava[id]."' ; ");
			 	
				$TEXT='<font color=red>Внимание!</font> На <a href=http://oldbk.com/encicl/arena.html target=_blank>Арене Богов</a> через 4 часа состоится бой "'.$zayava[coment].'"! Награда за победу: 250% опыта, 250% РК. Награда за поражение: 140% опыта, 250% РК.';
