...
if( !lockCreate("cron_locbat_job") ) {
	...
	//для 2-х стороннего боя - лидеры + очередб V6
...
return $cc[0];
  ...
  echo "//бой идет  <br>";
	...
	if (($kand[battle]==0) and ($kand[hp] > 2) and  ($ALL_IN_T1<$MAX_USERS_LIM) and ($kand[weap]>0 and $kand[kulon]>0 and $kand[sergi]>0 and $kand[perchi]>0 and $kand[bron]>0 and $kand[rubashka]>0 and $kand[helm]>0 and $kand[shit]>0 and $kand[boots]>0 and $kand[r1]>0 and $kand[r2]>0 and $kand[r3]>0) ) 		
	  {
...
//				addlog($batl[val],"!:V:".time().":".nick_new_in_battle($kand).":".$ac."\n");				
	        ...
	        {
	        	if (!(($kand[weap]>0 and $kand[kulon]>0 and $kand[sergi]>0 and $kand[perchi]>0 and $kand[bron]>0 and $kand[rubashka]>0 and $kand[helm]>0 and $kand[shit]>0 and $kand[boots]>0 and $kand[r1]>0 and $kand[r2]>0 and $kand[r3]>0)))
	        	{
			telepost_new($kand,'<font color=red>Внимание!</font> Нахождение в очереди возможно только при надетых 13 вещах, не включая руны! Вы были автоматически удалены из очереди!');
	        	}
	 ...
	 if ( ($t2life < $maxp ) and ($t2life>0))
	 {
	  // можно пустить в бой
		 ...
		 if ($turn[owner] > 0 )
			...
			if (($kand[battle]==0) and ($kand[hp] > 2) and  ($ALL_IN_T2<$MAX_USERS_LIM) and ($kand[weap]>0 and $kand[kulon]>0 and $kand[sergi]>0 and $kand[perchi]>0 and $kand[bron]>0 and $kand[rubashka]>0 and $kand[helm]>0 and $kand[shit]>0 and $kand[boots]>0 and $kand[r1]>0 and $kand[r2]>0 and $kand[r3]>0) ) 	
			      {				
					if(mysql_query('UPDATE `battle` SET to1='.$time.', to2='.$time.', to3='.$time.', `t'.$ttt.'`=CONCAT(`t'.$ttt.'`,\';'.$turn[owner].'\') , `t'.$ttt.'hist`=CONCAT(`t'.$ttt.'hist`,\''.BNewHist($kand).'\')   WHERE  `win`=3 and `status`=0 and `id` = '.$batl[val].' ;'))
					{
