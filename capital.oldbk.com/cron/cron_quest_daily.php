...
try {
    ...
    $UserQuest = UserQuest::from('user_quest as uq')
		...
		->whereRaw('ql.is_deleted = 0 and ql.quest_type = "daily"')
    ...
    $db->rollBack();
    if($ex->getMessage() != '') {
        echo 'Error. See log file cron_quest_daily.txt'.PHP_EOL;
        \components\Helper\FileHelper::writeException($ex, 'cron_quest_daily');
    }
}
