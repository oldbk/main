...
////
		 ...
		 //2.2 проверка на окончание боя
			...
			if ($BSTAT[win]==1)
			 ...
			 //mysql_query("delete from `users_clons` where `battle`={$data_battle[id]} ; ");
				...
				/* if ($data_battle[type]==100)
	          	{
	                //$t1=mysql_fetch_array(mysql_query('select id from users where battle='.$data_battle[id].' AND battle_t=1 '));
	                //$t2=mysql_fetch_array(mysql_query('select id from users where battle='.$data_battle[id].' AND battle_t=2 '));
	                	$t1=explode(";",$data_battle[t1]);
             	        $t2=explode(";",$data_battle[t2]);
	                // echo '1 t1, t2:' . $t1[0]. ' '. $t2[0]. '<br>';
	                    addchp ('<font color=red>Внимание победа тимы 1! bot 1 </font>тимы t1, t2: ' . $t1[0]. ' || '. $t2[0].' бой'.$data_battle[id],'{[]}A-Tech{[]}');
	                	$tt2=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t2[0].');'));
						$tt1=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t1[0].');'));
                	// echo '1 kl1, kl2:' . $tt1[0]. ' '. $tt2[0].'<br>';
					   addchp ('<font color=red>Внимание!2</font>после первого запроса ID кланов' . $tt1[0]. '||'. $tt2[0].' ','{[]}A-Tech{[]}');

						$t1_klan=mysql_fetch_array(mysql_query('select '.$tt1[id].' as my_clan, cw.* from clans_war as cw where
						((defender = '.$tt1[id].' AND agressor ='.$tt2[id].') or
						 (defender = '.$tt2[id].' AND agressor ='.$tt1[id].')) AND osnova = 1 AND `date`>='.time().' AND id= '.$data_battle[war_id].';'));
                    	//echo mysql_error();
                        if($t1_klan[my_clan]==$t1_klan[agressor]) { $win=' `win1`=`win1`+1 '; }  else { $win=' `win2`=`win2`+1 '; }
                    	 addchp ('<font color=red>Внимание!3</font> кому плюс '.$win. ' айди войны:' . $t1_klan[id]. '  ','{[]}A-Tech{[]}');
                    	//echo $win. ' t1:' .$t1_klan[id];
						$sql='update clans_war set '.$win.' where `date` >= "'.time().'" AND id="'.$t1_klan[id].'";';
						addchp ('<font color=red>Внимание!4</font> ' . $sql.' ','{[]}A-Tech{[]}');
						mysql_query($sql);
				}    */
				...
				if ($data_battle[type]==100)
	          	{
		                ...
		                //$t2=mysql_fetch_array(mysql_query('select id from users where battle='.$data_battle[id].' AND battle_t=2 '));
	       	    			$t1=explode(";",$data_battle[t1]);
	             	        $t2=explode(";",$data_battle[t2]);
		                // echo '2 t1, t2: ' . $t1[0]. ' '. $t2[0] .'<br>';

	                    addchp ('<font color=red>Внимание победа тимы 2! bot 2 </font>тимы t1, t2: ' . $t1[0]. ' || '. $t2[0].' бой'.$data_battle[id],'{[]}A-Tech{[]}');
		                $tt2=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t2[0].');'));
						$tt1=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t1[0].');'));
	     			    //  echo '2 kl1:, kl2: ' . $tt1[0]. ' '. $tt2[0].'<br>';
	                    addchp ('<font color=red>Внимание!2</font>после первого запроса ID кланов' . $tt1[0]. '||'. $tt2[0].' ','{[]}A-Tech{[]}');

						$t1_klan=mysql_fetch_array(mysql_query('select '.$tt2[id].' as my_clan, cw.* from clans_war as cw where
						((defender = '.$tt2[id].' AND agressor ='.$tt1[id].') or
						 (defender = '.$tt1[id].' AND agressor ='.$tt2[id].'))
						 AND osnova = 1 AND `date`>='.time().' AND id= '.$data_battle[war_id].';'));
 ...
 // проверка на рамку тайма
	                ...
	                if ($data_battle[win]==1)
		                ...
		                //$t2=mysql_fetch_array(mysql_query('select id from users where battle='.$data_battle[id].' AND battle_t=2 '));
		                	$t1=explode(";",$data_battle[t1]);
	             	        $t2=explode(";",$data_battle[t2]);
		                // echo '1 t1, t2:' . $t1[0]. ' '. $t2[0]. '<br>';
		                    addchp ('<font color=red>Внимание победа тимы 1!bot 3</font>тимы t1, t2: ' . $t1[0]. ' || '. $t2[0].' бой'.$data_battle[id],'{[]}A-Tech{[]}');
                  			$tt2=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t2[0].');'));
							$tt1=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t1[0].');'));
	                	// echo '1 kl1, kl2:' . $tt1[0]. ' '. $tt2[0].'<br>';
						   ...
						   if ($data_battle[win]==2)
						{
		                    ...
		                    addchp ('<font color=red>Внимание победа тимы 2!bot 4</font>тимы t1, t2: ' . $t1[0]. ' || '. $t2[0].' бой'.$data_battle[id],'{[]}A-Tech{[]}');
			                $tt2=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t2[0].');'));
							$tt1=mysql_fetch_array(mysql_query('select if(base_klan=0,id,base_klan) as id from clans  where short=(select klan from users where id='.$t1[0].');'));
		     			    //  echo '2 kl1:, kl2: ' . $tt1[0]. ' '. $tt2[0].'<br>';
		                    addchp ('<font color=red>Внимание!2</font>после первого запроса ID кланов' . $tt1[0]. '||'. $tt2[0].' ','{[]}A-Tech{[]}');
	...
	$bots2=mysql_query('SELECT * FROM `users_clons` WHERE hp >0 and battle_t=2 ;');
$restal_bots_arr=array(247,248,249,250,251);		
	  // проходимся по каждому
	   while ($user = mysql_fetch_array($bots2))
	    {
	    $hhh3=0;
    		 $hhh3=get_hil_bot($user);//хил в команде ботов 1
    		 if ($hhh3 >0) {$user[hp]=$hhh3; }

	    	 $data_battle=mysql_fetch_array(mysql_query("SELECT * FROM battle where id={$user[battle]} ; "));
	    	 
	    	  if (in_array($data_battle[type],$restal_bots_arr))
	    	 {
	    	 //если бой в ристалище пропуск
	    	  continue;
	    	  }
