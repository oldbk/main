<?
	...
	if ((!($_SESSION[jail_kr]>0)) AND (!($_SESSION[jail_ekr]>0)))  { header("Location: ../haosexit.php?err=Неизвестная ошибка"); 		die();}
...
if ($menu==11) {
	...
	echo '<form method="post" accept-charset="windows-1251"  onSubmit="return checkSubmit();">
		...
		<input type="hidden" name="com" value="Погашение долга в хаосе: '.$EKR_DOLG.' екр. персонажа: '.$user['login'].'"/>';
...
?>    	
	...
	echo '<input type="hidden" name="LMI_PAYMENT_DESC"  value="Погашение долга в хаосе: '.$EKR_DOLG.' екр. персонажа: '.$user['login'].'"/>';
...
elseif (($menu==13) OR ($menu==14) OR ($menu==15)) {
	echo '<center>
	<table border=0 bgcolor=#eeeeee width="100%" height="100%" align="center">
	<tr valign="top"  align="center" >
	<td align="center">';
