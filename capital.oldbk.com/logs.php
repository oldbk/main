...
if (($data['teams']=='AFB') and (!ADMIN) )
	{
	die("<center><b>Лог боя скрыт!</center></body></html>");
	}
...
$filename=$logdir."/battle".$_REQUEST['log'].".txt";
	...
	if ($user['klan']=='radminion') { echo 'Размер файла ' . $filename . ': ' . filesize($filename) . ' байт <br>';	 }
...
<?
	...
	if (($data['coment']=='<b>Куча-мала</b>') and ($user[klan]=='radminion') )
	{
		...
		if ($data['win']==3)
		{
			...
			if (mysql_num_rows($get_data) > 0)
			{
				...
				echo "<b><i>Весы куча-мала:  {$teams[1]}  /   {$teams[2]}  </i></b><br> ";
			}
		}
			...
			if (file_exists($fllog))
				{
				$kuchalog = file($fllog);
				echo "<small><hr>История весов:<br>";
			...
			if (($data['coment']=='<b>Бой с Волнами Драконов</b>') and ($data['id']>222482354) and time()<1462060800 )
			{
			$data['coment']='<b>Нашествие обезьян</b>';
			}

			if (($data['coment']=='<b>#zlevels</b>') or ($data['coment']=='#zlevels') or ($data['coment']=='<b>#zbuket</b>') or ($data['coment']=='<b>#zelka</b>') )
				{
				$data['coment']='<b>Автозаявка</b>';
				}
			...
			elseif ($data['blood']>0 && $data['type'] == 7) {
				$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype6.gif\" WIDTH=20 HEIGHT=20 ALT=\"Кровавый бой \"><IMG SRC=\"http://i.oldbk.com/i/fighttype7.gif\" WIDTH=20 HEIGHT=20 ALT=\"Бой на елках\"> (кровавый бой на елках {$data['coment']})";
			}
			elseif ($data['blood']==0 && $data['type'] == 7) {
			...
			elseif ($data['blood']>0 && $data['type'] == 8) {
				$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype6.gif\" WIDTH=20 HEIGHT=20 ALT=\"Кровавый бой \"><IMG SRC=\"http://i.oldbk.com/i/fight_flowers.png\" WIDTH=20 HEIGHT=20 ALT=\"Бой на букетах\"> (кровавый бой на букетах {$data['coment']})";
			}
			elseif ($data['blood']==0 && $data['type'] == 8) {
				$rr = "<IMG SRC=\"http://i.oldbk.com/i/fight_flowers.png\" WIDTH=20 HEIGHT=20 ALT=\"Бой на букетах \"> (Бой на букетах {$data['coment']})";
			}
			...
			elseif (($data['type'] == 3 && $data_coment != "Бой склонностей") OR $data['type'] == 2) {
			...
			elseif ($data['type'] == 40 || $data_coment == "Бой склонностей") {
				if ($data_coment == "Бой склонностей") {
					...
					if ($bd_levels[1]==$bd_levels[0])
						{
						$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype40.gif\" WIDTH=20 HEIGHT=20 ALT=\"Противостояние\"> (Противостояние) ".$bd_levels[0].'-уровни ';
						}
						else
						{
						$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype40.gif\" WIDTH=20 HEIGHT=20 ALT=\"Противостояние\"> (Противостояние) ".$bd_levels[0].'-'.$bd_levels[1].'уровни ';
						}
				}
			}
			elseif ($data['type'] == 41) {
				$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype41.gif\" WIDTH=20 HEIGHT=20 ALT=\"Кровавое Противостояние\"> (Кровавое Противостояние) ".$data['damage'].'-уровни ';
			}
			else
			{
			$rr = "<IMG SRC=\"http://i.oldbk.com/i/fighttype3.gif\" WIDTH=20 HEIGHT=20 ALT=\"групповой бой\"> (групповой поединок{$data['coment']})";
...
$ffs=$kom1_echo.$kom2_echo.$kom3_echo;
			...
			if (($data['status_flag'] == 10) and ($data['type'] == 7) ) {
				$battle_status = "<h3>Безумный бой на елках</h3>";
			}else
			if (($data['status_flag'] == 10) and ($data['type'] == 8) ) {
				$battle_status = "<h3>Безумный бой на букетах</h3>";
			}
			else
			...
			}elseif ($data['status_flag'] == 4)
			{
				if ($data['id'] == 232196922)
					{
					$battle_status = "<h3>Бой защитников Кэпитал-сити!</h3>";
					}
					else
					{
			...
			if  ($data[win]==3)
				...
				if (mysql_affected_rows()==2)
				  ...
				  $get_closetime=mysql_fetch_array($get_info);
					...
					if ($MM>=60) {$MM=59;}
				        echo "<font color=red><b>Бой закроется через: ".$HH." ч. ".$MM." мин.</b></font><br>";
				        }
				  }
				}
...
echo "<table border=0 width=100% > <tr> <td width=33%>";
			...
			$timeo = "Бой идет с таймаутом {$data['timeout']} мин.";
			...
			if (($data['id']==337326646) and ($data['teams']=='') )
				...
				if (($finish_time-time())>0)
					{
					$timeo .="<br><font color=red><b>Бой будет закрыт от вмешательства через ".prettyTime(null,$finish_time)."</b></font><br>";
					}
				}
			...
			else
			{
				if ($data['teams']!='')
					...
					$h=explode(":||:",$data['teams']);
					      if ($h[0]==20000)
					      	{
							$timeo .= " Бой изолирован...";
					      	}
					      	else
				      		{
							$timeo .= "  Бой закрыт от вмешательств...{$data['teams']}";
						}
					}
			}
			...
			*/
				...
				if ($data['type'] == 40 ||  $data['type'] == 41 || $data['type'] == 140 ||  $data['type'] == 141 || $data['type'] == 150 || $data['type'] == 151 ||  $data_coment == "<b>Бой на Центральной площади</b>")
				{
			...
			if ($_GET['stat']!='1')
			{
			echo "<div align=right><form method=get><input type=hidden name='log' value='".(int)$_GET['log']."'><input type=hidden name='stat' value='1'><input type=submit value='Статистика боя'></form></div>";
			}

			echo "<div align=right><FORM METHOD=GET ACTION='logs.php'>
			<INPUT TYPE=hidden name='page' value='".$_GET['page']."'>
			<INPUT TYPE=hidden name='log' value='".$_REQUEST['log']."'>
			Фильтровать по нику:<INPUT TYPE='text' name='flogin' value='".$_GET['flogin']."'>
			<INPUT TYPE=submit name=filt value=\"Показать\"><br>";
			if ($user['id']>0)
				{
				echo "<INPUT TYPE=submit name=myonly value=\"Только мои размены\"><br>";
				}
			echo "
...
ksort($abilname);
	...
	for($i=$start;$i<=$stop;$i++)
	{
		...
		if ($NEW_LOG)
		{
			...
			*/
				if (isset($_GET['flogin']))
				{
					$get_arr_line=explode(":",$log[$i]); //6- ник противника


					if (( ((strpos($log[$i],$_GET['flogin'])) > 0) and  (((strpos($get_arr_line[3],'Невидимка')) !==false) and ((strpos($get_arr_line[3],$_GET['flogin'])) !==false) ) or (((strpos($get_arr_line[6],'Невидимка')) !==false) and ((strpos($get_arr_line[6],$_GET['flogin'])) !==false) )  ) AND (!(($_GET['flogin']==$user['login']) and ($user['id']>0))) )
					{
						//не печатаем - т.к. искомый ник был неведом
	...
	if ((isset($_GET['flogin'])) and ($print_flag==false) )
		{
			echo "<center><b>Размены в бою не найдены.</b></center><BR>";
		}
?>
<HR>
<?
