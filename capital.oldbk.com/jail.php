<?php
	...
	$qiwi_to=(int)($_POST[to]);
		...
		if ( ($_POST[amount_rub]==$RUR_DOLG) AND ($RUR_DOLG>0) )
		{
			...
			$linkqiwi="http://w.qiwi.ru/setInetBill.do?frm=1&from=6920&lifetime=0.0&check_agt=false&com=".urlencode("Погашение долга в темнице {$EKR_DOLG} екр. персонажа {$user['login']}")."&to={$qiwi_to}&txn_id={$txn_id}&amount_rub={$RUR_DOLG}";
			//echo $linkqiwi;
			$f=@fopen($linkqiwi,"r");
				if($f)
				{
	...
	else if ( (isset($_POST[mk_bill_cost])) and ( (float)($_POST[mk_bill_cost])>0 ) )
	{
		...
		if ($get_bill_id[id] >0)
			...
			mysql_query("INSERT INTO oldbk.`inventory` (`owner`,`name`,`type`,`massa`,`cost`,`img`,`letter`,`maxdur`,`isrep`,`present`) VALUES('{$user[id]}','Счет на оплату услуг','50',0,0,'paper100.gif','Счет на оплату услуги:<b>Оплата досрочного выхода из темницы</b>.<br>\n Сумма:{$EKR_DOLG} <br>\n Номер счета:<b>{$str_bill}</b>',1,0,'Коммерческий отдел') ;");
	...
	function err($txt)
	{
		echo '<br><font color=red><b>'.$txt.'</b></font>';
	}
	$can_take=false;
	...
	//временно палам
	if($user[id]==14622 || $user[id]==8635 || $user[id]== 356294 || $user[id]==13752 || $user[id]==64494)
	{
		$access[item_hist]=1;
	}
	
	if ($user['in_tower'] != 4 && !$access[item_hist]) { header('Location: main.php'); die(); }
	...
	{
		$pal=true;
		echo '<form action? method=post>Введите ник заключенного <input name=telo type=text value='.$_POST[telo].'><input type=submit name=просмотреть>';
		if($_POST[telo])
		{
			$telo=mysql_fetch_assoc(mysql_query("SELECT * FROM oldbk.users WHERE login='".$_POST[telo]."' AND in_tower=4  AND block=0;"));
			$telo=check_users_city_data($telo[id]);
		}
	}
	
	$effects=mysql_fetch_assoc(mysql_query("SELECT * FROM ".$db_city[$telo[id_city]]."effects WHERE owner='".$telo[id]."' AND type=444"));
	...
	if($_POST[razdel] && $effects['time'] && ($vikup[kr]>0 || $vikup[ekr]>0) && $can_take==true)
	{
					...
					if($bank[id])
					{
						if($data_schet[3]=='kr' && $bank[cr]>=$_POST['count'] && $vikup[kr]>=$_POST['count'])
						{
							if(mysql_query("UPDATE oldbk.bank SET cr=cr-'".(int)$_POST['count']."' WHERE id=".$bank[id]." AND owner=".$telo[id]." "))
							{
								...
								SET date='".time()."', text='Списано <b>".$_POST['count']."кр.</b> В счет уплаты долга.', bankid=".$bank[id].";");
						...
						if($data_schet[3]=='ekr' && $bank[ekr]>=$_POST['count'] && $vikup[kr]>=($_POST['count']*$course_ekr_kr))
						{
							if(mysql_query("UPDATE oldbk.bank SET ekr=ekr-'".(int)$_POST['count']."' WHERE id=".$bank[id]." AND owner=".$telo[id]." "))
							{
								mysql_query("INSERT INTO oldbk.bankhistory 
								SET date='".time()."', text='Списано <b>".$_POST['count']."екр.	</b> В счет уплаты долга.', bankid=".$bank[id].";");
		...
		if($data_schet[1]=='item')
		{
			echo '-1';
			if(is_numeric($data_schet[2]))
			{
				//echo '-2';
				$item=mysql_fetch_assoc(mysql_query("SELECT * FROM oldbk.inventory WHERE owner ='".$telo[id]."' AND id='".$data_schet[2]."' AND type<12 AND prototype not in (".implode(',',$vaucher).") AND prokat_idp = 0 AND present!='Арендная лавка' "));
				...
				if($vikup[kr]>0)
				{
					...
					if($item[id]==$data_schet[2] && (($item[unik]==0 && $item[art_param]=='' && ($prisoner==true || $pal==true)) || (($item[unik]!=0 || $item[art_param]!='')&&$pal==true)))
					{
						...
						if(($item[unik]!=0 || $item[art_param]!='')&&$pal==true && $_POST[count_art])
						{
							
							$realcost=(int)$_POST[count_art];
						}
						
						if(mysql_query("UPDATE oldbk.inventory set owner=8325 WHERE id=".$item[id]." AND owner='".$telo[id]."' AND type<12 AND prototype not in (".implode(',',$vaucher).") AND prokat_idp = 0 AND present!='Арендная лавка' "))
						{
		...
		if($data_schet[1]=='sertk' || $data_schet[1]=='serte') 
		{
		//Сертификаты
			if(is_numeric($data_schet[2]))
			{
				...
				if($vikup[kr]>0 && $data_schet[1]=='sertk')
				{
					...
					//echo '-3';
					//узнали стоимость сертификата в КР
					...
					{				
						...
						if(mysql_query("UPDATE oldbk.inventory set owner=8325 WHERE id=".$item[id]." AND owner='".$telo[id]."' AND prototype in (".implode(',',$vaucher).")"))
						{
							...
							if($realcost>$vikup[kr])
							{
								...
								$data=mysql_query("SELECT * FROM oldbk.eshop WHERE id in 
								(".implode(',',$vaucher).") order by ecost desc;");
				...
				//гасим екр долг
				if($vikup[ekr]>0 && $data_schet[1]=='serte' && is_numeric($bank_rez))
				{
		...
		if($ok==1)
		{
			...
			{
				//выпускаем тело
				mysql_query("update ".$db_city[$telo[id_city]]."users set room=1, in_tower=0 where id='".$telo[id]."'");
				
				$mess="Досрочно выпущен из заточения, долг погашен.";
				mysql_query("INSERT INTO oldbk.`lichka`(`id`,`pers`,`text`,`date`) VALUES ('','".$telo['id']."','".$mess."','".time()."');");
				mysql_query("INSERT INTO oldbk.`jail_log`(uid,type,date_time) VALUES ('".$telo['id']."','".$type."','".time()."')");
				
				
				$effects['add_info']=serialize($vikup);
				mysql_query("DELETE from ".$db_city[$telo[id_city]]."effects where type=444 AND owner='".$telo[id]."' ");
			...
			else
			{
				...
				//echo ' '.$vikup .'<br>';
				$effects['add_info']=serialize($vikup);
				mysql_query("UPDATE ".$db_city[$telo[id_city]]."effects set add_info='".($effects['add_info'])."' WHERE owner=".$telo[id]." AND type=444");
			}
		}
...
//во все файлы с переходами добавить in_tower=3. (маин, сити и тд..)
...
<script>	
			...
			function getformdata(id,param,event)
			{
				...
				if (event ) 
				{
				       ...
				       $.get('jailpayform.php?id='+id+'&param='+param+'', function(data) {
					  $('#pl').html(data);
...
<?
	...
	echo (($vikup[kr]>0) || ($vikup[ekr]>0)?'Досрочный выход за: <b>':'<br>Досрочный выход:</b> обратитесь в коммерческий отдел').($vikup[kr]>0?'<span id="vik_kr">'.$vikup[kr].'</span>кр.':'').($vikup[ekr]>0?'<span id="vik_ekr">'.$vikup[ekr].'</span>екр. ':'').'</b>';
...
?>
	...
	if($vikup[kr]>0)
	{
		...
		if(mysql_num_rows($data)>0)
		{
			...
			{
				echo "<tr><td>Счет № ". $bank[id] . " : </td><td>&nbsp;</td><td><b><span id='bank_money_".$bank[id]."_exist_kr'>" .$bank[cr]. "</span> кр.</b></td><td>&nbsp;&nbsp;&nbsp;</td><td><a href=# onclick=\"spisanie('bank_money_".$bank[id]."','kr',' Списать кредиты<br>Счет № ".$bank[id]."<br>')\">Списать со счета.</a></td></tr>
				<tr valign=top><td>Счет № ". $bank[id] . " : </td><td>&nbsp;</td><td><b><span id='bank_money_".$bank[id]."_exist_ekr'>".$bank[ekr]."</span> екр.</b></td><td>&nbsp;&nbsp;&nbsp;</td><td><a href=# onclick=\"spisanie('bank_money_".$bank[id]."','ekr',' Списать еврокредиты<br>Счет № ".$bank[id]."<br>')\">Списать со счета.</a><br><br></td></tr>";
			}
...
function inv($telo)
{
	global $prisoner, $vaucher, $vikup, $pal, $can_take;
		$serts=array();
		$items=array();
		//print_r(implode(',',$vaucher));
		
		//prokat_idp = 0 ".$what_not_to_sell." AND bs_owner = 0 AND arsenal_klan='' AND present!='Арендная лавка'
		
		$data=mysql_query("SELECT * FROM oldbk.inventory WHERE owner = '".$telo[id]."' AND (type <12 or prototype in (".implode(',',$vaucher).")) AND prokat_idp = 0 AND present!='Арендная лавка' ");
		while($row=mysql_fetch_assoc($data))
		{
...
if($pal==true && $can_take==false && $_POST)
{
