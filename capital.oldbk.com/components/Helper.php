        ...
        //креды/екры/репа
        {
            ...
            //екры
            {
                ...
                //заплатил
                {
                    ...
                    $delo_type[2283]="\"{$rec['owner_login']}\" заплатил {$rec['sum_ekr']} екр. со своего счета №.{$rec['bank_id']} в банке за услуги Коммерческого отдела. {$rec['add_info']}";
            ...
            //репа
            {
                //получил
                {
                    ...
                    //тут еще и шмотки....  победы разные
                    {
                        ...
                        $delo_type[4]="\"{$rec['owner_login']}\" получил: ".self::if_have(" За участие:",$rec['sum_kr']," кр.")." ".self::if_have(" В награду:",$rec['item_name']," и ")."  ".self::if_have("",$rec['sum_rep']," репутации ")." на ристалище в \"Одиночных сражениях\".";
...
//Предметы
        {
            //купил
            {
                $delo_type[1]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ".self::if_have(" ",$rec['sum_kr']," кр.")."  ".self::if_have(" ",$rec['sum_ekr']," екр.")."  ".self::if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
                $delo_type[401]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за ".self::if_have(" ",$rec['sum_kr']," кр.")."  ".self::if_have(" ",$rec['sum_ekr']," екр.")."  ".self::if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
                $delo_type[2]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_rep']} кр.".self::if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
                ...
                $delo_type[354]="\"{$rec['owner_login']}\" купил \"{$rec['item_name']}\" за ".self::if_have(" ",$rec['sum_ekr']," екр.")." ".self::if_have(" ",$rec['sum_rep']," реп.").".";
                ...
                $delo_type[272]="\"{$rec['owner_login']}\" купил товар: \"{$rec['item_name']}\" (x{$rec['item_count']}) [{$rec['item_dur']}/{$rec['item_maxdur']}] за {$rec['sum_kr']} кр.".self::if_have(" (в арсенал клана:",$rec['item_arsenal'],")");
            ...
            //получил/подарили
            {
                ...
                $delo_type[452]="\"{$rec['owner_login']}\" получил ваучер на {$rec['sum_ekr']} екр, от \"{$rec['target_login']}\".";
                ...
                $delo_type[185]="\"{$rec['owner_login']}\" выйграл предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. в Лотерею.";
                $delo_type[1185]="\"{$rec['owner_login']}\" выйграл предмет:\"{$rec['item_name']}\" [{$rec['item_dur']}/{$rec['item_maxdur']}] {$rec['item_count']}шт. в Пасхальную лотерею.";
            ...
            {//разные действия с предметами
                ...
                $delo_type[80]="\"{$rec['owner_login']}\" перенес свиток \"{$rec['item_incmagic']}\" из \"{$rec['add_info']}\" в \"{$rec['item_name']}\".".self::if_have(" (предмет клана:",$rec['item_arsenal'],")");
                $delo_type[81]="\"{$rec['owner_login']}\" перевстроил свиток \"{$rec['add_info']}\" в \"{$rec['item_name']}\".".self::if_have(" (предмет клана:",$rec['item_arsenal'],")");
...
// вывод
        if ($al[perevodi]>=5 && $add_info_off==2)
        {
            ...
            //админам доп инфа
            $out.=" (".
                self::if_have("операция с:",$rec['target_login']).
                self::if_have(", Сумма:",$rec['sum_kr']," кр.").
                self::if_have(", Сумма:",$rec['sum_ekr']," екр.").
                self::if_have(", коммисия:",$rec['sum_kom']).
                self::if_have(", номер счета:",$rec['bank_id']).
                self::if_have(", использован предмет id:",$rec['aitem_id']).
                self::if_have(", стоимость предмета:",$rec['item_cost']).
                self::if_have(", id:",$rec['item_id']).
                self::if_have(", ups:",$rec['item_ups']).
                ($rec['item_unic'] > 0 ? ", <font color=red>Уник: ".$rec['item_unic']."</font>" : "").
                self::if_have(", Встройка:",$rec['item_incmagic']).
                self::if_have(", Юзов:",$rec['item_incmagic_count']).
                self::if_have(", Арсенал клана:",$rec['item_arsenal']).
                self::if_have(", лог боя:",$rec['battle']).
                self::if_have(", Баланс до:",$rec['owner_balans_do']," /").
                self::if_have(" Баланс после:",$rec['owner_balans_posle']).
                self::if_have(", реп. до:",$rec['owner_rep_do']," /").
                self::if_have(" реп. после:",$rec['owner_rep_posle']).
                self::if_have(" доп инфо:",$rec['add_info'])."   )";
            $out=str_replace("(,","(",$out);
            $out=str_replace(",,",",",$out);
        }
        else
