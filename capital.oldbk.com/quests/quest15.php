	...
	for ($x=1;$x<$maxc;$x++)
		{
			for ($y=1;$y<$maxc;$y++)		
				{
					if ($map[$x][$y]=='R') // ставим метки только там где монстры
						{
					 	$map[$x][$y]='Q'; 
						$k_it--;
...
//тут же можно кинуть системку
	...
	$mob[$rmt]=$Q_MOB; // добавл€ем монстра к существующей патти
}
...
addchp ('<font color=red>¬нимание!</font> ќтличный —увенир, пригодитс€...','{[]}'.$telo['login'].'{[]}',$telo['room'],$telo['id_city']);
