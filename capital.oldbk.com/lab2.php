...
$rep_kof=2; // множитель репутации на на одного монстра от его ленейки
	   ...
	   if ($_SERVER["SERVER_NAME"]=='capitalcity.oldbk.com') { $cityname='capitalcity' ;  }
		...
		include ("labconfig_s2.php"); // настройки
			...
			$dkey=array(1=>3301,2=>3302,3=>3303,4=>3304);
			/////////////////////////////////////////////
...
////////////////////////////////////////////load map file//////////////////////////////////////////////////
		...
		if (file_exists($mfile)) 
			...
			{
			copy('/www/capitalcity.oldbk.com/labmaps/default_lab2.map', $mfile);
			$map=file($mfile);
			}
...
//загрузка нужных эффектов
	$load_eff=mysql_query("SELECT * FROM `effects` WHERE `owner` = '".$user['id']."' AND (`type` in (11,12,13,7777)) ");
	$STRAHOVKA=array();
	while ($eff = mysql_fetch_array($load_eff))	
		{
			if ($eff['type']==7777)
				...
				elseif ($eff['type']==11||$eff['type']==12||$eff['type']==13)
				{
				$tviw=true;
				}
		}
				   ...
				   // echo "The file $fmap exists";
				    ...
				    $qdata_array = file($fmap);
						foreach($qdata_array as $qi=>$qstring)
					                        ...
					                        if ( ($map[$qmas[0]][$qmas[1]]!='R') AND ($map[$qmas[0]][$qmas[1]]!='J')) //накладываем если точка не монстр
					                          {
...
/////////////////////////////////////////////////////////////////////////
		...
		if ( ($labpoz[x] ==0 ) OR ($labpoz[y] ==0 ) )
			{
			$fotofile='http://i.oldbk.com/i/laba/foto1.jpg';
			unset($_SESSION['charka_lab_count']);
			//$msg = "<br>";
...
//убрано в fbattle     mysql_query("INSERT `labirint_var` (`owner`,`var`,`val`) values('".$user[id]."', 'labcount', '1' ) ON DUPLICATE KEY UPDATE `val` =`val`+1;");
		...
		else
		{
			...
			if (($labpoz[dead] > 0) and isset($STRAHOVKA['owner']) )
					...
					mysql_query("INSERT `labirint_items` (`map`,`item`,`x`,`y`,`active`,`count`,`owner`,`val`) values('{$mapid}','T','".$need_poz[2]."','".$need_poz[3]."','1','1','{$user['id']}','1')  ON DUPLICATE KEY UPDATE `active` =1, `count`=`count`+1;");
				     	//удаляем страховку
				     	mysql_query("DELETE FROM `oldbk`.`effects` WHERE owner='{$user['id']}' and `type`=7777 ");
					}
			...
			$rmt=(int)$rmt;
			if ($rmt < 1) {  $rmt=1; }
			$fotofile='http://i.oldbk.com/i/laba/foto'.$LAB.'/'.$rmt.'.jpg';
			}
...
if (($Aitems[$labpoz[x]][$labpoz[y]]!='R') AND ($Aitems[$labpoz[x]][$labpoz[y]]!='J'))  // не ставим Q-поинт
{
...
if (($_POST[exit_good])and($map[$labpoz[x]][$labpoz[y]]=='F'))
		{
		// правильный выход
		$_SESSION['quest']='';
		$_SESSION['questid']='';
	...
	if ($testdress[id] >0)
		...
		mysql_query("UPDATE oldbk.`inventory` SET `labflag`=0  WHERE  `labflag`=1 and `owner`='".$user[id]."';");
				...
				//удаляем файл карты
				unlink('/www/capitalcity.oldbk.com/shared/labmaps/'.$mapid.'.map');
...
if ($_GET[getitem])
				{
		          ...
		          ///old 		if (mysql_query("DELETE FROM `labirint_items` where `x`='".$labpoz[x]."' and `y`='".$labpoz[y]."' and `map`='".$mapid."' and `active`=0 and `item`='I' and `val`='".$getitem[val]."' LIMIT 1;"))
				   ...
				   if ($checkok=="TRUE")
					...
					if ( ($dress['id']>=50076)and($dress['id']<=50078))
					{
					/*
					Телепорт Лабиринта PROTO_ID:50076
					Открыть Портал PROTO_ID:50077
					Познание Лабиринта PROTO_ID:50078					
					*/
					$dress['ekr_flag']=0;
					$dress['ecost']=0;
					}
...
mysql_query("INSERT INTO oldbk.`inventory`
				...
				VALUES
				('{$dress['id']}','{$user['id']}','{$dress['name']}','{$dress['type']}',{$dress['massa']},{$dress['cost']},'{$dress['img']}' , '{$dress['img_big']}' , '12','{$dress['rareitem']}','{$dress['ekr_flag']}' , {$dress['duration']},{$dress['maxdur']},{$dress['isrep']},'{$dress['nclass']}','{$dress['gsila']}','{$dress['glovk']}','{$dress['ginta']}','{$dress['gintel']}','{$dress['ghp']}','{$dress['gnoj']}','{$dress['gtopor']}','{$dress['gdubina']}','{$dress['gmech']}','{$dress['gfire']}','{$dress['gwater']}','{$dress['gair']}','{$dress['gearth']}','{$dress['glight']}','{$dress['ggray']}','{$dress['gdark']}','{$dress['needident']}','{$dress['nsila']}','{$dress['nlovk']}','{$dress['ninta']}','{$dress['nintel']}','{$dress['nmudra']}','{$dress['nvinos']}','{$dress['nnoj']}','{$dress['ntopor']}','{$dress['ndubina']}','{$dress['nmech']}','{$dress['nfire']}','{$dress['nwater']}','{$dress['nair']}','{$dress['nearth']}','{$dress['nlight']}','{$dress['ngray']}','{$dress['ndark']}',
				'{$dress['mfkrit']}','{$dress['mfakrit']}','{$dress['mfuvorot']}','{$dress['mfauvorot']}','{$dress['bron1']}','{$dress['bron2']}','{$dress['bron3']}','{$dress['bron4']}','{$dress['maxu']}','{$dress['minu']}','{$dress['magic']}','{$dress['nlevel']}','{$dress['nalign']}','".(($dress['goden'])?($dress['goden']*24*60*60+time()):"")."','{$dress['goden']}','{$dress['nsex']}','{$dress['razdel']}','{$labonly}','1','{$needpres}','{$dress['group']}','{$dress['letter']}','{$soown}','{$dress['ecost']}','{$user[id_city]}'
				) ;");
...
//////////
 if ($_GET[useitem])
 	...
 	// использование
 	// ключей
 	// определяем какую дверь
	$TTD=(int)$_GET[useitem];//номер двери
	//5 навсяк
	if (($TTD < 5) AND ($TTD>0) AND ($doors[$TTD]!=''))
		{
		$test_door=$doors[$TTD]; //буква двери
 		$DOOR=get_door_xy($test_door);	// прверка наличия нужной двери там где она должна быть + закрыта
 		if ($DOOR)
			{
			// правильная дверь
			//запрашиваем ключь
			$i_have_key=mysql_fetch_array(mysql_query("select * from oldbk.inventory where (prototype={$dkey[$TTD]} OR prototype=3333) and owner='{$user[id]}' and setsale=0 ORDER by labflag DESC, prototype LIMIT 1;"));
			         if   ($i_have_key[id]>0)
			         {
			         // есть ключик
			         // проверяем дурейшен
			          if ( ($i_have_key[duration]+1)>=$i_have_key[maxdur] )
			            {
			            //удаляем
            			 		if (mysql_query("delete from oldbk.inventory where id={$i_have_key[id]} and owner='{$user[id]}' ; "))
			 			{
						...
						$Aitems[$DOOR[x]][$DOOR[y]]=$test_door;//апдейтим в памяти
					 	$msg="<font color=red>Вы открыли дверь...</font>";
			 			}
			            }
			            else
			            {
			            //иначе апдейтим дурайшен
           			 		if (mysql_query("UPDATE oldbk.inventory SET duration=duration+1 where id={$i_have_key[id]} and owner='{$user[id]}' ; "))
			 			{
						mysql_query("INSERT `labirint_items` (`map`,`item`,`x`,`y`,`active`,`count`) values('".$mapid."','{$test_door}','".$DOOR[x]."','".$DOOR[y]."','1','1' ) ON DUPLICATE KEY UPDATE `active` =1, `count`=`count`+1;");
						$Aitems[$DOOR[x]][$DOOR[y]]=$test_door;//апдейтим в памяти
					 	$msg="<font color=red>Вы открыли дверь...</font>";
			         ...
			         {
			         $msg="Нет нужного ключика :(";
			         }
			}
			else
			{
		         $msg="Это неправильные пчелы :(";
			}
		}
 		else
 		{
	         $msg="Чето, как-то, не использовать это тут... :(";
 		}
 ...
 if ($_GET[openbox]) {
			...
			$dpbox=round(($labpoz[y]*$dpbox/(strlen($map[1])))); // получаем уровень сложности взависимости от положения перса к выходу.
				...
				if ($rmt>=13)
					...
					{
					//если больше половины карты проверяем ивент
							if (!(isset($_SESSION['charka_lab_count'])))
								{
								$_SESSION['charka_lab_count']=mysql_fetch_array(mysql_query("select * from labirint_var where owner='{$user['id']}' and var='charka_lab_count'"));
								}
						
						if ($_SESSION['charka_lab_count']>=10)
							{
									//добавка чарки
									$pbox[$rmt] = array("charka1" => 56661);
									$_SESSION['charka_lab_count']=1;
									mysql_query("UPDATE `oldbk`.`labirint_var` SET `val`=1 WHERE  owner='{$user['id']}'  AND `var`='charka_lab_count' ");
							}
...
if ($_GET[sunduk])
			...
			if(mysql_affected_rows()>0)			
				...
				(`prototype`,`owner`,`name`,`type`,`massa`,`cost`,`img` ,`img_big`,`getfrom`,`rareitem`,`ekr_flag` ,`duration`,`maxdur`,`isrep`,`nclass`,
					`gsila`,`glovk`,`ginta`,`gintel`,`ghp`,`gnoj`,`gtopor`,`gdubina`,`gmech`,`gfire`,`gwater`,`gair`,`gearth`,`glight`,`ggray`,`gdark`,`needident`,`nsila`,`nlovk`,`ninta`,`nintel`,`nmudra`,`nvinos`,`nnoj`,`ntopor`,`ndubina`,`nmech`,`nfire`,`nwater`,`nair`,`nearth`,`nlight`,`ngray`,`ndark`,
					`mfkrit`,`mfakrit`,`mfuvorot`,`mfauvorot`,`bron1`,`bron2`,`bron3`,`bron4`,`maxu`,`minu`,`magic`,`nlevel`,`nalign`,`dategoden`,`goden`,`nsex`,`otdel`,`present`,`labonly`,`labflag`,`letter`,`group`,`idcity`
				)
				VALUES
				('{$dress['id']}','{$user['id']}','{$dress['name']}','{$dress['type']}',{$dress['massa']},{$dress['cost']},'{$dress['img']}', '{$dress['img_big']}' , '12','{$dress['rareitem']}','{$dress['ekr_flag']}' ,{$dress['duration']},{$param['maxdur']},{$dress['isrep']},'{$dress['nclass']}','{$dress['gsila']}','{$dress['glovk']}','{$dress['ginta']}','{$dress['gintel']}','{$dress['ghp']}','{$dress['gnoj']}','{$dress['gtopor']}','{$dress['gdubina']}','{$dress['gmech']}','{$dress['gfire']}','{$dress['gwater']}','{$dress['gair']}','{$dress['gearth']}','{$dress['glight']}','{$dress['ggray']}','{$dress['gdark']}','{$dress['needident']}','{$dress['nsila']}','{$dress['nlovk']}','{$dress['ninta']}','{$dress['nintel']}','{$dress['nmudra']}','{$dress['nvinos']}','{$dress['nnoj']}','{$dress['ntopor']}','{$dress['ndubina']}','{$dress['nmech']}','{$dress['nfire']}','{$dress['nwater']}','{$dress['nair']}','{$dress['nearth']}','{$dress['nlight']}','{$dress['ngray']}','{$dress['ndark']}',
				'{$dress['mfkrit']}','{$dress['mfakrit']}','{$dress['mfuvorot']}','{$dress['mfauvorot']}','{$dress['bron1']}','{$dress['bron2']}','{$dress['bron3']}','{$dress['bron4']}','{$dress['maxu']}','{$dress['minu']}','{$dress['magic']}','{$dress['nlevel']}','{$dress['nalign']}','".(($dress['goden'])?($dress['goden']*24*60*60+time()):"")."','{$dress['goden']}','{$dress['nsex']}','{$dress['razdel']}','{$param['present']}','{$param['labonly']}','1','{$dress['letter']}','{$dress['group']}','{$user[id_city]}'
				) ;");
...
///
		...
		if ($_SESSION['time_trap']==1) {$time_go=6;} else {$time_go=1;}
			...
			switch($_GET['gto']) {
				case "p1":
					//$msg="P1<br>";
				if (($map[$labpoz[x]-1][$labpoz[y]]=="A")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Можно пройти только с другой стороны...</b></font>";
					//добавляем в итемы отработку ледяной стенки
					}
				else
				if (($map[$labpoz[x]-1][$labpoz[y]]=="G")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Лёд треснул...</b></font>";
					//добавляем в итемы отработку ледяной стенки
					mysql_query("INSERT `labirint_items` (`map`,`item`,`x`,`y`,`active`,`count`) values('".$mapid."','G','".($labpoz[x]-1)."','".$labpoz[y]."','1','1' ) ON DUPLICATE KEY UPDATE `active` =1, `count`=`count`+1;");
					}
				else
				if (
				...
				if ((($map[$labpoz[x]-1][$labpoz[y]]=="N") OR ($map[$labpoz[x]-1][$labpoz[y]]=="M") OR ($map[$labpoz[x]-1][$labpoz[y]]==" ")) and ($_SESSION['sumrak']==0 ))
					{
					$msg= "Стена..";
					}
					else
					{
					if (($_SESSION['sumrak']>0) and ( ($map[$labpoz[x]-1][$labpoz[y]]=="N")   OR ($labpoz[x]==2) OR ($labpoz[y]==1) OR ($labpoz[y]==strlen($map[1])-2) ))
					  {
				...
				case "p2":
					//$msg="P2<br>";
				if (($map[$labpoz[x]][$labpoz[y]+1]=="1")   and ($_SESSION['sumrak']==0))
					{
				...
				if (($map[$labpoz[x]][$labpoz[y]+1]=="E")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Можно пройти только с другой стороны...</b></font>";
					}
				else
					if (($map[$labpoz[x]][$labpoz[y]+1]=="G")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Лёд треснул...</b></font>";
					//добавляем в итемы отработку ледяной стенки
					mysql_query("INSERT `labirint_items` (`map`,`item`,`x`,`y`,`active`,`count`) values('".$mapid."','G','".$labpoz[x]."','".($labpoz[y]+1)."','1','1' ) ON DUPLICATE KEY UPDATE `active` =1, `count`=`count`+1;");
				...
				if (( ($map[$labpoz[x]][$labpoz[y]+1]=="N")  OR ($map[$labpoz[x]][$labpoz[y]+1]=="M") OR ($map[$labpoz[x]][$labpoz[y]+1]==" ") ) and ($_SESSION['sumrak']==0 ))
					{
					$msg= "<font color=red><b>Стена...</b></font>";
					}
					else
					{
					if (($_SESSION['sumrak']>0) and ( ($map[$labpoz[x]][$labpoz[y]+1]=="N")  OR  ($labpoz[y]==(strlen($map[1])-3)) ) )
					  {
	 				$msg= "<font color=red><b>Стена очень уж большая..</b></font>";
					  }
					  else
				...
				case "p3":
					//$msg="P3<br>";
					if (($map[$labpoz[x]+1][$labpoz[y]]=="E")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Можно пройти только с другой стороны...</b></font>";
					//добавляем в итемы отработку ледяной стенки
					}
				...
				if (( ($map[$labpoz[x]+1][$labpoz[y]]=="N")  OR  ($map[$labpoz[x]+1][$labpoz[y]]=="M") OR ($map[$labpoz[x]+1][$labpoz[y]]==" ") ) and ($_SESSION['sumrak']==0 ))
					...
					{
					if (($_SESSION['sumrak']>0) and ( ($map[$labpoz[x]+1][$labpoz[y]]=="N")  OR  ($labpoz[x]==(count($map)-2)) OR ($labpoz[y]==1) OR ($labpoz[y]==strlen($map[1])-2) ) )
					  {
	 				$msg= "<font color=red><b>Стена очень уж большая..</b></font>";
					  }
					  else
				...
				case "p4":
					//$msg="P4<br>";
				if (($map[$labpoz[x]][$labpoz[y]-1]=="A")   and ($_SESSION['sumrak']==0))
					{
					$msg= "<font color=red><b>Можно пройти только с другой стороны...</b></font>";
					}
				else
				...
				if (( ($map[$labpoz[x]][$labpoz[y]-1]=="N")  OR  ($map[$labpoz[x]][$labpoz[y]-1]=="M") OR ($map[$labpoz[x]][$labpoz[y]-1]==" ")) and ($_SESSION['sumrak']==0 ))
					...
					{
					if (($_SESSION['sumrak']>0) and ( ($map[$labpoz[x]][$labpoz[y]-1]=="N")  OR ($labpoz[y]==2) ))
					  {
	 				$msg= "<font color=red><b>Стена очень уж большая..</b></font>";
					  }
					  else
...
if (($map[$labpoz[x]][$labpoz[y]]!='R') and ($map[$labpoz[x]][$labpoz[y]]!='J')) // не ставим Q-поинт - не ставим тут поинт если моб на клетке!!!
{
   if ($Qmap[$labpoz[x]][$labpoz[y]]!='') // сверка позиции чара и карты итемов
     {
     // если кать квестовый итем на карте то надо проверить отработам или нет
...
<?
$map_echo.=" координаты : X=".$labpoz['x']."  Y=".$labpoz['y']."<br>";
echo $map_echo;
echo "<form method=POST >";
echo "  <div class=\"btn-control\"><input class=\"button-big btn\" type=Submit name=exit value='Выйти и потерять все найденное!' onClick=\"return confirmSubmit()\" ></div></form>";
///Paint map prep
////////////////////////////////////////////////////////////////////////////////
		 ...
		 if ($TA[$i][$j]=='')
			{
				if ($Aitems[$i][$j]!='л')
					{
					 $linte.='<img src=http://i.oldbk.com/llabb/uk2.gif alt="Направление налево" title="Направление налево" >'; // активная хрустальная стенка
					}
					else
					{
					 $linte.='<img src=http://i.oldbk.com/llabb/o'.$LAB.'.gif>'; // отюзаная
		 ...
		 if ($TA[$i][$j]=='')
			{
				if ($Aitems[$i][$j]!='п')
					{
					 $linte.='<img src=http://i.oldbk.com/llabb/uk1.gif alt="Направление направо" title="Направление направо" >'; // активная хрустальная стенка
					}
					else
					{
					 $linte.='<img src=http://i.oldbk.com/llabb/o'.$LAB.'.gif>'; // отюзаная
		 ...
		 if ($TA[$i][$j]=='')
			{
				if ($Aitems[$i][$j]!='2')
					{
//					 $linte.='<img src=http://i.oldbk.com/llabb/2.gif alt="Тайный Вход" title="Тайный Вход" >'; // старьевщик
					 $linte.='<img src=http://i.oldbk.com/llabb/o'.$LAB.'.gif>'; // отюзаная
					}
					else
					{
  ...
  <tr>
    ...
    <td width="99%">
		...
		<div style="width:200px;text-align:center;">
			<img src="http://i.oldbk.com/i/align_<?echo ($user['align']>0 ? $user['align']:"0");?>.gif"><?php if ($user['klan'] <> '') { echo '<img src="http://i.oldbk.com/i/klan/'.$user['klan'].'.gif">'; } ?><B><?=$user['login']?></B> [<?=$user['level']?>]<a href=inf.php?<?=$user['id']?> target=_blank><IMG SRC=http://i.oldbk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT="Инф. о <?=$user['login']?>"></a>
			<?
			echo setHP($user['hp'],$user['maxhp'],1);
			if ($user['maxmana'])
			{
				echo setMP($user['mana'],$user['maxmana'],1);
			}
			?>
		</div>
...
//else
 {
			...
			else
			{
				...
				//делаем
					...
					{
				// рисуем
				echo "<img src=i/magic/event_stat_trap.gif alt='Яд' title='Яд'>-<i>Ослаблены характеристики (Осталось:".$delay."мин.)</i><br>\n";
...
////////////////////////////
			      ...
			      if ( (($Aitems[$labpoz[x]][$labpoz[y]]!='J') and  ($Aitems[$labpoz[x]][$labpoz[y]]!='R'))  and (!$Aitems_val[$labpoz[x]][$labpoz[y]]>0)  )
				       // нету боя на клетке
	{
		//echo "СБ 2<br>";
...
//fix bag ставим пока все не подготовили win=0
	mysql_query("INSERT INTO `battle` (`id`,`coment`,`teams`,`timeout`,`type`,`status`,`t1`,`t2`,`to1`,`to2`,`win`,`t1hist`,`t2hist`)
			...
			if ($room_map=="J")
			{
			mysql_query("INSERT `labirint_items` (`map`,`item`,`x`,`y`,`active`,`val`) values('".$mapid."','J','".$labpoz[x]."','".$labpoz[y]."','1','".$id_battl."' ) ON DUPLICATE KEY UPDATE `active` =1;");
			}
			else
			{
...
//////////////////////
			...
			mysql_query("UPDATE `battle` set win=3 where id={$id_battl};");
				...
				{
				mysql_query("UPDATE `labirint_items` SET `count`=0  WHERE `map`='".$mapid."' and `item`='I' and `x`='".$labpoz[x]."' and `y`='".$labpoz[y]."' and `count`=-1 ;");
				//echo "нету боя - смотрим есть ли дроп тут";
...
///////////////////////////////
		if (($room_map=="Y"))
			...
			{
			echo "<br><img src='http://i.oldbk.com/llabb/kb_y_off.gif' alt='Открытый Ларец' title='Открытый Ларец' border=0>";
			...
			echo "<br><a href=?keybox=get><img src='http://i.oldbk.com/llabb/kb_y_on.gif' alt='Ларец' title='Ларец' border=0></a>";
				}
			echo "<br>";
				}
		else
		if (($room_map=="L"))
			...
			{
			echo "<br><img src='http://i.oldbk.com/llabb/kb_l_off.gif' alt='Открытый Ларец' title='Открытый Ларец' border=0>";
			...
			echo "<br><a href=?keybox=get><img src='http://i.oldbk.com/llabb/kb_l_on.gif' alt='Ларец' title='Ларец' border=0></a>";
				}
			echo "<br>";
				}
		else
		if (($room_map=="W"))
			...
			{
			echo "<br><img src='http://i.oldbk.com/llabb/kb_w_off.gif' alt='Открытый Ларец' title='Открытый Ларец' border=0>";
			//////проверяем  $Aitems_count;
				   if ($Aitems_count[$labpoz[x]][$labpoz[y]]>0)
				   	{
				   	//есть насобраные итемы
			...
			echo "<br><a href=?keybox=get><img src='http://i.oldbk.com/llabb/kb_w_on.gif' alt='Ларец' title='Ларец' border=0></a>";
				}
			echo "<br>";
				}
		else
		if (($room_map=="K"))
			...
			{
			echo "<br><img src='http://i.oldbk.com/llabb/kb_k_off.gif' alt='Открытый Ларец' title='Открытый Ларец' border=0>";
			...
			echo "<br><a href=?keybox=get><img src='http://i.oldbk.com/llabb/kb_k_on.gif' alt='Ларец' title='Ларец' border=0></a>";
				}
			echo "<br>";
				}
		else
///////////////////////////////////
		...
		if (($room_map=="T"))
			{
			echo '<br><img src=http://i.oldbk.com/llabb/port.gif alt="Портал X:'.$labpoz[x].'/Y:'.$labpoz[y].'" title="Портал X:'.$labpoz[x].'/Y:'.$labpoz[y].'" border=0 style=\'background-image:url(http://i.oldbk.com/i/blink.gif);\' >';
			echo "<br>";
				}
		else
		if (($room_map=="п"))
			{
			echo '<br><a href=?chukaz=1><img src=http://i.oldbk.com/llabb/ukaz_1.gif alt="Повернуть указатель" title="Повернуть указатель" border=0 style=\'background-image:url(http://i.oldbk.com/i/blink.gif); cursor: pointer\' ></a>';
			echo "<br>";
				}
		else
		if (($room_map=="л"))
			{
			echo '<br><a href=?chukaz=1><img src=http://i.oldbk.com/llabb/ukaz_2.gif alt="Повернуть указатель" title="Повернуть указатель" border=0 style=\'background-image:url(http://i.oldbk.com/i/blink.gif); cursor: pointer\' ></a>';
			echo "<br>";
				}
		else
		if (($room_map=="в"))
			{
			echo '<br><a href=?chukaz=1><img src=http://i.oldbk.com/llabb/ukaz_3.gif alt="Повернуть указатель" title="Повернуть указатель" border=0 style=\'background-image:url(http://i.oldbk.com/i/blink.gif); cursor: pointer\' ></a>';
			echo "<br>";
				}
		else
		if (($room_map=="н"))
			{
			echo '<br><a href=?chukaz=1><img src=http://i.oldbk.com/llabb/ukaz_4.gif alt="Повернуть указатель" title="Повернуть указатель" border=0 style=\'background-image:url(http://i.oldbk.com/i/blink.gif); cursor: pointer\' ></a>';
			echo "<br>";
				}
		else
		if (($room_map=="F"))
		...
		if (($room_map=="2"))
			{
			echo '<br><A HREF="?talk=75"><img src="http://i.oldbk.com/i/lock.gif" width=20 height=15></A><img src="http://i.oldbk.com/i/align_2.gif"><B>Старьевщик</B> [12]<a href=inf.php?75 target=_blank><IMG SRC=http://i.oldbk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT="Инф. о Старьевщик"></a>';
/*
...
*/
			...
			echo " <b><INPUT TYPE=button onclick=\"location.href='?talk=75';\" value=\"Поговорить\" name=\"talk\" style=\"background-color:#A9AFC0\"><b>";
			echo "<br>";
				}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (get_door_xy($dname))
			{
			echo "<br><B>Дверь закрыта:</b><br>";
			// запрашиваем наличие нужного ключика или универсальный 3333
			 $i_have_key=mysql_fetch_array(mysql_query("select * from oldbk.inventory  where (prototype={$dkey[$did]} OR prototype=3333) and owner='{$user[id]}' and setsale=0 ORDER by labflag DESC , prototype LIMIT 1;"));
			 	if ($i_have_key[id] >0)
			 		{
			 		//echo "есть нужный ключик <br>";
			 		echo "<a href=?useitem={$did}><IMG SRC=\"http://i.oldbk.com/i/sh/{$i_have_key['img']}\" BORDER=0 alt='Использовать {$i_have_key[name]}' title='Использовать {$i_have_key[name]}' style='background-image:url(http://i.oldbk.com/i/blink.gif);' ></a><br>";
			 		}
			 		else
			 		{
			 		echo "<font color=red>У Вас нет подходящего Ключа №".$did."...</font><br>";
			 		}
			}
		}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
?>
  ...
  <tr>
    ...
    <td width="12" ></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab2.php?gto=p1";'><img src="http://i.oldbk.com/i/laba/arr1.gif" border="0" title="Вверх" alt="Вверх" /></a></td>
    <td width="13"></td>
    <td width="26"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="26" height="26"><a href="#" onclick='location.href="lab2.php?gto=p4";'><img src="http://i.oldbk.com/i/laba/arr4.gif" border="0" title="Налево" alt="Налево" /></a></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab2.php?refresh=13.4";'><img src="http://i.oldbk.com/i/laba/refresh.gif" border="0" title="Обновить" alt="Обновить" /></a></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab2.php?gto=p2";'><img src="http://i.oldbk.com/i/laba/arr2.gif" border="0" title="Направо" alt="Направо" /></a></td>
  </tr>
  <tr>
    <td ></td>
    <td></td>
  ...
  <tr>
    <td></td>
    <td></td>
    <td width="26" height="26"><a href="#" onclick='location.href="lab2.php?gto=p3";'><img src="http://i.oldbk.com/i/laba/arr3.gif" border="0" title="Вниз" alt="Вниз" /></a></td>
    <td></td>
    <td></td>
  </tr>
</table>    </td>
