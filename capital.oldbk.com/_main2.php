...
{
	/** @var \components\models\UserSecurity $UserSecurity */
	$UserSecurity = \components\models\UserSecurity::firstOrCreate(['user_id' => $user['id']], ['user_id' => $user['id'], 'security_cooldown' => 0, 'enable_2fa' => 0]);
	...
	<body bgcolor=e2e0e0>
	<FORM ACTION="_main2.php" METHOD=POST>
				...
				<td valign="top" width="50%" valign=top>
					<fieldset style="text-align:justify;">
						...
						if ($_POST['oldpass'] && $_POST['npass'] && $_POST['npass2']) {
							...
							$_db = \components\Component\Db\CapitalDb::connection();
							$_db->beginTransaction();
							try {
							    $_change_password_result = $UserSecurity->changePassword($_POST['oldpass'], $_POST['npass'], $_POST['npass2']);
							    if(!$_change_password_result) {
									throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
                                }
...
<br>
					<fieldset style="text-align:justify;">
						...
						$_second_messages = null;

						switch(true) {
                            case (isset($_POST['second_password_prepare'])):
                                try {
									if($UserSecurity->security_cooldown > time()) {
										throw new Exception('Кулдаун 3ое суток еще не истек', -1);
									}

									$_SESSION['second_pass_generate'] = $UserSecurity->generateSecondPassword($_POST['pwdlenght']);
                                } catch (Exception $ex) {
									if($ex->getCode() == -1) {
										$_second_messages = $ex->getMessage();
									}
                                }

                                break;
                            case (isset($_POST['second_password_set'])):
								$_db = \components\Component\Db\CapitalDb::connection();
								$_db->beginTransaction();
                                try {
									if($second_password != $second_password_confirm) {
										throw new Exception('Пароль не совпадает', -1);
									}

									$_enable_second_result = $UserSecurity->enableSecondPassword();
									if(!$_enable_second_result) {
										throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
									}

									$_db->commit();

									$_second_messages = "Второй пароль установлен";
									$second_password = null;
									unset($_SESSION['second_pass_generate']);
									$app->email->changeSecondPassword($user['email']);
                                } catch (Exception $ex) {
									$_db->rollback();
                                ...
                                break;
                            case (isset($_POST['unset_second_password'])):
								$_db = \components\Component\Db\CapitalDb::connection();
								$_db->beginTransaction();
								try {
									$_disable_second_result = $UserSecurity->disableSecondPassword();
									if(!$_disable_second_result) {
										throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
									}

									$_db->commit();

									$_second_messages = "Второй пароль снят";
						...
						?>
                        <table width="100%">
                            <?php if($_second_messages): ?>
                                <font color=red><b><?= $_second_messages ?></b></font>
                            <?php endif; ?>
                            <tr>
                                <td>
                            ...
                            <tr>
                                <td>
                                    <div class="btn-control">
	                                    <?php if(!$user['second_password']): ?><input class="button-mid btn" type="submit" name="second_password_prepare" value="Установить" /><?php endif; ?>
										<?php if($user['second_password']): ?><input class="button-big btn" type="submit" name="unset_second_password" value="Снять второй пароль" /> <?php endif; ?>
                                    </div>
                                    ...
                                    <br /><small><font color=red><b>Внимание!</b></font> При установке, смене или отключении этого уровня защиты вы не сможете вносить изменения, связанные со вторым паролем или двухфакторной аутентификацией в течение трех суток!</small>
                                </td>
                            </tr>
                        </table>
					</fieldset>
					...
					$_2fa_messages = null;
					switch(true) {
                        case (isset($_POST['prepare_2fa'])):
							$_db = \components\Component\Db\CapitalDb::connection();
							$_db->beginTransaction();
                            try {
								$prepare_2fa = $UserSecurity->prepare2Fa($google2fa);
								if(!$prepare_2fa) {
									throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
								}
								...
								if($ex->getCode() == -1) {
									$_2fa_messages = $ex->getMessage();
								}
                            }

                            break;
                        case (isset($_POST['set_2fa']) && isset($_POST["2fa_code"])):
							$_db = \components\Component\Db\CapitalDb::connection();
							$_db->beginTransaction();
                            try {
								$enable_2fa = $UserSecurity->enable2Fa($google2fa, $_POST["2fa_code"]);
								if(!$enable_2fa) {
									throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
								}

								if(\components\models\UserRewardList::isGotReward($user['id'], \components\models\UserRewardList::REWARD_2FA)) {
									$UserRewardList = new \components\models\UserRewardList();
									$UserRewardList->reward_type = \components\models\UserRewardList::REWARD_2FA;
									if(!$UserRewardList->save()) {
										throw new Exception('Произошла ошибка, попробуй позже', -1);
									}

									$_item_bonus_date_goden = (new DateTime())->modify('+1 month')->getTimestamp();
									$_item_bonus_goden = round(($_item_bonus_date_goden - time()) / 60 / 60 / 24);
									if ($_item_bonus_goden < 1) { $_item_bonus_goden = 1; }
									put_bonus_item(56907, $telo, 'Удача', [], ['goden' => $_item_bonus_goden, 'dategoden' => $_item_bonus_date_goden]);
								}

								$_db->commit();

								$app->email->change2FA($user['email']);
                            } catch (Exception $ex) {
								$_db->rollback();
                            ...
                            break;
                        case (isset($_POST['unset_2fa']) && ($UserSecurity->isEnabled2Fa() || $UserSecurity->isPrepared2Fa())):
							$_db = \components\Component\Db\CapitalDb::connection();
							$_db->beginTransaction();
							try {
								$disable_2fa = $UserSecurity->disable2Fa();
								if(!$disable_2fa) {
									throw new Exception(implode('<br>', $UserSecurity->errors()), -1);
								}
								$_db->commit();
                    ...
                    <fieldset style="text-align:justify;">
                        ...
                        <table width="100%">
                            <?php if($_2fa_messages): ?>
                                <tr>
                                    <td colspan="2">
                                        <font color=red><b><?= $_2fa_messages ?></b></font>
                                    </td>
                                </tr>
                            <?php endif; ?>
                            <tr>
                                <td>
									<?php if($UserSecurity->isDisabled2Fa()): ?>
                                            ...
                                            <tr>
                                                <td>
													...
													$google2fa_url = $google2fa->getQRCodeGoogleUrl(
														...
														$UserSecurity->secret_2fa
													);
													?>
                                                    <img src="<?= $google2fa_url ?>" alt=""><br>
	                                                <?php if($UserSecurity->isPrepared2Fa()): ?>
                                                        <div>
                                                            Secret key: <?= $UserSecurity->secret_2fa ?>
                                                        </div>
                                                    <?php endif; ?>
...
if($_GET['effects'])
{
	...
	<div id="page-wrapper">
		...
		elseif ($_GET['cancel']==656)
		{
			//удаляем эффект если он есть
			mysql_query("DELETE from effects where owner='{$user['id']}' and type=656 LIMIT 1;");
			if (mysql_affected_rows()>0)
			{
				$out_buff="Эффект \"«Иммунитет»\" - Удален!<br>";
			}
		}		
     ...
     Таймер остатка времени бонуса в мм., если в остатке меньше 60 минут. Пример: Осталось 14 мин.			
		...
		<table align="center" class="table-list sostoyanie" cellspacing="0" cellpadding="0">
			...
			if (mysql_num_rows($q) > 0) {
				...
				echo render_sost_row('<img src="http://i.oldbk.com/i/fighttype7.gif" alt=""><br> <b>Ёлочное безумие</b></a>','Вам необходимо одержать победу в 25-ти елочных хаотичных боях в полном комплекте (13 вещей, не считая рун). Бой будет засчитан, если вами будет нанесен урон. <b>('.$nyq['val'].'/25)</b> <br>Награда: 6000 репутации, Волшебная шляпа.','<a href="?effects=1&cancel=60006000">Отказаться</a>');
			}
...
//															$addinfo_kof=round($paskill[$_GET['skill']]['bylevel']*$naem['level']*0.01,2);
											...
											foreach($paskill as $ss => $skd)
											{
												...
												if ($skd['id']==20001) 
													...
													elseif ($naem['passkills_points']>0 and ($skd['procent']<5*($skd['by_point'])) ) 
													{ 
