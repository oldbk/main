...
function add_to_new_delo($rec)
{
		...
		if  ((($rec['item_type']>0 && $rec['item_type']<12 || $rec['item_type']==28 || $rec['item_type']==555 || $rec['item_type']==27 || (in_array($rec['item_proto'],$sert))) && $rec['type']!=32 && $rec['type']!=33) OR ($rec['type']==1179 || $rec['type']==1180 ||  $rec['type']==1181 ||  $rec['type']==179 || $rec['type'] == 1303) )
...
//Предметы
	{
		...
		//получил/подарили
		{
			...
			$delo_type[267]="У \"{$rec['owner_login']}\" украли лошадь за городом.";
		    	...
		    	$delo_type[2284]="\"{$rec['owner_login']}\" заплатил {$rec['add_info']} монет за оплату услуг Коммерческого отдела";
 ...
 //Аукцион
	...
	$delo_type[1304]="\"{$rec['owner_login']}\" неудачно закончил производство \"{$rec['item_name']}\"";
...
// вывод
	if ($al[perevodi]>=5 && $add_info_off==2)
	{
	  ...
	  //админское время
		...
		if ($rec['type'] == 508 || $rec['type'] == 506 || $rec['type'] == 519) {
			$t = explode("/",$rec['add_info']);
